...
$pageCss = 'assets/css/exam-ui.css';          /* v4.129.0 */
...
#webcamPreview .label { position:absolute; top:3px; left:3px; background:rgba(239,68,68,.92); color:white; font-size:9px; padding:2px 6px; border-radius:6px; display:inline-flex; align-items:center; gap:4px; }
/* نقطهٔ ضبط با انیمیشن — به‌جای ایموجی 🔴 */
.rec-dot{ width:6px; height:6px; border-radius:50%; background:#fff; display:inline-block; animation:recPulse 1.6s ease-in-out infinite; }
...
/* v4.128.0: در موبایل نقشهٔ سوالات از حالت fixed خارج می‌شود.
   قبلاً bottom:15px/right:15px ثابت بود و دقیقاً روی دکمهٔ «ارسال نهایی آزمون»
   می‌نشست و آن را می‌پوشاند. حالا در جریان سند قرار می‌گیرد و وسط‌چین است. */
/* ── v4.129.0: آیکون‌ها به‌جای ایموجی ───────────────────────────────
   ایموجی بین پلتفرم‌ها متفاوت رندر می‌شود، هم‌ترازی عمودی‌اش قابل کنترل
   نیست و در بعضی فونت‌های فارسی فاصلهٔ عجیب می‌اندازد. svg هیچ‌کدام از
   این مشکل‌ها را ندارد و رنگش از متن می‌آید. */
.ex-chip svg, .submit-hero svg, .q-done-tick svg, .timer-pill svg { vertical-align:-0.18em; }
.q-done-tick{font-size:0}                 /* تیک متنی قدیمی دیده نشود */
.q-done-tick svg{width:13px;height:13px}

@media (max-width:768px){
  .q-map{position:static;max-width:none;width:100%;margin:0 0 8px;box-shadow:none;backdrop-filter:none;border-radius:12px}
  .q-map-title{text-align:center;font-size:11px}
  .q-map-grid{max-height:104px;justify-content:center}
  .q-map-btn{width:38px;height:38px;font-size:13px;border-radius:11px}   /* هدف لمسی بزرگ‌تر */
  .exam-topbar{padding:8px 10px;gap:8px}
  .exam-topbar .ex-title{flex:1 0 100%;font-size:14px;min-width:0}      /* عنوان یک خط کامل */
  .timer-pill{font-size:13px;padding:6px 12px}
  .question-box{margin-bottom:14px;border-radius:14px}
  .q-body{padding:14px}
  /* دکمهٔ ارسال تمام‌عرض و با ارتفاع لمسی استاندارد */
  .submit-hero{width:100%;padding:16px 20px;font-size:16px;border-radius:14px}
  /* پیش‌نمایش وب‌کم کوچک‌تر تا روی محتوا نیفتد؛ فقط از دستگیره جابه‌جا می‌شود */
  #webcamPreview{width:104px;height:78px;bottom:calc(12px + env(safe-area-inset-bottom,0px));left:12px;border-radius:11px}
  #webcamPreview .drag-hint{display:none}
  /* پاسخ‌های گزینه‌ای راحت‌تر لمس شوند */
  .opt-row{padding:13px 12px}
}
/* ارتفاع واقعی viewport در موبایل (منوی آدرس جمع/باز می‌شود) */
@supports (height: 100dvh) { .exam-stage-min { min-height: 100dvh; } }
@media print { body { display:none; } }
</style>

<div class="watermark-overlay" id="watermarkOverlay"></div>
<!-- v4.129.0: پیش‌نمایش وب‌کم.
     «بکشید برای جابجایی» روی گوشی حذف شد (جا نمی‌گرفت و خوانا نبود) و
     جابه‌جایی فقط از دستگیرهٔ بالایی انجام می‌شود. -->
<div id="webcamPreview" class="hidden">
    <span class="label" id="webcamDragHandle" style="cursor:move;touch-action:none;user-select:none;-webkit-user-select:none">
        <span class="rec-dot"></span> وب‌کم فعال
    </span>
    <video id="webcamVideo" autoplay muted playsinline></video>
    <canvas id="webcamCanvas" class="hidden"></canvas>
    <div class="drag-hint" style="position:absolute;bottom:2px;right:2px;background:rgba(0,0,0,0.6);color:white;font-size:8px;padding:1px 3px;border-radius:3px;">بکشید تا جابه‌جا شود</div>
</div>
...
<div class="max-w-5xl mx-auto space-y-4 anti-copy" id="examContainer">
    ...
    <div id="examStage" class="hidden space-y-4">
        ...
        <div class="exam-topbar">
            ...
            <span class="ex-chip"><?php em_icon('list', 'ic ic-sm'); ?> <?php echo tr_num(count($questions),'fa'); ?> سوال</span>
            <span class="ex-chip"><?php em_icon('shield', 'ic ic-sm'); ?> <?php echo tr_num($maxScore,'fa'); ?> نمره</span>
            <span class="autosave-dot" id="autosaveDot"><i></i><span id="autosaveText">ذخیره خودکار فعال</span></span>
            <span class="timer-pill" id="timerDisplay"><svg class="ic ic-sm" aria-hidden="true"><use href="#i-clock"></use></svg> <?php echo gmdate('H:i:s', $remaining); ?></span>
        </div>
        <div>
            ...
            <div class="question-box" data-qid="<?php echo $q['id']; ?>" id="qbox_<?php echo $q['id']; ?>">
                <div class="q-head">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <span class="q-done-tick"><?php em_icon('check', 'ic'); ?></span>
                    </div>
                </div>
                <div class="q-body">
                ...
                <?php elseif($q['question_type']==='whiteboard'): ?>
                    <div class="space-y-2 whiteboard-wrapper" data-qid="<?php echo $q['id']; ?>">
                        <div class="flex flex-wrap gap-1 items-center p-2 bg-slate-50 rounded border text-xs">
                            ...
                            <?php /* v4.128.0: دکمه‌های بزرگنمایی/کوچکنمایی حذف شدند.
                                   علت: CSS transform باعث می‌شد بوم از قاب بیرون بزند ولی
                                   ناحیهٔ اسکرول ظرف بزرگ نشود، در نتیجه بخشی از تخته
                                   غیرقابل دسترسی می‌شد و دانش‌آموز نمی‌توانست نقاشی کند. */ ?>
                            <button type="button" class="btn btn-success text-[10px]" onclick="saveWhiteboard(<?php echo $q['id']; ?>)">ذخیره نقاشی</button>
                            <span class="text-[10px] text-muted">تخته به اندازهٔ قاب است — اسکرول ندارد</span>
                        </div>
                        <?php /* v4.128.0: تخته دقیقاً به اندازهٔ قاب — بدون اسکرول، بدون
                               transform، با وضوح داخلی ثابت تا خطوط تار نشوند. */ ?>
                        <div class="whiteboard-container" style="width:100%;overflow:hidden;border:1px solid #e2e8f0;border-radius:8px;background:#fff;position:relative;line-height:0;">
                            <canvas class="whiteboard-canvas border-0 bg-white touch-none" width="1000" height="560" data-qid="<?php echo $q['id']; ?>" style="display:block;width:100%;height:auto;touch-action:none;cursor:crosshair;"></canvas>
                        </div>
                    </div>
                <?php endif; ?>
                </div>
        ...
        <?php /* v4.128.0: نقشه سوالات «بالای» دکمه ارسال قرار گرفت — هم در موبایل
               و هم در دسکتاپ ترتیب منطقی‌تر است و هیچ‌وقت روی دکمه نمی‌نشیند. */ ?>
        ...
        <div class="text-center py-8">
            <button type="button" onclick="submitExam()" class="submit-hero">
                <svg class="ic ic-lg" aria-hidden="true"><use href="#i-send"></use></svg> ارسال نهایی آزمون
            </button>
            <div class="em-hint mt-3">پیش از ارسال، از نقشهٔ سوالات بالا مطمئن شوید همه را پاسخ داده‌اید.</div>
        </div>
    </div>
</div>
...
/* v4.129.0 — آیکون svg در متن‌هایی که با JS ساخته می‌شوند.
   به‌جای ✅/❌ که بین پلتفرم‌ها متفاوت رندر می‌شوند. */
function ic(name, cls){ return '<svg class="'+(cls||'ic')+'" aria-hidden="true"><use href="#i-'+name+'"></use></svg>'; }
function emState(kind, text){ return '<span class="em-state is-'+kind+'">'+text+'</span>'; }
...
async function checkCamera(){
    try {
        ...
        document.querySelector('#check_camera .status').innerHTML=emState('ok', ic('check','ic ic-sm')+' تایید شد');
        document.getElementById('check_camera').classList.add('bg-green-50');
        s.getTracks().forEach(t=>t.stop());
        checkAllPermissionsDone();
    } catch(e){
        document.querySelector('#check_camera .status').innerHTML=emState('bad', ic('x','ic ic-sm')+' '+e.message);
...
async function checkMic(){
    try {
        ...
        document.querySelector('#check_mic .status').innerHTML=emState('ok', ic('check','ic ic-sm')+' تایید شد');
        ...
        if(e.name==='NotFoundError' || msg.includes('Requested device not found') || msg.includes('Not found')){
            document.querySelector('#check_mic .status').innerHTML=emState('warn', ic('mic-off','ic ic-sm')+' میکروفون یافت نشد (دسکتاپ) — با IP ادامه می‌دهیم');
            ...
            if(!requireMic){
                ...
                document.querySelector('#check_mic .status').innerHTML=emState('warn', ic('mic-off','ic ic-sm')+' میکروفون ندارد ولی الزامی نیست — ادامه مجاز است');
                checkAllPermissionsDone();
            } else {
                document.querySelector('#check_mic .status').innerHTML=emState('bad', ic('mic-off','ic ic-sm')+' میکروفون الزامی است ولی یافت نشد: '+msg)
                    +' <button type="button" onclick="continueWithoutMic()" class="em-btn em-btn--ghost em-btn--sm">ادامه بدون میکروفون</button>';
            ...
            document.querySelector('#check_mic .status').innerHTML=emState('bad', ic('x','ic ic-sm')+' '+msg);
...
function continueWithoutMic(){
    permissionState.mic=true;
    document.querySelector('#check_mic .status').innerHTML=emState('ok', ic('check','ic ic-sm')+' ادامه بدون میکروفون');
    document.getElementById('check_mic').classList.add('bg-amber-50');
...
async function checkLocationRobust(){
    ...
    const finishOk = (mode) => {
        ...
        if(statusEl) statusEl.innerHTML = emState('ok', ic('check','ic ic-sm')+' تایید شد');
        if(detailEl) detailEl.textContent = '';
...
function checkInternet(){
    ...
    fetch('online-exam-media-upload.php?check='+Date.now(), {method:'HEAD', cache:'no-store'}).then(()=>{
        ...
        statusEl.innerHTML=emState('ok', ic('check','ic ic-sm')+' '+duration+'ms — '+quality);
        document.getElementById('check_internet').classList.add('bg-green-50');
        checkAllPermissionsDone();
    }).catch(()=>{
        statusEl.innerHTML=emState('warn', ic('alert','ic ic-sm')+' خطا در اتصال — ولی ادامه می‌دهیم');
...
async function requestAllPermissions(){
    ...
    try {
        ...
        try {
            ...
            document.querySelector('#check_camera .status').innerHTML=emState('bad', ic('x','ic ic-sm')+' '+ee.message);
        }
    }
    checkLocationRobust();
    checkInternet();
...
async function startExamAfterChecks(){
    ...
    try {
        ...
        fd.append('device_token', deviceToken());   /* v4.128.0 */
        let res = await fetch('online-exam-api.php',{method:'POST',body:fd});
        let j = await res.json();
        /* v4.128.0: اگر دستگاه دیگری آزمون را گرفته باشد، اینجا متوقف می‌شویم */
        if(j && !j.ok && j.data && j.data.reason === 'other_device'){
            blockExamDevice(j.msg || 'این آزمون روی دستگاه دیگری در حال برگزاری است.', j.data.retry_after);
            return;
        }
...
function playNextVoice(){
    ...
    bn.innerHTML=ic('mic','ic ic-sm')+' '+(vn.target?'تذکر صوتی دبیر (مخصوص شما)':'پیام صوتی دبیر')+' — در حال پخش…';
    ...
    au.play().catch(()=>{
        ...
        bn.innerHTML=ic('mic','ic ic-sm')+' پیام صوتی دبیر — برای پخش یک بار روی صفحه کلیک یا لمس کنید';
...
function startHeartbeat(){
    ...
    async function beat(){
        ...
        try{
            let res = await fetch('online-exam-api.php',{method:'POST',body:fd});
            let j = await res.json();
            /* v4.128.0: اگر دستگاه دیگری مالک آزمون شده، این صفحه کنار می‌رود */
            if(j && j.ok && j.data && j.data.kicked){
                blockExamDevice(j.msg || 'این آزمون روی دستگاه دیگری در حال برگزاری است.', null);
                return;
            }
...
/* v4.128.0: zoomWhiteboard و resetZoomWhiteboard حذف شدند.
   بوم حالا با CSS width:100% دقیقاً داخل قاب می‌نشیند، پس بزرگنمایی معنا ندارد.
   نسبت «وضوح داخلی به اندازهٔ نمایش» در displayScale() محاسبه می‌شود تا ضخامت
   قلم روی هر دستگاهی یکسان به نظر برسد. */
...
document.querySelectorAll('.whiteboard-canvas').forEach(canvas=>{
    ...
    function drawLine(x,y){
        ...
        /* v4.128.0: ضخامت انتخابی کاربر «پیکسل نمایشی» است؛ چون بوم وضوح داخلی
           بالاتری دارد و با CSS کوچک نمایش داده می‌شود، در مقیاس ضرب می‌کنیم تا
           روی صفحه دقیقاً همان‌قدر که کاربر انتخاب کرده دیده شود. */
        ctx.lineWidth = Math.max(1, st.width * displayScale(canvas));
        if(st.tool==='eraser'){
...
(function(){
    ...
    function onStart(e){
        /* v4.129.0: فقط از دستگیره جابه‌جا می‌شود. پیش از این کل کادر
           دستگیره بود و touchstart با preventDefault ثبت می‌شد؛ preventDefault
           روی touchstart «کلیک مصنوعی» بعدی را لغو می‌کند — همان باگی که
           دکمه‌های پنل وب‌کم را روی گوشی از کار انداخته بود. حالا به‌جایش
           touch-action:none روی دستگیره است: اسکرول را می‌گیرد ولی کلیک را نه. */
        if(!(e.target.closest && e.target.closest('#webcamDragHandle'))) return;
        isDragging=true;
        let pos = getPos(e);
...
/* ══════════ v4.128.0: قفل تک‌دستگاهی ══════════
   هر مرورگر یک شناسهٔ پایدار در localStorage دارد. قبل از شروع آزمون این
   شناسه روی سرور ثبت می‌شود؛ اگر دستگاه دیگری در حال آزمون باشد، این صفحه
   قفل می‌شود و دانش‌آموز راهنمایی می‌شود. */
function deviceToken(){
    try {
        let t = localStorage.getItem('sdp_device_token');
        if(!t || t.length < 16){
            t = 'dev_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2,10) + Math.random().toString(36).slice(2,10);
            localStorage.setItem('sdp_device_token', t);
        }
        return t;
    } catch(e){ return 'dev_nostorage_' + Math.random().toString(36).slice(2,12); }
}
function deviceLabel(){
    let n = (navigator.userAgent || '').slice(0,120);
    let w = window.screen ? (window.screen.width + 'x' + window.screen.height) : '';
    return (n + ' | ' + w).slice(0,160);
}
function blockExamDevice(msg, retryAfter){
    if(document.getElementById('deviceLockOverlay')) return;
    let ov = document.createElement('div');
    ov.id = 'deviceLockOverlay';
    ov.style.cssText = 'position:fixed;inset:0;z-index:2000;background:rgba(15,23,42,.94);display:flex;align-items:center;justify-content:center;padding:20px;';
    let box = document.createElement('div');
    box.style.cssText = 'max-width:440px;background:#fff;border-radius:16px;padding:24px;text-align:center;font-family:inherit;';
    box.innerHTML =
        '<div style="display:flex;justify-content:center;color:#4f46e5"><svg style="width:44px;height:44px" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24" aria-hidden="true"><rect x="4" y="10.5" width="16" height="10.5" rx="2.4"/><path d="M8 10.5V7.6a4 4 0 0 1 8 0v2.9"/></svg></div>' +
        '<div style="font-weight:800;font-size:17px;margin:8px 0">آزمون روی دستگاه دیگری در حال برگزاری است</div>' +
        '<div style="color:#475569;font-size:13.5px;line-height:1.9">' + msg + '</div>' +
        '<div id="deviceLockCount" style="color:#0369a1;font-size:12.5px;margin-top:12px"></div>' +
        '<button onclick="location.reload()" style="margin-top:16px;background:#4f46e5;color:#fff;border:0;border-radius:10px;padding:10px 22px;font-weight:700;font-family:inherit;cursor:pointer">تلاش دوباره</button>';
    ov.appendChild(box);
    document.body.appendChild(ov);
    /* پنهان‌کردن محتوای آزمون تا راه دور زدن نماند */
    let c = document.getElementById('examContainer');
    if(c) c.style.display = 'none';
    let sb = document.getElementById('startExamBtn');
    if(sb) sb.classList.add('hidden');
    if(retryAfter){
        let left = retryAfter;
        let el = document.getElementById('deviceLockCount');
        let iv = setInterval(()=>{
            left--;
            if(el) el.textContent = 'تا امکان تلاش دوباره: ' + left + ' ثانیه';
            if(left<=0){ clearInterval(iv); if(el) el.textContent=''; }
        }, 1000);
    }
}
async function claimDeviceSlot(){
    try{
        let fd = new FormData();
        fd.append('action','claim_device');
        fd.append('attempt_id',attemptId);
        fd.append('device_token',deviceToken());
        fd.append('device_label',deviceLabel());
        let r = await fetch('online-exam-api.php',{method:'POST',body:fd});
        let j = await r.json();
        if(j && j.ok && j.data && j.data.allowed === false){
            blockExamDevice(
                'این آزمون هم‌اکنون روی دستگاه دیگری باز است. برای ادامه، به همان دستگاه برگردید.<br>' +
                'اگر از آن دستگاه خارج شده‌اید، کمی صبر کنید تا قفل آزاد شود.',
                j.data.retry_after);
            return false;
        }
        return true;
    }catch(e){
        /* اگر سرور در دسترس نبود دانش‌آموز را قفل نمی‌کنیم — start_attempt خودش
           سمت سرور جلوی دستگاه دوم را می‌گیرد. */
        return true;
    }
}
...
document.addEventListener('DOMContentLoaded', async ()=>{
    initWatermark();
    const allowed = await claimDeviceSlot();
    if(!allowed) return;
