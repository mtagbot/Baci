 ...
 */
list(, , $_backArt) = card_theme_art($D['theme'] ?? 'classic');
$_layout = $D['layout'] ?? 'full';
$_layCls = ($_layout === 'sq') ? 'sq' : (($_layout === 'qrmax') ? 'qrmax' : '');
?>
<div class="card-back<?php echo $_layCls !== '' ? ' ' . clean($_layCls) : ''; ?><?php echo !empty($D['cut']) ? ' cut' : ''; ?>">
    <div class="card-back-in">
        ...
        <svg class="card-back-orn" viewBox="0 0 100 100" aria-hidden="true"><use href="#orn-<?php echo clean($_backArt); ?>" xlink:href="#orn-<?php echo clean($_backArt); ?>"></use></svg>
