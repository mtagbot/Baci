...
if (!function_exists('bot_exam_q_plain')) {
    ...
    function bot_exam_q_plain($html) {
        ...
        /* v4.105.0: کسر پله‌ای فارسی (pfrac) → صورت/مخرج در متن ساده */
        $html = preg_replace('/<span class="pfrac"[^>]*>\s*<span[^>]*>([^<]*)<\/span>\s*<span[^>]*>([^<]*)<\/span>\s*<\/span>/iu', ' $1/$2 ', $html);
...
if (!function_exists('bot_exam_design_pdf_path')) {
    ...
    function bot_exam_design_pdf_path($examId) {
        ...
        $noColW = (int)round(8 * $S); $scoreColW = (int)round(10 * $S); /* v4.114.0: هماهنگ با ویرایشگر — ستون‌های باریک‌تر */
        ...
        for ($p = 1; $p <= $totalDesignPages; $p++) {
            ...
            if ($pageRows) {
                ...
                gd_text($qc, 'ردیف', $tw - (int)($noColW / 2), $thBase, $thPt, $ink, true, 'center'); /* v4.114.0 */
        ...
        foreach ($students as $si => $st) {
            ...
            /* v4.118.0: قاب دوخطه رسمی — بیرونی ضخیم + داخلی نازک */
            $dbl = max(2, (int)round(0.5 * $S));
            for ($t0 = 0; $t0 < max(2, (int)round(0.35 * $S)); $t0++) imagerectangle($strip, $t0, $t0, $W - 1 - $t0, $hH - 1 - $t0, $ink);
            $inGap = max(3, (int)round(0.7 * $S));
            imagerectangle($strip, $inGap, $inGap, $W - 1 - $inGap, $hH - 1 - $inGap, $ink);
            ...
            /* v4.118.0: خطوط تزئینی دو طرف عنوان (دوخطه ظریف) */
            $tW2 = function_exists('imagettfbbox') ? 0 : 0;
            $lnW = (int)round(20 * $S); $lnGapY = (int)round($titlePt / 0.75 * 0.32);
            $tbb = imagettfbbox($titlePt, 0, report_image_font_path(false), rtl_shape_persian_for_gd(tr_num($exTitle,'fa')));
            $titleHalf = (int)(abs($tbb[4] - $tbb[0]) / 2) + (int)round(2.5 * $S);
            $lyy = $yCur - $lnGapY;
            $cxm = $cx1 + (int)($cw / 2);
            if ($cxm - $titleHalf - $lnW > $cx1) {
                imageline($strip, $cxm - $titleHalf - $lnW, $lyy, $cxm - $titleHalf, $lyy, $ink);
                imageline($strip, $cxm - $titleHalf - $lnW, $lyy + max(2,(int)round(0.35*$S)), $cxm - $titleHalf, $lyy + max(2,(int)round(0.35*$S)), $ink);
                imageline($strip, $cxm + $titleHalf, $lyy, $cxm + $titleHalf + $lnW, $lyy, $ink);
                imageline($strip, $cxm + $titleHalf, $lyy + max(2,(int)round(0.35*$S)), $cxm + $titleHalf + $lnW, $lyy + max(2,(int)round(0.35*$S)), $ink);
            }
            ...
            foreach ($fieldsOn as $fi => $f) {
                ...
                /* v4.118.0: برچسب کوچک سمت راست + مقدار وسط باقی کادر */
                $labPt = 10 * $PX * 0.75;
                $labTxt = (string)($f['label'] ?? '') . ':';
                gd_text($strip, $labTxt, $bx2 - (int)round(1 * $S), $by1 + (int)round($boxH * 0.70), $labPt, $ink, false, 'right');
                $lbb = imagettfbbox($labPt, 0, report_image_font_path(false), rtl_shape_persian_for_gd(tr_num($labTxt,'fa')));
                $labW = (int)abs($lbb[4] - $lbb[0]);
                $vx = $bx1 + (int)(($boxW - $labW - (int)round(1 * $S)) / 2);
                gd_text($strip, (string)$val, $vx, $by1 + (int)round($boxH * 0.72), $fieldPt, $ink, false, 'center');
            }
