...
<div id="printQualityModal" class="upload-modal" style="display:none">
  <div class="upload-card">
    ...
    <div class="upload-actions"><button type="button" class="btn btn-secondary" onclick="closePrintQualityModal()">انصراف</button><button type="button" class="btn btn-primary" onclick="startQualityPrint()">آماده‌سازی و چاپ</button></div>
  </div>
</div>
...
function prepareAllStudents(){
  // Performance-critical print preparation (v4.77.0 optimized for weak/old systems):
  ...
  // 4) v4.77.0: the whole print DOM is built as ONE string and assigned in a
  //    single innerHTML pass (the old per-page insertAdjacentHTML forced a
  //    reflow for every page — with 30 students × 4 pages that was 120 reflows).
  // 5) v4.77.0: blank <canvas> elements are stripped from the clones; the
  //    handwriting is already baked in as a static overlay image. Hundreds of
  //    live canvases were the main crash cause on old systems.
  collectDrawings();
  ...
  const designedWraps=firstPages.map(p=>{
    ...
    c.querySelectorAll('img').forEach(x=>{x.setAttribute('decoding','async');});
    return c.outerHTML;
  });
  previewOnly=false;
  const parts=[];
  studentsData.forEach(st=>{
    designedWraps.forEach((wrap,idx)=>{
      const pageNo=idx+1;
      parts.push(`<div class="page ${pageNo%2===0?'no-header':''}" data-student-id="${st.id}" data-page-index="${pageNo}">${headerHtml(st,pageNo)}${wrap}</div>`);
    });
  });
...
function openPrintQualityModal(){
  ...
  const pages=Math.max(1,sourcePageCount()); const total=pages*studentsData.length;
...
function closePrintQualityModal(){document.getElementById('printQualityModal').style.display='none';}
function startQualityPrint(){
  ...
  closePrintQualityModal();
  printSavePromptPending=true;
  ...
  const finish=()=>{waitForPrintReady(()=>{btns.forEach(b=>{b.disabled=false;if(b.dataset.oldTxt)b.textContent=b.dataset.oldTxt;}); window.print();});};
  prepareAllStudents();
  degradePrintImages(q,finish);
}
function degradePrintImages(q,cb){
  /* هر تصویر یکتا فقط «یک بار» پردازش می‌شود و نتیجه برای تمام
     دانش‌آموزان به اشتراک گذاشته می‌شود (کپی‌ها همان src را دارند). */
  const conf={high:null, medium:{w:1400,q:0.8}, low:{w:900,q:0.62}}[q];
  if(!conf){cb();return;}
  const imgs=[...document.querySelectorAll('#pagesRoot .qwrap img')];
  const bySrc={};
  ...
  const done=(src,out)=>{ if(out){bySrc[src].forEach(im=>{im.src=out;});} if(--left<=0)cb(); };
  srcs.forEach(src=>{
    const probe=new Image();
    probe.onload=()=>{
      try{
        const scale=Math.min(1, conf.w/Math.max(1,probe.naturalWidth));
        if(scale>=1){done(src,null);return;}   // already small enough
        ...
        let out=c.toDataURL('image/webp',conf.q);
        if(out.indexOf('data:image/webp')!==0){
          // webp پشتیبانی نشد: دست‌نویس‌ها (نیازمند شفافیت) دست‌نخورده بمانند،
          // بقیه با JPEG روی زمینه سفید سبک شوند.
          const needsAlpha=bySrc[src].some(im=>im.classList.contains('drawOverlay'));
          if(needsAlpha){done(src,null);return;}
          const c2=document.createElement('canvas'); c2.width=c.width; c2.height=c.height;
          const x2=c2.getContext('2d'); x2.fillStyle='#fff'; x2.fillRect(0,0,c2.width,c2.height); x2.drawImage(probe,0,0,c2.width,c2.height);
          out=c2.toDataURL('image/jpeg',conf.q);
        }
        done(src,out);
      }catch(e){done(src,null);}
    };
    probe.onerror=()=>done(src,null);
    probe.src=src;
  });
}
