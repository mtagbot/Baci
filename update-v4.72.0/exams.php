...
if ($_SERVER['REQUEST_METHOD']==='POST') {
 ...
 if(isset($_POST['auto_assign_seats'])){
  ...
  $all=DB::fetchAll("SELECT id,first_name,last_name,class_name,grade_level FROM students WHERE status='active'");
  $byGrade=[];
  foreach($all as $st){
  ...
  foreach($byGrade as $g=>$rows){
    ...
    usort($classKeys, fn($a,$b)=> class_number_weight($a) <=> class_number_weight($b) ?: strcmp(norm_persian_str($a), norm_persian_str($b)));
    $num=1;
...
?>
<div class="space-y-6"><?php /* v4.61.0: teacher hero + app-style grid on every teacher page */ require_once __DIR__ . '/includes/teacher_nav.php'; render_teacher_nav(); ?><div class="page-hero flex justify-between items-center"><div><h2 class="text-2xl font-bold">📝 امتحانات</h2><p class="text-sm text-muted">مدیریت برنامه امتحانی، صندلی، کلاس امتحان و چاپ سربرگ سوالات</p></div><div class="flex gap-2"><a class="btn <?php echo $tab==='schedule'?'btn-primary':'btn-outline'; ?>" href="exams.php?tab=schedule&year=<?php echo urlencode($year); ?>">برنامه امتحانی</a><a class="btn <?php echo $tab==='assign'?'btn-primary':'btn-outline'; ?>" href="exams.php?tab=assign&year=<?php echo urlencode($year); ?>">صندلی و کلاس</a><a class="btn <?php echo $tab==='print'?'btn-primary':'btn-outline'; ?>" href="exams.php?tab=print&year=<?php echo urlencode($year); ?>">چاپ</a><?php if(!(function_exists('current_teacher_is_executive') && current_teacher_is_executive() && !is_admin_logged_in())): ?><a class="btn btn-warning" href="exam-question-bank.php">بانک سوالات</a><?php endif; ?></div></div>
<?php if($tab==='schedule'): ?>
...
<?php else: ?><div class="card"><h3 class="font-bold mb-3">چاپ برنامه، سوالات و کارت صندلی</h3><div class="flex gap-2 mb-3"><a target="_blank" class="btn btn-success" href="exam-print.php?type=seatcards&year=<?php echo urlencode($year); ?>">چاپ کارت صندلی (شماره‌های سراسری سال)</a></div><?php
...
$pxSeen = [];
?><div class="table-container"><table><thead><tr><th>امتحان</th><th>برنامه جدولی</th><th>طراحی سوال کلاس</th><th>سوالات با سربرگ</th><th>چاپ پایه‌ای (مشترک کلاس‌های پایه)</th><th>چاپ شده؟</th></tr></thead><tbody><?php foreach($printExams as $e):
