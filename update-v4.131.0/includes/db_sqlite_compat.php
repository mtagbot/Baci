...
class SQLiteCompat {
    ...
    public static function rewrite($sql) {
        ...
        /* v4.131.0: MySQL's NULL-safe equality `a <=> b` has no SQLite
           equivalent operator, but `a IS b` behaves identically there
           (SQLite's IS compares NULLs as equal). Without this, every
           query using <=> was a syntax error on the desktop — it hit the
           de-duplication step of db-optimizer.php, which compares
           academic_year columns that are routinely NULL. */
