...
?>
<div class="space-y-6">
    <!-- v4.60.0: self-contained hero styles — the old Tailwind-like utility
         classes (bg-gradient-to-l / from-green-600 / bg-white / text-green-700)
         do NOT exist in this project's CSS, which made white text sit on a
         white card. Everything below uses explicit CSS. -->
    <style>
    .tp-hero{background:linear-gradient(135deg,#059669,#065f46);color:#fff;border-radius:18px;padding:22px;box-shadow:0 10px 25px rgba(5,150,105,.28)}
    ...
    <div class="tp-hero">
        <div>
            ...
            <p class="tp-meta">کد ملی: <span class="font-mono"><?php echo tr_num($teacher['national_id'], 'fa'); ?></span> | کد پرسنلی: <b><?php echo clean($teacher['personnel_code']); ?></b></p>
            <form method="GET" class="mt-2"><select name="year" class="form-select text-xs" style="max-width:160px" onchange="this.form.submit()"><?php foreach($teacherYears as $ty): ?><option value="<?php echo clean($ty['academic_year']); ?>" <?php echo $teacherYear===$ty['academic_year']?'selected':''; ?>><?php echo clean($ty['academic_year']); ?></option><?php endforeach; ?></select></form>
        </div>
        <div class="tp-grid">
            <a href="teacher-panel.php?tab=schedule&year=<?php echo urlencode($teacherYear); ?>" class="tp-tile"><span class="ic">📅</span>برنامه هفتگی من</a>
            <a href="teacher-panel.php?tab=grading&year=<?php echo urlencode($teacherYear); ?>" class="tp-tile"><span class="ic">📝</span>ثبت و ویرایش نمرات</a>
            <a href="teacher-panel.php?tab=messages&year=<?php echo urlencode($teacherYear); ?>" class="tp-tile"><span class="ic">💬</span>اعتراضات و پیام‌ها</a>
            <a href="teacher-panel.php?tab=exams&year=<?php echo urlencode($teacherYear); ?>" class="tp-tile"><span class="ic">🖊️</span>طراحی سوالات تشریحی</a>
            <a href="online-exams.php" class="tp-tile"><span class="ic">🧪</span>آزمون‌های آنلاین</a>
            <a href="online-question-bank.php" class="tp-tile"><span class="ic">📚</span>بانک سوالات آنلاین</a>
            <?php if (!empty($teacher['is_deputy'])): ?><a href="deputy-panel.php" class="tp-tile"><span class="ic">🛡️</span>پنل معاونت</a><?php endif; ?>
            <?php if (!empty($teacher['is_counselor'])): ?><a href="counselor-panel.php" class="tp-tile"><span class="ic">🧭</span>پنل مشاور</a><?php endif; ?>
            <?php if (!empty($teacher['is_executive'])): ?><a href="executive-panel.php" class="tp-tile"><span class="ic">🏢</span>پنل معاون اجرایی</a><?php endif; ?>
        </div>
    </div>
