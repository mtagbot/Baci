...
def fix_rtl(text):
    ...
    if not confident:
        # واژه‌های فارسی سالم (پیوستگی حروف) نشانه ترتیب منطقی درست است:
        # کلمات ≥۳ حرفی که با «ا آ د ذ ر ز ژ و» شروع نمی‌شوند و در هر دو جهت هستند
        words = [w for w in re.split(r'[^\u0600-\u06FF\u200c]+', base) if len(w) >= 3]
        if len(words) >= 2:
            confident = True   # متن چندکلمه‌ای فارسی؛ bidi برعکسش نکرده چون امتیازی نداشت
    ...
    for row in segs_all:
        ...
        if not FA_LETTER.search(txt) and MATHY_SEG.search(txt) and not has_frac:
            ...
            if other and len(fa_core) <= 4 and re.fullmatch(r'[الفبجدهویـ‌]+', fa_core or 'x') :
                # قطعات بارم (Bold ارقام/کسر مثل «5»، « /0») در محاسبه ناحیه ریاضی نیایند
                mspans = [s0 for s0 in other
                          if not ('Bold' in s0.get('f', '') and re.fullmatch(r'[\s0-9./]+', s0['t']))]
                if mspans:
    ...
    for pno, page in enumerate(doc):
        ...
        for l in lines:
            ...
            if mm and l['rect'].x1 >= pw * 0.9 and re.search(r'[\u0600-\u06FF]', l['text']):
                virt = {'text': mm.group(1), 'rect': fitz.Rect(pw * 0.92, l['rect'].y0, l['rect'].x1, l['rect'].y1),
