...
/* v4.83.0: the discipline page (deputy-panel.php) is available to deputy
   teachers too — the hover tooltip / info / discipline modals must work for
   them exactly like for admins, so accept both roles here. */
$smIsAdmin  = is_admin_logged_in() && has_permission('manage_students');
$smIsDeputy = function_exists('is_teacher_logged_in') && is_teacher_logged_in()
              && function_exists('teacher_has_deputy') && teacher_has_deputy($_SESSION['teacher_id'] ?? 0);
if (!$smIsAdmin && !$smIsDeputy) {
    header('Content-Type: application/json; charset=utf-8');
    echo json_encode(['ok' => false, 'html' => 'دسترسی غیرمجاز'], JSON_UNESCAPED_UNICODE);
    exit;
}
header('Content-Type: application/json; charset=utf-8');
