    ...
    <div class="grid grid-cols-3 gap-6 responsive-grid">
        ...
        <section class="card col-span-2 shadow-lg space-y-4">
            ...
            <form method="POST" class="space-y-4" id="targetForm" enctype="multipart/form-data">
                ...
                <textarea name="target_message" rows="4" class="form-textarea" placeholder="متن اطلاعیه را بنویسید... (در صورت پیوست فایل، این متن به عنوان توضیح فایل ارسال می‌شود)"></textarea>
                <div class="soft-panel text-xs space-y-2">
                    <label class="block font-semibold">پیوست فایل (اختیاری)</label>
                    <input type="file" name="target_file" id="targetFile" class="form-input text-xs" accept=".jpg,.jpeg,.png,.webp,.gif,.pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.zip,.rar,.txt,.csv,.mp3,.ogg,.wav,.mp4">
                    <p class="text-muted">تصویر به صورت عکس و بقیه فرمت‌ها به صورت فایل (سند) ارسال می‌شوند. حداکثر حجم: ۴۹ مگابایت. فرمت‌های مجاز: تصویر، PDF، Word، Excel، PowerPoint، ZIP/RAR، متن، صوت و ویدیو MP4.</p>
                    <p class="text-muted" id="targetFileInfo" style="display:none"></p>
                </div>
                <button type="submit" class="btn btn-primary px-8">📨 ارسال به مخاطبان انتخاب‌شده</button>
            </form>
        </section>
    </div>
...
(function(){
  ...
  const refresh = () => {
    ...
    form.querySelectorAll('.target-value').forEach(el => { el.style.display = (el.dataset.scope === scope) ? '' : 'none'; });
  };
  form.querySelectorAll('input[name="target_scope"]').forEach(r => r.addEventListener('change', refresh));
  refresh();

  // v4.47.0: file attachment feedback + "message OR file" validation
  ...
  if (fileInput && fileInfo) {
    fileInput.addEventListener('change', () => {
      ...
      if (f.size > MAX_BYTES) {
        fileInfo.textContent = 'حجم فایل (' + mb + ' مگابایت) بیشتر از سقف مجاز ۴۹ مگابایت است.';
        fileInfo.style.color = '#dc2626';
        fileInput.value = '';
      } else {
        fileInfo.textContent = 'فایل انتخاب‌شده: ' + f.name + ' (' + mb + ' مگابایت)';
  ...
  form.addEventListener('submit', (e) => {
    const msg = (form.querySelector('textarea[name="target_message"]')?.value || '').trim();
