...
if (!function_exists('bot_send_targeted_file')) {
    ...
    function bot_send_targeted_file($platform, $scope, $value, $caption, $filePath, $mime = '', $origName = '') {
        $recipients = bot_target_recipients($platform, $scope, $value);
        $isImage = in_array(strtolower($mime), ['image/jpeg', 'image/png', 'image/webp'], true);
        $sent = 0; $failed = 0;
        foreach ($recipients as $r) {
            $text = $caption !== ''
                ? bot_text($platform, 'broadcast_prefix', ['message' => $caption, 'student_name' => trim($r['first_name'] . ' ' . $r['last_name'])])
                : '';
            $logMsg = '[فایل: ' . ($origName !== '' ? $origName : basename($filePath)) . '] ' . $caption;
            try {
                if ($isImage) {
                    bot_send_photo($platform, $r['chat_id'], $filePath, $text, null, $mime, $origName);
                } else {
