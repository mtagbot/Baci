...
$students = DB::fetchAll('SELECT * FROM students WHERE '.implode(' AND ',$where), $params);
usort($students, function($a,$b){ $c = strcmp((string)($b['academic_year']??''), (string)($a['academic_year']??'')); if ($c!==0) return $c; return persian_compare($a['class_name']??'', $b['class_name']??'') ?: persian_compare($a['last_name']??'', $b['last_name']??'') ?: persian_compare($a['first_name']??'', $b['first_name']??''); });   // v4.77.0: آ قبل از ا
