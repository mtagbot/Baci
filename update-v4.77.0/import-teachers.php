...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'execute_teachers') {
    ...
    foreach ($allParsed as $t) {
        ...
        if ($exists) {
            DB::execute("UPDATE teachers SET personnel_code=?, full_name=?, mobile=?, password=? WHERE national_id=?",
                [$code, $name, $mob, $pass, $nid]);
            $updatedCount++;
        } else {
            DB::execute("INSERT INTO teachers (national_id, personnel_code, full_name, mobile, password, status) VALUES (?, ?, ?, ?, ?, 1)",
                [$nid, $code, $name, $mob, $pass]);
            $insertedCount++;
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_teacher_manual'])) {
    if (verify_csrf($_POST['csrf_token'] ?? '')) {
        ...
        if ($tId > 0) {
            $q = "UPDATE teachers SET national_id=?, personnel_code=?, full_name=?, mobile=?, status=?, is_deputy=?, is_counselor=?, is_executive=? WHERE id=?";
            $p = [$nid, $code, $name, $mob, $st, $isDeputy, $isCounselor, $isExecutive, $tId];
            if (!empty($_POST['password'])) {
                $q = "UPDATE teachers SET national_id=?, personnel_code=?, full_name=?, mobile=?, status=?, is_deputy=?, is_counselor=?, is_executive=?, password=? WHERE id=?";
                $p = [$nid, $code, $name, $mob, $st, $isDeputy, $isCounselor, $isExecutive, password_hash($_POST['password'], PASSWORD_DEFAULT), $tId];
            }
            DB::execute($q, $p);
            ...
            if ($dupNid) {
                ...
                DB::execute("INSERT INTO teachers (national_id, personnel_code, full_name, mobile, password, status, is_deputy, is_counselor, is_executive) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    [$nid, $code, $name, $mob, $pass, $st, $isDeputy, $isCounselor, $isExecutive]);
