...
if ($action === 'finalize') {
    try {
        @set_time_limit(600);
        // v4.77.0: the cron endpoint is a SCHEDULED run — it must honor the
        // pause switch (att_auto_finalize), the off-days guard and the
        // once-per-day guard. Manual force stays available in the admin
        // panel («اجرای ثبت غیبت خودکار الان»).
        $res = att_finalize_absents(false);
