...
if (is_student_logged_in()) {
    ...
    if ($gradeAll && $targetGrade !== '') {
        ...
        $students = DB::fetchAll("SELECT s.* FROM students s WHERE s.status='active' AND (s.grade_level=? OR s.class_name LIKE ?)", [$targetGrade, $targetGrade . '%']);
    } else {
        $students = DB::fetchAll("SELECT s.* FROM students s WHERE s.status='active' AND (?='' OR s.class_name=?) AND (?='' OR s.grade_level=?)",[$exam['class_name'],$exam['class_name'],$exam['grade_level'],$exam['grade_level']]);
    }
...
function prepareAllAndPrint(){
  ...
  prepareAllStudents();
  waitForPrintReady(()=>{btns.forEach(b=>{b.disabled=false;if(b.dataset.oldTxt)b.textContent=b.dataset.oldTxt;}); window.print();});
}
