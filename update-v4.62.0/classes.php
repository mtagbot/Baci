    ...
    <div class="card shadow-lg space-y-4">
        ...
        <form method="POST" class="grid grid-cols-6 gap-2 items-end bg-emerald-50 dark:bg-slate-800 p-3 rounded border" style="border-color:#6ee7b7">
            ...
            <div>
                ...
                <!-- v4.62.0: دروس رسمی متوسطه اول به صورت انتخابی — تعریف درس فقط همین‌جا -->
                <select name="subject_name" class="form-select text-xs font-bold" required>
                    <option value="">---</option>
                    <optgroup label="دروس رسمی متوسطه اول (هفتم تا نهم)">
                        <?php foreach ($standardSubjects as $sn): ?><option value="<?php echo clean($sn); ?>"><?php echo clean($sn); ?></option><?php endforeach; ?>
                    </optgroup>
                    <?php $extraSubjects = array_values(array_diff($subjectNameOptions, $standardSubjects)); if ($extraSubjects): ?>
                    <optgroup label="سایر دروس موجود در سیستم">
                        <?php foreach ($extraSubjects as $sn): ?><option value="<?php echo clean($sn); ?>"><?php echo clean($sn); ?></option><?php endforeach; ?>
                    </optgroup>
                    <?php endif; ?>
                </select>
            </div>
                    ...
                    <tr id="schedEdit<?php echo $rid; ?>" style="display:none;background:rgba(79,70,229,0.05)">
                        ...
                        <td>
                            <select name="subject_name" form="schedForm<?php echo $rid; ?>" class="form-select text-xs font-bold" required>
                                <?php $found=false; foreach ($subjectPickerOptions as $sn): $sel = trim($sc['subject_name'])===$sn; if($sel)$found=true; ?>
