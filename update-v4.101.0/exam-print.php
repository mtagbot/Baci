...
function renderQuestions(){collectDrawings();renumberQuestions();document.getElementById('pagesRoot').innerHTML=''; let page=1; ensurePages(Math.max(page,sourcePageCount())); applySourceCropFits(); qItems.forEach(it=>{ if(it.type==='page'){page++; ensurePages(page); return;} appendItemToPage(it,page); /* v4.101.0: روی صفحه دارای فایل منبع، حتی اولین سوالِ سرریز هم به صفحه بعد می‌رود — سوال زیر تصویر منبع پنهان نمی‌ماند */ if(isPageOverflow(page) && (questionCountOnPage(page)>1 || pageHasSource(page))){removeItemFromPage(it.id,page); page++; ensurePages(page); appendItemToPage(it,page);} }); enableDrag(); resizeCanvases(); restoreDrawings(); autosaveLocal(); }
