...
.eq-slot:focus{outline:2px solid #2563eb}
...
function impDirection(btn,dir){
  ...
  if(sel.rangeCount && !sel.isCollapsed && el.contains(sel.anchorNode)){
    /* فقط متن انتخاب‌شده: بسته‌بندی در span ایزوله جهت‌دار */
    const range=sel.getRangeAt(0);
    const span=document.createElement('span');
    span.setAttribute('dir',dir);
    span.style.unicodeBidi='isolate';
    span.style.display='inline-block';
    span.style.textAlign=dir==='ltr'?'left':'right';
    try{
      range.surroundContents(span);
    }catch(e){
      /* انتخاب چندگره‌ای: extract + wrap */
...
const EQ_BOX='<span class="eq-slot" style="display:inline-block;min-width:22px;border:1px dashed #94a3b8;border-radius:4px;padding:0 4px;text-align:center">&#8203;</span>';
