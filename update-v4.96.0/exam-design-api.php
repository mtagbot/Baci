...
if ($action === 'load') {
    ...
    foreach (DB::fetchAll("SELECT academic_year AS v FROM exam_question_bank WHERE COALESCE(academic_year,'')<>'' UNION SELECT es.academic_year FROM exam_designs ed JOIN exam_schedules es ON es.id=ed.exam_id WHERE COALESCE(es.academic_year,'')<>''") as $r0) $fYears[] = (string)$r0['v'];
    foreach (DB::fetchAll("SELECT exam_month AS v FROM exam_question_bank WHERE COALESCE(exam_month,'')<>'' UNION SELECT es.exam_month FROM exam_designs ed JOIN exam_schedules es ON es.id=ed.exam_id WHERE COALESCE(es.exam_month,'')<>''") as $r0) $fMonths[] = (string)$r0['v'];
    foreach (DB::fetchAll("SELECT designer_name AS v FROM exam_question_bank WHERE COALESCE(designer_name,'')<>'' UNION SELECT ed.designer_name FROM exam_designs ed WHERE COALESCE(ed.designer_name,'')<>''") as $r0) $fDesigners[] = (string)$r0['v'];
    rsort($fYears);
