...
// v4.87.0 — the every-minute cron hit is the only guaranteed heartbeat on
// hosts without shell cron entries per feature: run the attendance finalizer
// and the deputies' scheduled absent-list push here too (both are internally
// guarded to fire at most once per day and only after their times pass).
try {
    require_once __DIR__ . '/includes/attendance_helpers.php';
    require_once __DIR__ . '/includes/bot_role_engine.php';
    att_finalize_absents(false);
    att_run_deferred();
    bot_deputy_absents_push();
} catch (Throwable $e) { error_log('poll heartbeat: ' . $e->getMessage()); }
