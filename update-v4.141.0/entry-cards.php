...
if (isset($_GET['print'])) {
    $D = [
        ...
        'layout' => in_array(($_GET['layout'] ?? 'full'), ['full', 'qrmax', 'sq'], true) ? $_GET['layout'] : 'full',
        /* v4.141.0 — کاغذ، جهت و مقیاس کارت */
        'paper'  => array_key_exists(($_GET['paper'] ?? 'A4'), card_paper_sizes()) ? $_GET['paper'] : 'A4',
        'orient' => in_array(($_GET['orient'] ?? 'portrait'), ['portrait', 'landscape'], true) ? $_GET['orient'] : 'portrait',
        /* مقیاس ۶۰٪ تا ۱۶۰٪ — پایین‌تر از ۶۰٪ ماژول‌های QR برای
           دوربین ریز می‌شوند، بالاتر از ۱۶۰٪ روی A4 حتی یک کارت هم
           جا نمی‌شود. */
        'scale'  => min(1.6, max(0.6, (float)($_GET['scale'] ?? 1))),
    ];
...
<div class="space-y-6">
    ...
    <div class="card no-print" id="cardDesigner">
        ...
        <div class="grid grid-cols-4 gap-4">
            ...
            <div>
                ...
                <input type="range" id="cScale" min="60" max="160" step="5" value="100" style="width:100%" oninput="applyCardDesign()">
            </div>
            <div>
    ...
    <div class="card no-print">
        ...
        <?php card_ornament_defs($themeColor, $accentColor); ?>
        <?php /* v4.141.0 — پیش‌نمایش «یک صفحهٔ واقعی».
              فقط صفحهٔ اول رندر می‌شود (خواستهٔ کارفرما برای بهینه ماندن):
              حداکثر ۷۰ کارت که سقف A2 در حالت مربع است. چاپ نهایی همهٔ
              دانش‌آموزان را در همهٔ صفحات می‌آورد. */ ?>
        <?php
        $D = ['photo' => true, 'nid' => true, 'year' => true, 'theme' => 'classic',
              'cut' => true, 'layout' => 'full'];
        $previewList = array_slice($students, 0, 70);
        ?>
        <div class="pv-wrap">
...
function applyCardDesign(){
  ...
  var cards = pv.querySelectorAll('.card-id');
  var shown = 0;
  ...
  var pagesNeeded = perPage > 0
      ? Math.ceil(TOTAL_STUDENTS * (d.side === 'both' ? 2 : 1) / perPage) : 0;
  ...
  if (perPage === 0) {
    ...
    var t = 'این فقط صفحهٔ اول است. در چاپ، هر ' + faNum(TOTAL_STUDENTS) +
            ' دانش‌آموز' + (d.side === 'both' ? ' (روی و پشت)' : '') +
            ' در ' + faNum(pagesNeeded) + ' صفحه چاپ می‌شوند.';
    if (shown < perPage) {
      t += ' (در این پیش‌نمایش ' + faNum(shown) + ' کارت موجود است.)';
    }
    note.textContent = t;
  }
...
function resetCardDesign(){
  setCardDesign({paper:'A4',orient:'portrait',scale:1,layout:'full',theme:'classic',gap:4,margin:8,side:'front',photo:true,nid:true,year:true,cut:true});
  applyCardDesign();
}
