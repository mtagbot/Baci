...
if ($type === 'seatcards') {
    ...
    $rows = DB::fetchAll("SELECT seat.*, s.first_name, s.last_name, s.photo_url, s.class_name, s.grade_level
        ...
        WHERE seat.academic_year=? AND seat.exam_month=?
        ORDER BY seat.exam_room, CAST(seat.seat_number AS UNSIGNED), s.class_name, s.last_name", [$year,$month]);
...
if (is_student_logged_in()) {
    ...
    if ($gradeAll && $targetGrade !== '') {
        ...
        $students = DB::fetchAll("SELECT s.* FROM students s WHERE s.status='active' AND (s.grade_level=? OR s.class_name LIKE ?) ORDER BY s.class_name,s.last_name,s.first_name", [$targetGrade, $targetGrade . '%']);
    } else {
        $students = DB::fetchAll("SELECT s.* FROM students s WHERE s.status='active' AND (?='' OR s.class_name=?) AND (?='' OR s.grade_level=?) ORDER BY s.class_name,s.last_name",[$exam['class_name'],$exam['class_name'],$exam['grade_level'],$exam['grade_level']]);
    }
}
...
<?php foreach($students as $s): $seat=DB::fetch('SELECT * FROM exam_student_seating WHERE academic_year=? AND exam_month=? AND student_id=?',[$exam['academic_year'],$exam['exam_month']?:'خرداد',$s['id']]); ?><tr><td><?php echo clean($s['first_name'].' '.$s['last_name']); ?></td><td><?php echo clean($s['class_name']); ?></td><td><?php echo clean($exam['subject_name']); ?></td><td><?php echo tr_num($exam['exam_date_jalali'],'fa'); ?></td><td><?php echo clean($exam['exam_day_name']); ?></td><td><?php echo tr_num($exam['start_time'],'fa'); ?></td><td><?php echo tr_num($exam['duration_minutes'],'fa'); ?></td><td><?php echo clean(($seat['exam_room']??'') ?: $exam['exam_room_default']); ?></td><td><?php echo tr_num($seat['seat_number']??'---','fa'); ?></td></tr><?php endforeach; ?>
...
if ($type === 'questions_editor') {
    ...
    foreach ($students as $s0) {
        $seat0 = DB::fetch('SELECT * FROM exam_student_seating WHERE academic_year=? AND exam_month=? AND student_id=?', [$exam['academic_year'], $exam['exam_month'] ?: 'خرداد', $s0['id']]);
        $studentsData[] = [
            'id' => (int)$s0['id'],
...
let qItems=[], bankItems=[], drawingsData={}, sourceCrops={}, sourceOrder=[], pageStyle={}, printNote='', selectedImageId=null, tool='off', drawing=false, last=null, booting=true, printSavePromptPending=false;
...
function drawStrokeOnPage(page,a,b,t){document.querySelectorAll(`.page[data-page-index="${page}"] .drawCanvas`).forEach(c=>{const ctx=c.getContext('2d');ctx.lineCap='round';ctx.lineJoin='round';if(t==='eraser'){ctx.globalCompositeOperation='destination-out';ctx.lineWidth=18;}else{ctx.globalCompositeOperation='source-over';ctx.strokeStyle=(document.getElementById('penColor')?.value||'#111111');ctx.lineWidth=2;}ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke();}); collectDrawings(); autosaveLocal();}
function clearDrawings(){document.querySelectorAll('.drawCanvas').forEach(c=>c.getContext('2d').clearRect(0,0,c.width,c.height)); drawingsData={}; autosaveLocal();}
...
function prepareAllStudents(){
  // Performance-critical print preparation:
  // 1) Keep live editor limited to the first student.
  // 2) Before print, clone the *already designed* pages of the first student to all other students.
  // 3) Source PDF pages are converted once on the server and reused by URL; no per-student PDF rendering happens.
  collectDrawings();
  ...
  const designedWraps=firstPages.map(p=>p.querySelector('.qwrap') ? p.querySelector('.qwrap').outerHTML : '');
  root.innerHTML='';
  previewOnly=false;
  studentsData.forEach(st=>{
    designedWraps.forEach((wrap,idx)=>{
      const pageNo=idx+1;
      root.insertAdjacentHTML('beforeend', `<div class="page ${pageNo%2===0?'no-header':''}" data-student-id="${st.id}" data-page-index="${pageNo}">${headerHtml(st,pageNo)}${wrap}</div>`);
    });
  });
  resizeCanvases();
}
function prepareAllAndPrint(){printSavePromptPending=true; prepareAllStudents(); setTimeout(()=>{applyDrawingOverlays(); window.print();},550);}
window.addEventListener('beforeprint',()=>{if(previewOnly)prepareAllStudents(); applyDrawingOverlays();});
...
function collectDrawings(){const first=studentsData[0]; if(!first)return; document.querySelectorAll(`.page[data-student-id="${first.id}"] .drawCanvas`).forEach(c=>{const page=c.closest('.page').dataset.pageIndex; if(!isCanvasBlank(c) || !drawingsData[page]) drawingsData[page]=c.toDataURL('image/png');});}
