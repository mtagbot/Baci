...
<div class="max-w-3xl mx-auto space-y-4">
    <div class="card">
        <h2 class="text-lg font-bold">📍 تست موقعیت مکانی - عیب‌یابی GPS (سازگار با ایران)</h2>
    ...
    <div class="card space-y-3">
        <div id="infoBox" class="p-3 bg-blue-50 border rounded text-xs space-y-1">
            ...
            <div>🌐 UserAgent: <span id="uaStatus"></span></div>
            <div>📱 Platform: <span id="platformStatus"></span></div>
            <div>📍 Geolocation موجود: <span id="geoExist"></span></div>
            <div>🔐 Permission API موجود: <span id="permExist"></span></div>
        </div>

        <div class="grid grid-cols-2 gap-2">
            <button onclick="testLow()" class="btn btn-primary text-xs">🟢 تست حالت کم‌دقت (شبکه - بدون ماهواره - پیشنهادی ایران)</button>
            <button onclick="testBalanced()" class="btn btn-secondary text-xs">🟡 تست متعادل (شبکه + GPS)</button>
            <button onclick="testHigh()" class="btn btn-warning text-xs">🔴 تست دقیق GPS ماهواره‌ای</button>
            <button onclick="testWatch()" class="btn btn-success text-xs">👁️ تست WatchPosition (مداوم)</button>
        </div>
