        ...
        if ($subj && $msg && is_student_logged_in()) {
            ...
            if ($tId) {
                foreach (['bale','telegram'] as $pf) {
                    try {
                        $sessions = DB::fetchAll("SELECT chat_id FROM bot_admin_sessions WHERE platform=? AND teacher_id=? AND role_type='teacher' AND is_active=1", [$pf, $tId]);
                        foreach ($sessions as $bs) bot_send_message($pf, $bs['chat_id'], "💬 اعتراض/پیام نمره جدید\nدانش‌آموز: {$report['first_name']} {$report['last_name']}\nدرس: {$subj}\nزمان: " . jalali_now());
                    } catch (Exception $e) {}
                }
            }
    ...
    <div class="card p-8 border-2 border-primary shadow-2xl bg-white dark:bg-slate-900 text-gray-900 dark:text-white rounded-2xl">
        ...
        <div class="flex justify-between items-center border-b-2 border-gray-200 dark:border-slate-700 pb-6 mb-6">
            ...
            <div class="flex flex-col items-center gap-1.5 w-24">
                <div class="w-20 h-24 rounded-lg border-2 border-primary overflow-hidden bg-gray-100 flex items-center justify-center shadow">
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        backgroundColor: function(context) {
                            ...
                            return val >= 17 ? '#10b981' : (val >= 14 ? '#3b82f6' : (val >= 10 ? '#f59e0b' : '#ef4444'));
                        },
                        borderRadius: 6
                    }]
                },
