<?php
/**
 * Student Profiles Bulk Importer - v4.29.0 Comprehensive (7reporte.csv with 31 columns)
...
function parseStudentProfileCsv($filepath) {
    ...
    foreach ($rows as $r) {
        ...
        $bdate  = ($birthYear && $birthYear != '0') ? "$birthYear/$birthMonth/$birthDay" : trim($r['birth_date'] ?? '');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'execute_students') {
    ...
    foreach ($allParsed as $st) {
        ...
        $passHash = password_hash($ser ?: $nid, PASSWORD_DEFAULT);
...
<div class="space-y-6">
    <div class="flex justify-between items-center">
        ...
        <div class="flex gap-2">
            ...
            <a href="/home/user/uploads/7reporte.csv" download class="btn btn-outline gap-1 text-sm border-blue-500 text-blue-600"><span>📥 دانلود نمونه فرمت جدید (7reporte.csv)</span></a>
        </div>
    </div>
