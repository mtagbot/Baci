...
if (!function_exists('bot_migrate_to_latest_year')) {
    function bot_migrate_to_latest_year($platform = 'bale') {
        try {
            $platform = bot_valid_platform($platform);
            $userTable = bot_user_table($platform);
            // For each distinct national_id in students that has bot link, update to latest student id
            $rows = DB::fetchAll("SELECT b.id as link_id, s.national_id FROM `$userTable` b JOIN students s ON s.id=b.student_id WHERE s.national_id<>''");
            foreach ($rows as $r) {
                $latest = get_latest_student_by_national_id($r['national_id']);
                if ($latest && $latest['id'] != $r['link_id']) {
                    // Update link to latest student id if different academic year but same national_id
                    // We need to get current student_id of this link
                    $currentLink = DB::fetch("SELECT student_id FROM `$userTable` WHERE id=?", [$r['link_id']]);
                    if ($currentLink && (int)$currentLink['student_id'] !== (int)$latest['id']) {
                        // Check if latest already linked to same chat_id? If so, delete old duplicate
                        $chatCol = $platform === 'telegram' ? 'telegram_chat_id' : 'bale_chat_id';
                        $chatRow = DB::fetch("SELECT `$chatCol` as chat_id FROM `$userTable` WHERE id=?", [$r['link_id']]);
                        if ($chatRow) {
                            $exists = DB::fetch("SELECT id FROM `$userTable` WHERE `$chatCol`=? AND student_id=?", [$chatRow['chat_id'], $latest['id']]);
                            if ($exists) {
                                // Delete old link, keep latest
