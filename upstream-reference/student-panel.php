...
<div class="space-y-6">
    ...
    <div class="card bg-gradient-to-l from-blue-600 to-indigo-700 text-white p-6 rounded-2xl shadow-md flex justify-between items-center">
        ...
        <div class="text-left hidden md:block space-y-2">
            ...
            <form method="POST" onsubmit="return confirm('این دانش‌آموز از حساب/نشست فعلی حذف شود؟')">
                ...
                <button class="btn btn-danger text-xs">🗑️ حذف دانش‌آموز از حساب</button>
            </form>
        </div>
    </div>
    ...
    <div class="card border-indigo-200 bg-indigo-50 space-y-3">
        <div class="flex justify-between items-center">
            <h3 class="font-bold text-indigo-900 flex items-center gap-1">🧪 آزمون‌های آنلاین شما: <span class="text-xs">(سال جاری: <?php echo clean($currentDefaultYear); ?> - سال شما: <?php echo clean($student['academic_year'] ?? 'نامشخص'); ?>)</span></h3>
        ...
        <div class="p-2 bg-amber-100 border border-amber-200 rounded text-xs text-amber-800">⚠️ شما دانش‌آموز سال تحصیلی جاری (<?php echo clean($currentDefaultYear); ?>) نمی‌باشید. سال شما: <?php echo clean($student['academic_year']); ?></div>
        <?php endif; ?>
        <?php if ($liveAttempts): ?>
        <div class="p-2 bg-amber-100 border border-amber-200 rounded text-xs text-amber-800">⚠️ شما <?php echo count($liveAttempts); ?> آزمون ناتمام در حال برگزاری دارید. با کلیک روی ادامه آزمون می‌توانید بازگردید (زمان شما حتی در خارج صفحه محاسبه می‌شود)</div>
        <?php endif; ?>
            ...
            <div class="p-3 bg-white rounded border flex justify-between items-center">
                ...
                <div>
                    ...
                    <?php else: 
                        ...
                        if ($canTake): ?><a href="online-exam-take.php?exam_id=<?php echo $oe['id']; ?>" class="btn btn-success text-xs">🚀 شروع آزمون</a>
    ...
    <div class="card border-blue-200 bg-blue-50 space-y-3">
        <h3 class="font-bold text-blue-900">📝 برنامه امتحانی</h3>
        <?php if ($examItems): ?>
    ...
    <div class="card border-amber-200 bg-amber-50 space-y-3">
        <h3 class="font-bold text-amber-900">⚠️ موارد انضباطی ثبت‌شده</h3>
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if ($rep['is_locked']): ?>
                                <span class="badge badge-danger">مشاهده مسدود است 🔒</span>
                            <?php else: ?>
                                <span class="badge badge-success">آزاد و قابل دریافت 🔓</span>
                            <?php endif; ?>
                        </td>
                        <td>
                            <?php if ($rep['is_locked']): ?>
                            ...
                            <?php else: ?>
                                <div class="flex gap-2 justify-end">
                                    ...
                                    <a href="report-print.php?id=<?php echo $rep['id']; ?>&pt=<?php echo urlencode(make_report_print_token($rep['id'], $studentId)); ?>" target="_self" class="btn btn-secondary text-xs px-3 py-1">🖨️ چاپ کارنامه</a>
                                </div>
                            <?php endif; ?>
                        </td>
                    </tr>
