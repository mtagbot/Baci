...
usort($students, fn($a,$b)=>strcmp(norm_persian_str($a['last_name']), norm_persian_str($b['last_name'])) ?: strcmp(norm_persian_str($a['first_name']), norm_persian_str($b['first_name'])));
...
?>
<!DOCTYPE html><html lang="fa" dir="rtl"><head><meta charset="UTF-8"><title><?php echo clean($title); ?></title><style>body{font-family:Tahoma,sans-serif;direction:rtl} .header{text-align:center;border:2px solid #000;padding:12px;margin-bottom:16px}.meta{display:flex;justify-content:space-between;font-size:12px}table{width:100%;border-collapse:collapse}th,td{border:1px solid #333;padding:6px;font-size:12px}th{background:#eee}.sign{margin-top:40px;display:flex;justify-content:space-around}</style></head><body>
