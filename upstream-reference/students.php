        ...
        try {
            ...
            if ($srcYear === $destYear) {
                ...
                if ($exists) {
                    ...
                    DB::execute("INSERT INTO students (national_id, student_code, serial_number, first_name, last_name, father_name, birth_date, gender, grade_level, class_name, photo_url, password, phone, father_phone, mother_phone, status, is_temp, academic_year) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", [$st['national_id'],$st['student_code'],$st['serial_number'],$st['first_name'],$st['last_name'],$st['father_name'],$st['birth_date'],$st['gender'],$destGrade,$destClass,$st['photo_url'],$st['password'],$st['phone'],$st['father_phone'],$st['mother_phone'],'active',0,$destYear]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_student'])) {
    ...
    $cname  = trim($_POST['class_name'] ?? '101');
    ...
    if ($bYear && $bMonth && $bDay) { $bdate = "$bYear/$bMonth/$bDay"; }
    ...
    if ($studentId > 0) {
        DB::execute("UPDATE students SET national_id=?, student_code=?, serial_number=?, first_name=?, last_name=?, father_name=?, birth_date=?, birth_year=?, birth_month=?, birth_day=?, grade_level=?, class_name=?, phone=?, father_phone=?, mother_phone=?, student_mobile=?, postal_code=?, home_address=?, home_phone=?, birth_place=?, place_issued=?, religion_title=?, nationality_title=?, is_foreign=?, housing_title=?, cover_title=?, specific_disease_title=?, has_sport_limitation=?, mother_qualification=?, mother_job=?, father_qualification=?, father_job=?, mother_name=?, mother_first_name=?, mother_last_name=?, mother_deceased=?, father_deceased=?, last_year_average=?, status=?, is_temp=0 WHERE id=?",
        [$nid, $code, $ser, $fname, $lname, $father, $bdate, $bYear, $bMonth, $bDay, $grade, $cname, $phone, $fMob, $mMob, $sMobile, $postal, $homeAddr, $homePh, $birthPlace, $placeIssued, $religion, $nationality, $isForeign, $housing, $cover, $disease, $hasSport, $motherQual, $motherJob, $fatherQual, $fatherJob, $motherName, $motherFirst, $motherLast, $motherDeceased, $fatherDeceased, $lastAvg, $status, $studentId]);
        ...
        DB::execute("INSERT INTO students (national_id, student_code, serial_number, first_name, last_name, father_name, birth_date, birth_year, birth_month, birth_day, grade_level, class_name, password, phone, father_phone, mother_phone, student_mobile, postal_code, home_address, home_phone, birth_place, place_issued, religion_title, nationality_title, is_foreign, housing_title, cover_title, specific_disease_title, has_sport_limitation, mother_qualification, mother_job, father_qualification, father_job, mother_name, mother_first_name, mother_last_name, mother_deceased, father_deceased, last_year_average, status, is_temp) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        [$nid, $code, $ser, $fname, $lname, $father, $bdate, $bYear, $bMonth, $bDay, $grade, $cname, $pass, $phone, $fMob, $mMob, $sMobile, $postal, $homeAddr, $homePh, $birthPlace, $placeIssued, $religion, $nationality, $isForeign, $housing, $cover, $disease, $hasSport, $motherQual, $motherJob, $fatherQual, $fatherJob, $motherName, $motherFirst, $motherLast, $motherDeceased, $fatherDeceased, $lastAvg, $status, 0]);
    ...
    <form method="POST" action="students.php?id=<?php echo $studentId; ?>">
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
            ...
            <div>
                ...
                <input type="text" name="birth_date" class="form-input dir-ltr text-left" placeholder="1386/05/12" value="<?php echo clean($editData['birth_date'] ?? ''); ?>">
            </div>
        </div>

        <div class="grid grid-cols-2 gap-4 mb-4">
            <div>
                <label class="block text-xs font-semibold mb-1">کلاس تحصیلی</label>
                <input type="text" name="class_name" class="form-input" value="<?php echo clean($editData['class_name'] ?? '101 ریاضی'); ?>" required>
            </div>
            <div>
                <label class="block text-xs font-semibold mb-1">پایه تحصیلی</label>
                <input type="text" name="grade_level" class="form-input" value="<?php echo clean($editData['grade_level'] ?? 'دهم ریاضی'); ?>">
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <input type="text" name="postal_code" class="form-input dir-ltr" value="<?php echo clean($editData['postal_code'] ?? ''); ?>" placeholder="مثلا 33544...">
            </div>
            <div>
        ...
        <div class="grid grid-cols-3 gap-4 mb-4">
            <div><label class="block text-xs font-semibold mb-1">روز تولد (دو رقمی)</label><input type="number" min="1" max="31" name="birth_day" class="form-input" value="<?php echo clean($editData['birth_day'] ?? ''); ?>"></div>
            <div><label class="block text-xs font-semibold mb-1">ماه تولد (دو رقمی)</label><input type="number" min="1" max="12" name="birth_month" class="form-input" value="<?php echo clean($editData['birth_month'] ?? ''); ?>"></div>
            <div><label class="block text-xs font-semibold mb-1">سال تولد (4 رقمی)</label><input type="number" min="1300" max="1500" name="birth_year" class="form-input" value="<?php echo clean($editData['birth_year'] ?? ''); ?>"></div>
        </div>
        ...
        <div class="flex justify-between items-center">
            ...
            <button type="submit" class="btn btn-success px-8 py-2.5 font-bold">💾 ذخیره اطلاعات دانش‌آموز</button>
        </div>
    </form>
</div>
<?php
    ...
    <form method="POST" action="student-bulk-report.php" id="bulkStudentsForm" class="card">
        ...
        <div class="flex justify-between items-center mb-4 flex-wrap gap-2">
            ...
            <div class="flex gap-2 flex-wrap">
                <button type="button" class="btn btn-outline text-xs" onclick="document.querySelectorAll('.st-check').forEach(c=>c.checked=true)">انتخاب همه</button>
                <button type="button" class="btn btn-outline text-xs" onclick="document.querySelectorAll('.st-check').forEach(c=>c.checked=false)">لغو انتخاب</button>
                <button type="button" class="btn btn-primary text-xs" onclick="openReportModal('info')">گزارش اطلاعات</button>
                <button type="button" class="btn btn-warning text-xs" onclick="openReportModal('discipline')">گزارش انضباطی</button>
                <button type="button" class="btn btn-success text-xs" onclick="openReportModal('grades')">گزارش تحلیلی نمرات</button>
                <button type="button" class="btn btn-warning text-xs" onclick="openTransferModal()">انتقال دانش‌آموزان</button>
                <button type="submit" formaction="students.php" formmethod="POST" name="bulk_delete_students" value="1" class="btn btn-danger text-xs" onclick="return confirm('⚠️ آیا از حذف دسته‌جمعی دانش‌آموزان انتخاب شده اطمینان دارید؟\n\nاین عملیات حذف می‌کند:\n- پرونده دانش‌آموز\n- کارنامه‌ها\n- موارد انضباطی\n- اتصالات ربات\n- آزمون‌ها\n\nقابل بازگشت نیست!');" style="background:#dc2626;color:white;">🗑️ حذف دسته‌جمعی انتخاب شده‌ها</button>
            </div>
        </div>
        <div class="table-container">
            <table id="studentsTable">
                <thead><tr><th><input type="checkbox" onclick="document.querySelectorAll('.st-check').forEach(c=>c.checked=this.checked)"></th><th>کد ملی</th><th>نام و نام خانوادگی</th><th>کلاس</th><th>پایه</th><th>آخرین معدل</th><th>انضباط</th><th>بررسی نشده</th><th>بررسی کارنامه</th><th>وضعیت</th><th>عملیات</th></tr></thead>
                <tbody>
                <?php foreach ($studentsList as $st): ?>
                    <tr class="student-row" data-student-id="<?php echo (int)$st['id']; ?>">
                        ...
                        <td class="font-bold"><?php echo clean($st['last_name'] . '، ' . $st['first_name']); ?></td>
                        ...
                        <td><div class="flex gap-1 justify-end flex-wrap"><button type="button" onclick="openStudentModal('info', <?php echo $st['id']; ?>)" class="btn btn-outline text-xs px-2 py-1">اطلاعات</button><button type="button" onclick="openStudentModal('reports', <?php echo $st['id']; ?>)" class="btn btn-outline text-xs px-2 py-1 text-blue-600">کارنامه‌ها</button><button type="button" onclick="openStudentModal('discipline', <?php echo $st['id']; ?>)" class="btn btn-warning text-xs px-2 py-1">انضباط</button><a href="students.php?action=edit&id=<?php echo $st['id']; ?>" class="btn btn-secondary text-xs px-2 py-1">ویرایش</a><a href="students.php?action=reset_pass&id=<?php echo $st['id']; ?>" onclick="return confirm('رمز عبور دانش‌آموز به کد ملی بازنشانی شود؟')" class="btn btn-outline text-xs px-2 py-1 text-amber-600">🔑 رمز</a><a href="students.php?action=delete&id=<?php echo $st['id']; ?>" onclick="return confirm('آیا از حذف دانش‌آموز اطمینان دارید؟')" class="btn btn-danger text-xs px-2 py-1">حذف</a></div></td>
                    </tr>
                <?php endforeach; if (!$studentsList): ?><tr><td colspan="9" class="text-center text-muted">دانش‌آموزی با این مشخصات یافت نشد.</td></tr><?php endif; ?>
                </tbody>
            </table>
        </div>
        <?php
        ...
        <div id="reportModal" class="modal-backdrop" style="display:none">
            <div class="modal-card card">
                <h3 class="font-bold text-primary mb-3" id="modalTitle">گزارش</h3>
                <input type="hidden" name="report_type" id="reportType" value="info">
...
function openReportModal(type){
  ...
  document.getElementById('modalTitle').textContent = type==='info'?'گزارش اطلاعات دانش‌آموزان':(type==='discipline'?'گزارش انضباطی':'گزارش تحلیلی نمرات');
