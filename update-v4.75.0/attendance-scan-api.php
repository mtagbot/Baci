...
if ($action === 'status') {
    try {
        ...
        $st = DB::fetch("SELECT SUM(status='present') present, SUM(status='late') late, SUM(status='absent') absent FROM student_attendance WHERE date_jalali IN ($dfPh)", $df);
        $totalActive = DB::fetch("SELECT COUNT(*) c FROM students WHERE status='active'");
        $recent = DB::fetchAll("SELECT a.status, a.scan_time, a.minutes_late, s.first_name, s.last_name, s.class_name FROM student_attendance a JOIN students s ON s.id=a.student_id WHERE a.date_jalali IN ($dfPh) AND a.source='qr' ORDER BY a.id DESC LIMIT 8", $df);
        att_json([
            'ok' => true,
