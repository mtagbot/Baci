...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['bulk_action'])) {
        ...
        if ($action === 'file_absents' || $action === 'file_lates') {
            ...
            foreach ($rows as $rec) {
                $title = $target === 'late'
                    ? 'تأخیر ' . tr_num((string)max(1, (int)$rec['minutes_late']), 'fa') . ' دقیقه‌ای در ورود به مدرسه'
                    : 'غیبت از مدرسه';
                // one filing per attendance record — never duplicate
                $dup = DB::fetch("SELECT id FROM student_discipline_records WHERE student_id=? AND internal_note=?",
                    [(int)$rec['student_id'], 'att#' . (int)$rec['id']]);
                ...
                DB::execute("INSERT INTO student_discipline_records (student_id,title_id,title_text,internal_note,occurred_at_jalali,notify_parents,is_justified,review_status,review_note,created_by_teacher_id,created_at_jalali) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
                    [(int)$rec['student_id'], null, $title, 'att#' . (int)$rec['id'],
                     $rec['date_jalali'] . (!empty($rec['scan_time']) ? ' ' . $rec['scan_time'] : ''),
                     1, 0, 'pending',
                     'ثبت‌شده از سوابق حضور و غیاب' . (trim((string)$rec['note']) !== '' ? ' — ' . trim((string)$rec['note']) : ''),
                     $isDeputyAtt ? (int)$_SESSION['teacher_id'] : null, jalali_now()]);
                $newRid = (int)DB::lastInsertId();
                $filed++;
                try {
                    notify_student_discipline_bots((int)$rec['student_id'], $title,
                        $rec['date_jalali'] . (!empty($rec['scan_time']) ? ' ساعت ' . $rec['scan_time'] : ''), $newRid);
                    $notifiedChats++;
...
$statTotals = DB::fetch("SELECT SUM(status='present') presents, SUM(status='absent') absents, SUM(status='late') lates, SUM(review_status='acknowledged') acked FROM student_attendance WHERE date_jalali IN (" . implode(',', array_fill(0, count($dfS), '?')) . ")", $dfS);
