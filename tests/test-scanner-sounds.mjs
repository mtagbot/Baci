...
const candidateJs = [
  process.env.SCANNER_JS,
  new URL('../update-v4.131.0/assets/js/attendance-scanner-light.js', import.meta.url),
  new URL('../update-v4.152.0/assets/js/attendance-scanner-light.js', import.meta.url),
  new URL('../assets/js/attendance-scanner-light.js', import.meta.url)
].filter(Boolean);

let JS_PATH = candidateJs.find(p => {
  try { return existsSync(p); } catch(e) { return false; }
});
if (!JS_PATH) throw new Error('attendance-scanner-light.js not found');
...
test('مسیر صدا از پیکربندی صفحه قابل تغییر است (و پیش‌فرض uploads/sounds است)', () => {
  const htmlPath = [
    new URL('../update-v4.131.0/attendance-scanner.php', import.meta.url),
    new URL('../update-v4.152.0/attendance-scanner.php', import.meta.url)
  ].find(p => { try { return readFileSync(p); } catch(e){ return false; } });
  const html = readFileSync(htmlPath, 'utf8');
  assert.match(html, /sounds:\s*'uploads\/sounds\/'/, 'the page must point at the sounds directory');
  assert.match(html, /attendance-scanner-light\.js\?v=/, 'the JS cache id must change');
