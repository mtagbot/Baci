 ...
 for(const r of Object.values(byName)){
  ...
  for(const width of widths){
   ...
   const m=await page.evaluate(()=>{
    ...
    return{headerHeight:nav?nav.getBoundingClientRect().height:0,shellClipped:!!(shell&&getComputedStyle(shell).overflowY==='hidden'&&shell.scrollHeight>shell.clientHeight+1),width:w,scrollWidth:document.documentElement.scrollWidth,overflow,footer:foot?{top:foot.getBoundingClientRect().top,bottom:foot.getBoundingClientRect().bottom,mainBottom:main?main.getBoundingClientRect().bottom:0}:null,previewScope:document.body.getAttribute('data-school-return')==='preview',addedReturns:document.querySelectorAll('.school-return-nav,.school-editor-return,.school-preview-return').length,editorReturn:!!document.querySelector('#mainToolbar .school-editor-return button'),isEditor:!!document.getElementById('mainToolbar'),returnLinks:!!document.querySelector('.school-return-nav a,a[href="students.php"],a[href="index.php"]'),ui:!!window.SchoolUI,icons:document.querySelectorAll('.school-icon').length,metrics:window.SchoolUI&&SchoolUI.metrics};
   });
