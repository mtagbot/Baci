...
try{
 ...
 await context.route('**/*',async route=>{
  ...
  const snap=byName[p];if(snap)return route.fulfill({contentType:'text/html;charset=utf-8',body:readFileSync(join(REPO,'.cache/ui-audit/pages',p))});
 ...
 for(const r of Object.values(byName)){
  ...
  for(const width of widths){
   errors=[];await page.setViewportSize({width,height:900});await page.goto('https://school.test/'+r.snapshot);await page.evaluate(()=>document.fonts?document.fonts.ready:Promise.resolve());
