...
function qHeightGripDown(e,qid){
  ...
  const cell=e.target.closest('.q-content');
  qHeightDrag={qid,sy:e.clientY,h0:parseFloat(it.height)||18,cell};
  document.body.classList.add('qh-resizing');
}
document.addEventListener('pointermove',e=>{
  if(!qHeightDrag)return;
  const dyPx=e.clientY-qHeightDrag.sy;
  ...
  qHeightDrag.h=h;
  /* پیش‌نمایش زنده روی همه نسخه‌های همین سوال */
  document.querySelectorAll(`[data-qid="${qHeightDrag.qid}"] .q-content`).forEach(c=>{c.style.height=h+'mm';});
  const grip=qHeightDrag.cell?.querySelector('.q-h-grip');
  if(grip)grip.dataset.hint=(h.toLocaleString('fa-IR'))+' mm';
});
document.addEventListener('pointerup',()=>{
  if(!qHeightDrag)return;
  const {qid,h}=qHeightDrag;
...
function renderBank(items){
  ...
  bankItems=items;const box=document.getElementById('bankList'); box.innerHTML='';
