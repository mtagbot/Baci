   ...
   <div class="imp-grid" id="impGrid">
     ...
     <div class="imp-q" data-idx="<?php echo $iqIdx; ?>">
       ...
       <div class="imp-tools">
         ...
         <button type="button" class="imp-tb" title="نمادهای ریاضی" onclick="impToggleSymbols(this)">∑ نماد</button>
       ...
       <div class="imp-symbols" style="display:none"></div>
     </div>
     <?php endforeach; ?>
   </div>
 </div>
...
.imp-sym{background:#fff;border:1px solid #cbd5e1;border-radius:6px;min-width:28px;padding:3px 5px;font-size:14px;cursor:pointer}
...
const impSyms=['+','−','×','÷','=','≠','≈','≤','≥','±','√','∛','π','α','β','θ','Δ','∞','°','٪','²','³','⁴','½','⅓','¼','∠','⊥','∥','∈','∉','⊂','∪','∩','→','↔','{','}','[',']'];
function impToggleSymbols(btn){
  const q=btn.closest('.imp-q');
  const box=q.querySelector('.imp-symbols');
  if(box.style.display!=='none'){box.style.display='none';return;}
  if(!box.dataset.built){
    impSyms.forEach(sy=>{
      const b=document.createElement('button'); b.type='button'; b.className='imp-sym'; b.textContent=sy;
      b.onclick=()=>impInsertHTML(b,sy==='&'?'&amp;':sy.replace(/</g,'&lt;'));
      box.appendChild(b);
    });
    box.dataset.built='1';
  }
