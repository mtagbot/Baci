...
if (!function_exists('bot_exam_design_pdf_path')) {
    ...
    function bot_exam_design_pdf_path($examId) {
        ...
        foreach ($students as $si => $st) {
            ...
            $gap = (int)round(1.2 * $S);
            $titlePt = 15 * $PX * 0.75; $subPt = 12 * $PX * 0.75; $fieldPt = 13 * $PX * 0.75; /* v4.92.0: فونت سربرگ بزرگ‌تر و Bold */
            ...
            if ($exSub !== '') { $yCur += (int)round($subPt / 0.75 * 1.35); gd_text($strip, $exSub, $cx1 + (int)($cw / 2), $yCur, $subPt, $ink, true, 'center'); }
            ...
            foreach ($fieldsOn as $fi => $f) {
                ...
                gd_text($strip, ($f['label'] ?? '') . ': ' . $val, $bx2 - (int)round(1 * $S), $by1 + (int)round($boxH * 0.72), $fieldPt, $ink, true, 'right');
            }
