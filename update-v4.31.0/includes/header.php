...
<header class="navbar flex items-center justify-between px-6 py-4 border-b border-color bg-card shadow-sm">
    <button type="button" class="hamburger-btn" onclick="document.body.classList.toggle('sidebar-open')">☰</button>
    ...
    <div class="flex items-center gap-4">
        ...
        <div class="user-menu-wrap">
            ...
            <div class="user-menu-dropdown">
                <?php if (is_admin_logged_in()): ?><div class="badge bg-primary text-white w-full justify-center text-xs"><?php echo clean($_SESSION['admin_name'] ?? 'مدیر'); ?></div><a href="index.php?action=logout" class="btn btn-danger w-full text-xs">خروج</a>
                <?php elseif (is_student_logged_in()): ?><div class="badge bg-green-600 text-white w-full justify-center text-xs"><?php echo clean($_SESSION['student_name'] ?? 'دانش‌آموز'); ?></div><a href="index.php?action=logout" class="btn btn-danger w-full text-xs">خروج</a>
                <?php elseif (function_exists('is_teacher_logged_in') && is_teacher_logged_in()): ?><div class="badge bg-amber-600 text-white w-full justify-center text-xs"><?php echo clean($_SESSION['teacher_name'] ?? 'دبیر'); ?></div><a href="logout.php" class="btn btn-danger w-full text-xs">خروج</a><?php endif; ?>
            </div>
        </div>
    </div>
</header>
...
<div class="flex flex-1">
    ...
    <aside class="sidebar w-64 bg-card border-l border-color p-4 flex flex-col gap-1.5 shrink-0 shadow-lg">
        ...
        <?php if (has_permission('manage_students')): ?>
            ...
            <a href="deputy-panel.php" class="sidebar-item <?php echo basename($_SERVER['PHP_SELF']) == 'deputy-panel.php' ? 'active' : ''; ?>">🛡️ موارد انضباطی</a>
        <?php endif; ?>
        ...
        <?php if (has_permission('manage_reports')): ?>
            ...
            <a href="exams.php" class="sidebar-item font-bold text-rose-600 <?php echo basename($_SERVER['PHP_SELF']) == 'exams.php' ? 'active' : ''; ?>">📝 امتحانات حضوری</a>
            <!-- Virtual exams moved under evaluation, below presencial -->
            <a href="online-exams.php" class="sidebar-item font-bold text-indigo-600 <?php echo in_array(basename($_SERVER['PHP_SELF']), ['online-exams.php','online-exam-form.php','online-exam-questions.php','online-exam-monitor.php','online-exam-results.php','online-exam-result.php','online-exam-grading.php','online-exam-categories.php']) ? 'active' : ''; ?>" style="background:rgba(79,70,229,0.08);border-right:3px solid #4f46e5;">🧪 آزمون‌های مجازی (آنلاین)</a>
            <a href="online-question-bank.php" class="sidebar-item text-purple-600 <?php echo basename($_SERVER['PHP_SELF']) == 'online-question-bank.php' ? 'active' : ''; ?>" style="padding-right:20px;">📚 بانک سوالات مجازی</a>
        <?php endif; ?>
        <?php if (has_permission('import_data')): ?>
            <div class="sidebar-section-title">ورود و خروج داده</div>
            <a href="student-recovery.php" class="sidebar-item font-bold text-green-700 <?php echo in_array(basename($_SERVER['PHP_SELF']), ['student-recovery.php','import-students.php','import-photos.php']) ? 'active' : ''; ?>">👥 بازیابی دانش‌آموز</a>
        <?php endif; ?>
        <?php if (has_permission('send_sms')): ?>
            ...
            <a href="bale-bot.php" class="sidebar-item font-bold text-teal-600 <?php echo basename($_SERVER['PHP_SELF']) == 'bale-bot.php' ? 'active' : ''; ?>">🤖 بازوی بله</a>
            <a href="telegram-bot.php" class="sidebar-item font-bold text-sky-600 <?php echo basename($_SERVER['PHP_SELF']) == 'telegram-bot.php' ? 'active' : ''; ?>">✈️ ربات تلگرام</a>
            <a href="bot-accounts.php" class="sidebar-item font-bold text-cyan-600 <?php echo basename($_SERVER['PHP_SELF']) == 'bot-accounts.php' ? 'active' : ''; ?>">🔗 اکانت‌های متصل</a>
        <?php endif; ?>
        <div class="sidebar-section-title">سیستم</div>
        <a href="other-settings.php" class="sidebar-item <?php echo in_array(basename($_SERVER['PHP_SELF']), ['other-settings.php','backups.php','activity-logs.php','migration-updater.php']) ? 'active' : ''; ?>">⚙️ تنظیمات دیگر</a>
        ...
        <?php if (has_permission('system_settings')): ?><a href="settings.php" class="sidebar-item font-semibold text-amber-600 <?php echo basename($_SERVER['PHP_SELF']) == 'settings.php' ? 'active' : ''; ?>">🎨 سفارشی‌سازی</a><?php endif; ?>
        <?php if (($_SESSION['admin_role'] ?? '') === 'super_admin'): ?><a href="admins.php" class="sidebar-item text-purple-600 <?php echo basename($_SERVER['PHP_SELF']) == 'admins.php' ? 'active' : ''; ?>">مدیریت مدیران</a><?php endif; ?>
        </div>
    </aside>
    ...
    <aside class="sidebar w-64 bg-card border-l border-color p-4 flex flex-col gap-2 shrink-0 shadow-lg">
        ...
        <div class="sidebar-section-title">ارزشیابی و امتحانات</div>
        <a href="student-online-exams.php" class="sidebar-item font-bold text-indigo-600 <?php echo in_array(basename($_SERVER['PHP_SELF']), ['student-online-exams.php','online-exam-take.php','online-exam-result.php']) ? 'active' : ''; ?>" style="background:rgba(79,70,229,0.08);border-right:3px solid #4f46e5;">🧪 آزمون‌های مجازی من</a>
    </aside>
