        ...
        if (a.addEventListener) {
          a.addEventListener('error', function () {
            if (!st.elemFallback) {
              ...
              var altBase = (url.indexOf('uploads/sounds') !== -1) ? 'assets/audio/' : 'uploads/sounds/';
              try { var a2 = new Audio(altBase + SOUND_FILES[name]); a2.preload = 'auto'; st.element = a2; } catch(e){}
            }
          });
        }
      } else st.failed = true;
    ...
    try {
      ...
      xhr.onload = function () {
        ...
        if (!body || (xhr.status && (xhr.status < 200 || xhr.status >= 300))) {
          if (xhr.status === 404 && !st.triedFallback) {
            st.triedFallback = true;
            var altBase = (soundBase.indexOf('uploads/sounds') !== -1) ? 'assets/audio/' : 'uploads/sounds/';
            var altUrl = altBase + SOUND_FILES[name];
            var xhr2 = new XMLHttpRequest();
            xhr2.open('GET', altUrl, true);
            xhr2.responseType = 'arraybuffer';
            xhr2.onload = function () {
              var b2 = xhr2.response;
              if (!b2 || (xhr2.status && (xhr2.status < 200 || xhr2.status >= 300))) {
                st.loading = false; st.failed = true; return;
              }
              st.loading = false; st.data = b2; decodeSound(name, st);
            };
            xhr2.onerror = function () { st.loading = false; st.failed = true; };
            xhr2.send(null);
            return;
          }
          st.loading = false; st.failed = true; return;
