...
try {
    ...
    echo "تأیید: school_roles schema ok\n";
} catch (Exception $e) { echo "خطا: school_roles: ".$e->getMessage()."\n"; }

try {
    ensure_online_exams_schema();
    echo "تأیید: online_exams schema ok\n";
} catch (Exception $e) { echo "خطا: online_exams: ".$e->getMessage()."\n"; }
...
foreach ($tables as $t) {
    try {
        ...
        echo "تأیید: $t: ".$c['c']." rows\n";
    } catch (Exception $e) {
        echo "خطا: $t: ".$e->getMessage()."\n";
    }
}
...
foreach ($dirs as $d) {
    $full = __DIR__.'/'.$d;
    echo (is_dir($full) ? "تأیید:" : "خطا:")." $d ".(is_dir($full) ? (is_writable($full)?'writable':'NOT writable') : 'MISSING')."\n";
...
foreach ($files as $f) {
    $full = __DIR__.'/'.$f;
    echo (file_exists($full) ? "تأیید:" : "خطا:")." $f\n";
}
...
if (!isset($_SERVER['HTTPS']) || $_SERVER['HTTPS']=='off') {
    echo "هشدار: برای کارکرد موقعیت مکانی GPS، سایت باید https باشد. الان http است!\n";
}
