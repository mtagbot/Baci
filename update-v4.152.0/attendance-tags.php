...
/* v4.135.0: فیلتر پایه و چاپ تکی */
$fGrade = trim($_GET['grade'] ?? '');
$fOne   = (int)($_GET['student_id'] ?? 0);
/* v4.135.0: ترتیب چیدمان تگ‌ها */
...
/* فهرست پایه‌ها برای فیلتر (مستقل از فیلترهای جاری تا همیشه کامل باشد) */
$gradeRows = DB::fetchAll("SELECT DISTINCT s.grade_level FROM students s WHERE s.status='active' AND $tagYearSql AND s.grade_level<>'' ", $tagYearParams);
persian_usort_grades($gradeRows);

/* همهٔ دانش‌آموزان سال جاری — برای لیست «چاپ تکی» */
$allForPick = DB::fetchAll("SELECT s.id, s.first_name, s.last_name, s.class_name FROM students s WHERE s.status='active' AND $tagYearSql", $tagYearParams);
persian_usort_students($allForPick);

/* ---- چیدمان تگ‌ها ----------------------------------------------
   مرتب‌سازی فارسی است (آ قبل از ا) و در همهٔ حالت‌ها، نام خانوادگی
   به‌عنوان معیار دوم می‌آید تا خروجی پایدار و قابل پیش‌بینی بماند. */
switch ($sortKey) {
    case 'name':   /* نام خانوادگی، سپس نام */
        persian_usort_students($students);
        break;
    case 'first':  /* نام، سپس نام خانوادگی */
        usort($students, function ($a, $b) {
            $c = persian_compare($a['first_name'] ?? '', $b['first_name'] ?? '');
            return $c !== 0 ? $c : persian_compare($a['last_name'] ?? '', $b['last_name'] ?? '');
        });
        break;
    case 'grade':  /* پایه، سپس کلاس، سپس نام خانوادگی */
        usort($students, function ($a, $b) {
        ...
        break;
    case 'nid':    /* کد ملی — برای تطبیق با فهرست‌های اداری */
        usort($students, function ($a, $b) {
            return strcmp((string)($a['national_id'] ?? ''), (string)($b['national_id'] ?? ''));
        });
        break;
    case 'class':
    default:       /* کلاس، سپس نام خانوادگی (پیش‌فرض قبلی) */
        usort($students, function ($a, $b) {
            $c = persian_compare($a['class_name'] ?? '', $b['class_name'] ?? '');
            if ($c !== 0) return $c;
...
if (isset($_GET['print'])) {
    $D = [
        ...
        'frame'  => in_array(($_GET['frame'] ?? 'none'), ['none', 'dashed', 'solid'], true) ? ($_GET['frame'] ?? 'none') : 'none',
    ...
    /* ---- v4.135.0: فونت واقعی سامانه در چاپ ----------------------
       تا پیش از این، صفحهٔ چاپ font-family:Tahoma را ثابت داشت. نتیجه:
       پیش‌نمایش با فونت انتخابی مدرسه دیده می‌شد ولی خروجی چاپ با فونت
       سیستم بیرون می‌آمد. حالا همان فونتِ «سفارشی‌سازی» اینجا هم
       @font-face می‌شود.

       نکتهٔ مهم: این صفحه بدون هدر سایت رندر می‌شود، پس هیچ‌کدام از
       @font-face های header.php را ندارد و باید خودش تعریف کند. */
    ...
    } elseif (isset($fontDirs[$printFont])) {
        foreach ($fontDirs[$printFont] as $f) {
            list($base, $weight) = $f;
            $srcs = [];
            /* فقط فایل‌هایی که واقعاً روی دیسک هستند اعلام می‌شوند؛
               یک src شکسته، کل @font-face را بی‌اثر می‌کند. */
            ...
            if ($srcs) {
                $fontFaces .= "@font-face{font-family:'{$printFont}';src:" . implode(',', $srcs)
                            . ";font-weight:{$weight};font-display:block}\n";
            }
        }
        if ($fontFaces !== '') $fontStack = "'{$printFont}', Tahoma, sans-serif";
    }
    ?>
<!DOCTYPE html>
...
.t-name{font-family:<?php echo $fontStack; ?>;font-size:<?php echo round($fsName, 2); ?>mm;font-weight:700;color:#000;line-height:1.3}
.t-sub{font-family:<?php echo $fontStack; ?>;font-size:<?php echo round($fsSub, 2); ?>mm;color:#000;line-height:1.3}
...
(function(){
  ...
  /* v4.135.0: پلاک وسط QR هم با فونت مدرسه کشیده می‌شود، نه Tahoma ثابت */
  var PFONT  = <?php echo json_encode($fontStack, JSON_UNESCAPED_UNICODE); ?>;
  function draw(cv){
    var qr = qrcode(0, 'H');
    ...
    if (PLATE && LABEL) {
      ...
      ctx.font = '700 ' + fs + 'px ' + PFONT;
      while (ctx.measureText(label).width > boxW * 0.88 && fs > 8) { fs--; ctx.font = '700 ' + fs + 'px ' + PFONT; }
      ctx.fillText(label, size / 2, by + boxH / 2 + fs * 0.06);
    }
  }
  function drawAll(){ document.querySelectorAll('canvas.qr-tag[data-qr]').forEach(draw); }

  /* v4.135.0 — چاپ باید تا لود شدن فونت صبر کند.
     canvas متن را در همان لحظهٔ رسم «قفل» می‌کند؛ اگر فونت هنوز نرسیده
     باشد پلاک با فونت جایگزین کشیده می‌شود و دیگر اصلاح نمی‌شود. پیش از
     این فقط ۴۰۰ میلی‌ثانیه صبر می‌کرد که روی سیستم کند کافی نبود.
     document.fonts.ready دقیقاً همین را تضمین می‌کند؛ روی مرورگر قدیمی
     که این API را ندارد، به همان تأخیر ثابت برمی‌گردیم. */
  function whenFontsReady(cb){
    if (document.fonts && document.fonts.ready && typeof document.fonts.ready.then === 'function') {
      var done = false;
      var go = function(){ if (!done) { done = true; cb(); } };
      document.fonts.ready.then(go);
      setTimeout(go, 3000);          /* اگر فونت اصلاً نیامد، گیر نکنیم */
    } else {
      setTimeout(cb, 600);
    }
  }
...
<div class="space-y-6">
    ...
    <div class="card no-print">
        <form method="GET" class="grid grid-cols-4 gap-3 items-end">
            <?php /* v4.135.0: فیلتر پایه + ترتیب چیدمان + چاپ تکی */ ?>
            <div>
            ...
            <div>
                <label class="text-xs">ترتیب چیدمان تگ‌ها</label>
            ...
            <div>
                ...
                <select name="student_id" id="oneStudent" class="form-select">
            ...
            <?php if ($fClass !== '' || $fGrade !== '' || $fOne > 0 || $sortKey !== 'class'): ?>
                <a href="attendance-tags.php" class="btn btn-outline text-xs">حذف فیلترها</a>
            <?php endif; ?>
    ...
    <div class="card no-print" id="tagEditor">
        ...
        <div class="grid grid-cols-4 gap-4">
            <div>
                <label class="text-xs" for="dCopies">تعداد نسخه برای هر دانش‌آموز</label>
                <select id="dCopies" class="form-select" data-no-search="1" onchange="applyDesign()">
    ...
    <p id="copiesNote" class="text-xs text-muted no-print"></p>
    <div class="qr-grid" id="qrGrid">
        ...
        <div class="qr-item">
            ...
            <div class="qr-ident no-print" style="display:none">
                ...
                <?php /* v4.135.0: چاپ تکی مستقیم از روی همین کارت */ ?>
                <button type="button" class="btn btn-accent text-xs"
                        onclick="printOne(<?php echo (int)$s['id']; ?>)">چاپ این تگ</button>
            </div>
        </div>
...
/* v4.135.0: فونت انتخابی مدرسه، برای هماهنگی پیش‌نمایش با چاپ نهایی */
var PREVIEW_FONT = <?php echo json_encode("'" . get_setting('font_family','Vazirmatn') . "', Tahoma, sans-serif", JSON_UNESCAPED_UNICODE); ?>;
var QRCACHE = {};
var _redrawT = null;
...
function getDesign(){
  return {
    copies: printCopies($id('dCopies').value),
...
function setDesign(d){
  $id('dCopies').value = printCopies(d.copies);
...
function applyDesign(){
  var d = getDesign();
  $id('copiesNote').textContent = 'فهرست زیر هر دانش‌آموز را یک بار نشان می‌دهد؛ در چاپ، برای هر نفر ' + faNum(d.copies) + ' نسخه متوالی، در مجموع ' + faNum(<?php echo (int)count($students); ?> * d.copies) + ' تگ قرار می‌گیرد.';
...
function resetDesign(){
  setDesign({copies:1, size:40, gap:4, margin:8, frame:'none', plate:true, ptext:'', pscale:40, sname:false, scls:false, snid:false});
  applyDesign();
}
...
function drawTag(cv, plate, pscale, label){
  try {
    ...
    if (plate && label) {
      ...
      ctx.fillStyle = '#000'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
      var fs = Math.round(boxH * 0.52);
      /* v4.135.0: پیش‌نمایش هم همان فونت سامانه را می‌کشد، وگرنه آنچه
         روی صفحه می‌بینید با خروجی چاپ فرق می‌کرد. */
      ctx.font = '700 ' + fs + 'px ' + PREVIEW_FONT;
      while (ctx.measureText(lb).width > boxW * 0.88 && fs > 8) { fs--; ctx.font = '700 ' + fs + 'px ' + PREVIEW_FONT; }
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  <?php if (isset($_GET['copies'])): ?>$id('dCopies').value = <?php echo $copies; ?>;<?php endif; ?>
  applyDesign();
  renderAll();
});
</script>
