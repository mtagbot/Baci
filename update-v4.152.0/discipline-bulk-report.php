...
if (!(is_admin_logged_in() && has_permission('manage_students')) && !(is_teacher_logged_in() && teacher_has_deputy($_SESSION['teacher_id']))) app_page_error('غیرمجاز');
if ($_SERVER['REQUEST_METHOD'] !== 'POST' || !verify_csrf($_POST['csrf_token'] ?? '')) app_page_error('درخواست نامعتبر');
...
if (!$ids) app_page_error('دانش‌آموزی انتخاب نشده است.');
...
?><!DOCTYPE html><html lang="fa" dir="rtl"><head><meta name=viewport content="width=device-width, initial-scale=1"><meta charset="UTF-8"><style>body{font-family:Tahoma}table{width:100%;border-collapse:collapse}th,td{border:1px solid #333;padding:6px}th{background:#eee}.head{text-align:center;border:2px solid #000;margin-bottom:12px;padding:10px}</style><link rel=stylesheet href=assets/css/school-ui.css?v20260917c><script defer src=assets/js/school-icons.js?v20260917c></script><script defer src=assets/js/school-ui.js?v20260917c></script></head><body><div class="head"><h2><?php echo clean(get_setting('school_name','آموزشگاه')); ?></h2><h3>گزارش انضباطی دانش‌آموزان</h3><p>تاریخ گزارش: <?php echo tr_num(jdate('Y/m/d H:i'),'fa'); ?></p></div><table><thead><tr><th>ردیف</th><th>دانش‌آموز</th><th>کلاس</th><th>تعداد کل موارد</th><th>موجه</th><th>بررسی‌شده</th><th>آخرین مورد</th><th>جزئیات</th></tr></thead><tbody><?php foreach($students as $i=>$s):
