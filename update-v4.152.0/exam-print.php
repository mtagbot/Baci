...
function qDirection(dir){const ed=document.getElementById('qText'); qRestoreSel(); const sel=getSelection();
  if(sel.rangeCount&&!sel.isCollapsed&&ed.contains(sel.anchorNode)){
    ...
    span.setAttribute('dir',dir); span.style.unicodeBidi='isolate';
  ...
  }else{ed.setAttribute('dir',dir); ed.style.textAlign=dir==='ltr'?'left':'right';}
  updateQuestionPreview();}
...
function applyFontToEditor(){const ed=document.getElementById('qText'); ed.style.setProperty('font-family',document.getElementById('qFont').value,'important'); ed.style.fontSize=document.getElementById('qFontSize').value+'px'; updateQuestionPreview();}
...
function editorViewportScale(){
  ...
  if(!mobile){root.style.setProperty('--editorScale','1'); return;}
  const base=page.offsetWidth||793, available=Math.max(260,window.innerWidth-20);
