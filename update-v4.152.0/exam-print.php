...
</style><?php echo app_appearance_head(); ?><link rel=stylesheet href=assets/css/school-ui.css?v20260917d><script defer src=assets/js/school-icons.js?v20260917d></script><script defer src=assets/js/school-ui.js?v20260917d></script></head><body data-school-return="preview">
...
?><!DOCTYPE html><html lang="fa" dir="rtl"><head><meta name=viewport content="width=device-width, initial-scale=1"><meta charset="UTF-8"><title>ویرایشگر چاپ سوالات</title>
<style>
...
.bank-item{border:1px solid var(--line);border-radius:12px;padding:0;margin-bottom:9px;background:#fff;overflow:hidden;transition:border-color .15s,box-shadow .15s}
.bank-item:hover{border-color:var(--brand);box-shadow:0 3px 14px rgba(37,99,235,.12)}
.bi-head{background:linear-gradient(135deg,#eef4ff,#e0e9ff);border-bottom:1px solid #dbe3f5;padding:7px 9px;display:flex;align-items:center;gap:6px;flex-wrap:wrap}
.bi-subject{font-size:12.5px;color:#1e3a8a}
.bi-chips{display:flex;gap:4px;flex-wrap:wrap}
.bi-chip{background:#fff;border:1px solid #c7d5f2;color:#334155;border-radius:999px;padding:1px 8px;font-size:10px;white-space:nowrap}
.bi-chip.bi-grade{background:#1d4ed8;border-color:#1d4ed8;color:#fff;font-weight:700}
.bi-chip.bi-designer{background:#f0fdf4;border-color:#bbf7d0;color:#166534}
.bi-chip.bi-score{background:#fffbeb;border-color:#fde68a;color:#92400e;font-weight:700}
.bank-item .bank-q{margin:7px 9px}
.bi-actions{display:flex;gap:5px;padding:0 9px 8px}
.bi-insert{background:#2563eb!important;color:#fff!important;border:0!important;border-radius:8px;padding:5px 12px;font-weight:700;cursor:pointer}
.bi-insert:hover{background:#1d4ed8!important}
.bi-del{background:#fee2e2!important;color:#b91c1c!important;border:0!important;border-radius:8px;padding:5px 10px;cursor:pointer}
.bi-del:hover{background:#fecaca!important}
...
.rich-tools button:hover{background:#e0e9ff;border-color:var(--brand)}
.rt-sep{width:1px;height:20px;background:#cbd5e1;margin:0 3px;align-self:center}
.qsym-tabs{display:flex;gap:4px;flex-wrap:wrap;margin:10px 0 8px}
.qsym-tab{background:#f1f5f9;border:1px solid #cbd5e1;border-radius:8px;padding:4px 10px;font-size:12px;cursor:pointer}
.qsym-tab.active{background:#2563eb;color:#fff;border-color:#2563eb}
.qsym-grid{display:flex;flex-wrap:wrap;gap:4px;max-height:44vh;overflow:auto;background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:8px}
.qsym-btn{background:#fff;border:1px solid #cbd5e1;border-radius:6px;min-width:30px;padding:4px 6px;font-size:15px;cursor:pointer}
.qsym-btn:hover{border-color:#2563eb;background:#eff6ff}
.qeq-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:8px;max-height:46vh;overflow:auto;margin-top:8px}
.qeq-card{background:#fff;border:1px solid #cbd5e1;border-radius:10px;padding:10px 6px;cursor:pointer;display:flex;flex-direction:column;align-items:center;gap:6px;min-height:64px;justify-content:center}
.qeq-card:hover{border-color:#2563eb;background:#eff6ff}
.qeq-card small{color:#64748b;font-size:10.5px}
.rich-editor .eq-slot,.qeq-card .eq-slot{display:inline-block;min-width:22px;border:1px dashed #94a3b8;border-radius:4px;padding:0 4px;text-align:center}
.rich-editor .eq-slot:focus{outline:2px solid #2563eb}
.pfrac{direction:ltr}.rich-editor{min-height:105px;border:1px solid #94a3b8;border-radius:8px;padding:0 8px 8px;background:#fff;outline:none;position:relative;overflow:auto}.rich-editor p{margin:2px 0}.rich-editor p:first-child{margin-top:0}.rich-editor img{max-width:100%;height:auto;position:relative;display:inline-block}.page{--headerH:39mm;--photo:20mm;--stamp:18mm;/* v4.95.0: مهر ۱۸ + شماره صندلی ۸.۷ = ارتفاع عکس ۳×۴ *//* v4.100.0: کادر همیشه 1px، عرض حداکثر، فاصله 0، ارتفاع = کل فضای باقیمانده صفحه */--qW:210mm;--qTop:0mm;--qBorder:1px;background:white;width:210mm;min-height:297mm;margin:var(--toolbarH,58px) auto 0;padding:0;overflow:hidden}.exam-header{height:var(--headerH);border:1.1mm double #000;padding:1.6mm 3mm;display:grid;grid-template-columns:var(--photo) 1fr var(--stamp);gap:2.5mm;align-items:center;font-family:BTitr,Vazirmatn,Tahoma,sans-serif;background:#fff;position:relative/* v4.118.0: قاب دوخطه رسمی */}.page.no-header .exam-header{display:none}.photo,.stamp{width:100%;border:.3mm solid #000;border-radius:1mm;padding:.5mm;background:#fff;display:flex;align-items:center;justify-content:center;font-size:9px;overflow:hidden}
.photo img,.stamp img{border-radius:.6mm}.photo{height:calc(var(--photo) * 4 / 3)}.stamp{height:var(--stamp)}.photo img{width:100%;height:100%;object-fit:cover}.stamp img{width:100%;height:100%;object-fit:contain}/* v4.95.0: کادر شماره صندلی چسبیده به زیر مهر — مجموع = ارتفاع عکس ۳×۴ */.stampcol{width:100%;display:flex;flex-direction:column}.seatno{width:100%;height:calc(var(--photo) * 4 / 3 - var(--stamp));border:.3mm solid #000;border-top:0;border-radius:0 0 1mm 1mm;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:700;font-variant-numeric:tabular-nums;overflow:hidden;background:#fff}
.stamp{border-radius:1mm 1mm 0 0}.hcenter{display:flex;align-items:center;justify-content:center;gap:2.5mm;font-size:15.5px;font-weight:normal;letter-spacing:.2px}
.hcenter b{font-weight:normal;white-space:nowrap}
.hcenter .hline{flex:1;max-width:26mm;height:0;border-top:.35mm solid #000;position:relative}
.hcenter .hline:after{content:'';position:absolute;top:.45mm;left:0;right:0;border-top:.15mm solid #000}
.hsub{text-align:center;font-size:11px;color:#000;margin-top:.4mm;letter-spacing:.4px}.meta{display:grid;grid-template-columns:repeat(4,1fr);gap:1.3mm;margin-top:1.6mm}
.box{border:.3mm solid #000;border-radius:1mm;padding:.7mm 1.2mm;font-size:12.5px;font-weight:normal;display:flex;align-items:baseline;gap:1.2mm;background:#fff;min-height:6mm}
.box .box-l{font-size:10px;white-space:nowrap;opacity:.92}
.box .box-l:after{content:':'}
.box .box-v{flex:1;text-align:center;font-variant-numeric:tabular-nums}.qwrap{position:relative;height:calc(296mm - var(--headerH) - var(--qTop));width:var(--qW);margin:var(--qTop) auto 0;border:var(--qBorder) solid #000;overflow:hidden;background:#fff}.page.no-header .qwrap{height:294mm;margin-top:1mm}.page.blank-sep{background:#fff}.page.blank-sep:after{content:'';display:block}.source-file{display:flex;align-items:flex-start;justify-content:center;min-height:0}.source-file:empty{display:none}.source-file{position:relative;overflow:hidden;width:100%;height:auto;background:#fff}.source-file img{width:var(--srcW,100%);height:auto;max-height:none;object-fit:contain;clip-path:inset(var(--cropT,0%) var(--cropR,0%) var(--cropB,0%) var(--cropL,0%));transform:translate(var(--srcX,0mm), var(--srcY,0mm));transform-origin:top right;filter:contrast(var(--srcContrast,100%)) brightness(var(--srcBrightness,100%));mix-blend-mode:var(--srcBlend,normal)}.source-file iframe{width:100%;height:80mm;border:0}.questions-table{width:100%;border-collapse:collapse;direction:rtl}.questions-table th,.questions-table td{border:1px solid #000;padding:4px;vertical-align:top;font-size:11px}.questions-table th{background:#f1f5f9;text-align:center}.q-no{width:8mm;text-align:center;font-weight:bold;font-family:BTitr,Vazirmatn,Tahoma,sans-serif/* v4.114.0 */}.q-score{width:10mm;text-align:center;font-weight:bold;font-family:BTitr,Vazirmatn,Tahoma,sans-serif/* v4.114.0 */}.th-q-title{font-family:BTitr,Vazirmatn,Tahoma,sans-serif/* v4.114.0: فقط سرستون «سوال» با فونت تیتر */}.q-content{min-height:12mm;position:relative;overflow:hidden;padding-top:0!important}/* v4.100.0: سوال چسبیده به بالای فضای متن */.q-content p{margin:2px 0}.q-content p:first-child{margin-top:0}.q-content .q-options{margin-top:2px}.q-content img{position:absolute!important;z-index:3;max-width:none;cursor:move}.questions-body .q-content img{display:block}.bank-panel img{max-width:100%;height:auto;position:static!important}.q-options{margin-top:4px;display:grid;grid-template-columns:1fr 1fr;gap:3px}.blank-line{display:inline-block;border-bottom:1px dotted #000;min-width:35mm;height:10px}.tf-square{display:inline-block;width:12px;height:12px;border:1px solid #000;margin:0 5px;vertical-align:middle}.q-actions{display:flex;gap:4px;margin-top:4px}.q-actions button{font-size:9px;padding:2px 5px}
.q-h-grip{position:absolute;left:0;right:0;bottom:0;height:7px;cursor:ns-resize;z-index:8;background:transparent;transition:background .15s}
.q-h-grip:hover,body.qh-resizing .q-h-grip{background:linear-gradient(to bottom,transparent,rgba(37,99,235,.28))}
.q-h-grip:hover:after,body.qh-resizing .q-h-grip:after{content:attr(data-hint);position:absolute;left:50%;transform:translateX(-50%);bottom:8px;background:#1d4ed8;color:#fff;font-size:10px;padding:1px 8px;border-radius:6px;white-space:nowrap;font-family:Vazirmatn,Tahoma}
.q-h-grip:hover:before{content:'⋯';position:absolute;left:50%;transform:translateX(-50%);bottom:-3px;color:#1d4ed8;font-size:14px;line-height:7px}
body.qh-resizing{user-select:none;-webkit-user-select:none;cursor:ns-resize}
@media print{.q-h-grip{display:none!important}}.question-row[draggable="true"]{cursor:move}.question-row.dragging{opacity:.45}.drawCanvas{position:absolute;inset:0;z-index:5;pointer-events:none}.drawOverlay{position:absolute!important;inset:0!important;width:100%!important;height:100%!important;z-index:50!important;pointer-events:none!important;display:block!important;max-width:none!important}.draw-on .drawCanvas{pointer-events:auto}body.drawing-mode .drawOverlay{display:none!important}.header-field-row{display:grid;grid-template-columns:24px 1fr 1fr 28px;gap:4px;align-items:center;margin-bottom:4px}.selected-img{outline:2px solid #2563eb;outline-offset:1px}
.img-resize-box{position:absolute;z-index:60;pointer-events:none;border:1.5px dashed #2563eb;border-radius:2px}
.img-handle{position:absolute;width:12px;height:12px;background:#fff;border:2px solid #2563eb;border-radius:50%;pointer-events:auto;z-index:61;box-shadow:0 1px 3px rgba(0,0,0,.3)}
.img-handle-nw{left:-6px;top:-6px;cursor:nwse-resize}
.img-handle-n{left:calc(50% - 6px);top:-6px;cursor:ns-resize}
.img-handle-ne{right:-6px;top:-6px;cursor:nesw-resize}
.img-handle-e{right:-6px;top:calc(50% - 6px);cursor:ew-resize}
.img-handle-se{right:-6px;bottom:-6px;cursor:nwse-resize}
.img-handle-s{left:calc(50% - 6px);bottom:-6px;cursor:ns-resize}
.img-handle-sw{left:-6px;bottom:-6px;cursor:nesw-resize}
.img-handle-w{left:-6px;top:calc(50% - 6px);cursor:ew-resize}
body.img-resizing{user-select:none;-webkit-user-select:none}.upload-modal{position:fixed;inset:0;background:rgba(15,23,42,.55);z-index:60;display:flex;align-items:center;justify-content:center}.upload-card{width:min(560px,94vw);background:white;border-radius:14px;padding:16px;box-shadow:0 20px 60px rgba(0,0,0,.25);position:relative}.upload-close{position:absolute;left:12px;top:10px;border:0;background:#ef4444;color:white;border-radius:8px;padding:4px 10px}.upload-steps{display:grid;grid-template-columns:repeat(4,1fr);gap:6px;margin:12px 0}.upload-steps span{background:#e5e7eb;border-radius:8px;padding:8px;text-align:center;font-size:11px}.upload-steps span.active{background:#2563eb;color:white}.upload-step-body{border:1px solid #e2e8f0;border-radius:10px;padding:12px;margin:10px 0}.upload-step-body input{width:100%;padding:9px;border:1px solid #cbd5e1;border-radius:8px}.progressbar{height:12px;background:#e5e7eb;border-radius:999px;overflow:hidden}.progressbar div{height:100%;width:0;background:#16a34a;transition:.2s}.upload-actions{display:flex;gap:8px;justify-content:flex-end}.questions-hidden .questions-table{display:none!important}.btn-save{background:#16a34a!important;color:white!important}.btn-print{background:#f59e0b!important;color:white!important}.toolbar button{border:0;border-radius:6px;padding:5px 8px;font-family:inherit}.editor-panel,.bank-panel{font-family:Vazirmatn,Tahoma,sans-serif}.image-modal{position:fixed;inset:0;background:rgba(15,23,42,.58);z-index:75;display:none;align-items:center;justify-content:center}.image-card{width:min(980px,96vw);max-height:94vh;overflow:auto;background:white;border-radius:14px;padding:14px;box-shadow:0 20px 60px rgba(0,0,0,.3)}.image-editor-grid{display:grid;grid-template-columns:1.5fr 1fr;gap:12px}.image-canvas-wrap{background:#f1f5f9;border:1px solid #cbd5e1;border-radius:10px;padding:10px;text-align:center;overflow:auto}.image-canvas-wrap canvas{max-width:100%;height:auto;background:white}.image-controls label{font-size:11px;font-weight:700;display:block;margin-top:6px}.image-controls input[type=range],.image-controls input[type=file]{width:100%}.math-modal{position:fixed;inset:0;background:rgba(15,23,42,.55);z-index:70;display:none;align-items:center;justify-content:center}.math-card{width:min(760px,96vw);max-height:92vh;overflow:auto;background:white;border-radius:14px;padding:14px;box-shadow:0 20px 60px rgba(0,0,0,.28)}.math-grid{display:grid;grid-template-columns:repeat(8,1fr);gap:5px;margin:8px 0}.math-grid button{padding:7px;border:1px solid #cbd5e1;background:#f8fafc;border-radius:7px;font-size:16px}.math-preview{border:1px dashed #94a3b8;border-radius:8px;padding:10px;min-height:42px;background:#f8fafc;font-size:18px;direction:ltr;text-align:left}.math-token{direction:ltr;unicode-bidi:embed;font-family:Cambria Math,Times New Roman,serif;font-size:1.12em;display:inline-block;padding:0 2px}.math-frac{display:inline-flex;flex-direction:column;vertical-align:middle;text-align:center;line-height:1.05;margin:0 3px}.math-frac .top{border-bottom:1px solid #111;padding:0 4px}.math-frac .bottom{padding:0 4px}.math-frac .math-frac{font-size:.92em}.math-sqrt{display:inline-flex;align-items:baseline}.math-sqrt-sym{font-size:1.12em}.math-sqrt-body{border-top:1.2px solid #111;padding:0 3px}.math-matrix{display:inline-table;border-collapse:collapse;vertical-align:middle;margin:0 4px;direction:ltr}.math-matrix td{border:0!important;padding:1px 6px!important;text-align:center}.math-bracket{display:inline-flex;align-items:center;gap:2px;direction:ltr}.math-big{font-size:1.8em;line-height:1}.math-vector{position:relative;display:inline-block}.math-vector:before{content:"→";position:absolute;top:-.85em;left:0;right:0;text-align:center;font-size:.8em}/* v4.95.0: بخش آزمون‌های ذخیره‌شده در بانک */.bank-exam{border:1px solid #cbd5e1;border-radius:8px;padding:7px;margin-bottom:7px;background:#fff}.bank-exam-thumbs{display:flex;gap:4px;overflow-x:auto;margin:5px 0;cursor:zoom-in}.bank-exam-thumbs img{height:86px;width:auto;border:1px solid #e2e8f0;border-radius:4px;background:#fff;position:static!important}.bank-exam small{color:#475569}
.bank-sec{border-top:2px solid #e2e8f0;padding-top:8px}
.bank-sec-title{cursor:pointer;font-weight:800;font-size:13px;color:#0f172a;list-style:none;display:flex;align-items:center;gap:6px;padding:4px 0;user-select:none}
.bank-sec-title::-webkit-details-marker{display:none}
.bank-sec-title:before{content:'▸';color:#2563eb;transition:transform .18s;display:inline-block}
.bank-sec[open]>.bank-sec-title:before{transform:rotate(90deg)}
.bank-count{background:#e0e9ff;color:#1d4ed8;border-radius:999px;padding:0 8px;font-size:11px;font-weight:700}
.bank-pager{display:flex;align-items:center;gap:4px;flex-wrap:wrap;margin:8px 0 2px;padding:6px;background:#f1f5f9;border:1px solid #e2e8f0;border-radius:9px}
.bp-btn{background:#fff;border:1px solid #cbd5e1;border-radius:7px;min-width:26px;padding:3px 7px;font-size:11.5px;cursor:pointer;font-family:inherit;transition:all .12s}
.bp-btn:hover{border-color:#2563eb;color:#1d4ed8}
.bp-btn.active{background:#2563eb;border-color:#2563eb;color:#fff;font-weight:700}
.bp-btn.bp-nav{font-weight:700}
.bp-dots{color:#94a3b8;padding:0 2px}
.bp-info{margin-inline-start:auto;font-size:10.5px;color:#64748b}.exam-zoom-modal{position:fixed;inset:0;background:rgba(15,23,42,.72);z-index:80;display:none;align-items:center;justify-content:center}.exam-zoom-card{width:min(880px,96vw);max-height:94vh;overflow:auto;background:#fff;border-radius:14px;padding:14px;box-shadow:0 20px 60px rgba(0,0,0,.35);position:relative}.exam-zoom-card img{width:100%;height:auto;border:1px solid #e2e8f0;border-radius:6px;margin-bottom:8px;position:static!important}.exam-zoom-close{position:sticky;top:0;float:left;border:0;background:#ef4444;color:#fff;border-radius:8px;padding:5px 12px;cursor:pointer;z-index:2}.print-note-box{border:1px solid #e2e8f0;background:#f8fafc;border-radius:8px;padding:8px}.print-note-box textarea{width:100%;min-height:70px;border:1px solid #cbd5e1;border-radius:8px;padding:8px}/* ---------- v4.102.0: مودال ثبت سوال جدید ---------- */
...
@media print{.toolbar,.editor-panel,.bank-panel,.q-actions,.exam-zoom-modal,.q-modal,#uiToast,.img-resize-box{display:none!important}.eq-slot{border:0!important;outline:0!important;min-width:0!important;padding:0!important}.page{margin:0;width:210mm;height:297mm;break-after:page}body{background:white}.qwrap{overflow:hidden}.source-file iframe{height:65mm!important}.drawCanvas{pointer-events:none}}
...
<div class="toolbar" id="mainToolbar">
  ...
  <div class="tb-group tb-actions"><?php if($groupScope!==''): ?><b class="group-design-notice">آزمون مشترک پایه — <?php $noticeClasses=[]; foreach(($ownedClasses??ceg_classes($classGroup)) as $c)$noticeClasses[norm_class_str($c)]=$c; echo clean(implode('، ',array_values($noticeClasses))); ?></b><?php elseif($classGroup && $classGroup['excluded']): ?><b class="independent-design-notice">آزمون مستقل این کلاس — <?php echo clean($exam['class_name']); ?></b><?php endif; ?><button class="btn-save" onclick="saveDesign()">ذخیره طراحی</button><button class="btn-print" onclick="prepareAllAndPrint()">چاپ نهایی</button></div>
</div>
...
<div class="bank-panel collapsed" id="bankPanel">
  ...
  <?php /* v4.114.0: هر دو بخش تاشو — عنوان با شمارنده و قابلیت باز/بسته شدن */ ?>
  <details class="bank-sec" open style="margin-top:10px">
    <summary class="bank-sec-title">آزمون‌های ذخیره‌شده <span class="bank-count" id="designBankCount"></span></summary>
  ...
  <details class="bank-sec" open style="margin-top:8px">
    <summary class="bank-sec-title">سوالات بانک <span class="bank-count" id="bankCount"></span></summary>
    <div id="bankList" style="margin-top:6px"></div>
  </details>
</div>
    ...
    <div class="q-modal-grid">
      <div class="q-form-col">
        ...
        <div class="rich-tools">
          <button type="button" title="درشت" onclick="cmd('bold')"><b>B</b></button><button type="button" title="مورب" onclick="cmd('italic')"><i>I</i></button><button type="button" title="زیرخط" onclick="cmd('underline')"><u>U</u></button>
          <button type="button" title="بزرگ‌تر (متن انتخابی)" onclick="qFontStep(1)">A+</button><button type="button" title="کوچک‌تر (متن انتخابی)" onclick="qFontStep(-1)">A−</button>
          <span class="rt-sep"></span>
          <button type="button" onclick="cmd('justifyRight')">راست</button><button type="button" onclick="cmd('justifyCenter')">وسط</button><button type="button" onclick="cmd('justifyLeft')">چپ</button><button type="button" onclick="cmd('justifyFull')">تراز</button>
          <button type="button" title="راست به چپ (فقط متن انتخابی)" onclick="qDirection('rtl')">ر→چ</button><button type="button" title="چپ به راست (فقط متن انتخابی)" onclick="qDirection('ltr')">L→R</button>
          <span class="rt-sep"></span>
          <button type="button" title="جای خالی نقطه‌چین" onclick="qInsertHTML('<span class=&quot;blank-line&quot;></span>&nbsp;')">جای‌خالی</button>
          <button type="button" title="مربع صحیح/غلط" onclick="qInsertHTML('<span class=&quot;tf-square&quot;></span>&nbsp;')">□</button>
          <button type="button" title="کسر ایرانی (صورت بالا، مخرج پایین)" onclick="qInsertFrac()">کسر</button>
          <button type="button" title="معادله مثل Word" onclick="qOpenEq()">π معادله</button>
          <button type="button" title="نمادها مثل Word" onclick="qOpenSym()">Ω نماد</button>
          <button type="button" onclick="openMathModal()">∑ ریاضی</button>
          <button type="button" onclick="cmd('insertUnorderedList')">لیست</button>
          <button type="button" onclick="openImageEditor()">تصویر</button>
          <span class="rt-sep"></span>
          <button type="button" title="واگرد" onclick="cmd('undo')">↶</button><button type="button" title="ازنو" onclick="cmd('redo')">↷</button>
        </div>
      ...
      <div class="q-preview-col">
        ...
        <div class="q-preview-sheet">
          <table class="questions-table"><thead><tr><th class="q-no">ردیف</th><th class="th-q-title">سوال</th><th class="q-score">بارم</th></tr></thead>
...
<div id="qSymModal" class="math-modal" onclick="if(event.target===this)qCloseSym()">
  <div class="math-card" style="width:min(640px,94vw)">
    ...
    <div class="qsym-tabs" id="qSymTabs"></div>
    <div class="qsym-grid" id="qSymGrid"></div>
  </div>
</div>
<!-- v4.112.0: مودال معادله (مثل Word) برای پنجره سوال -->
<div id="qEqModal" class="math-modal" onclick="if(event.target===this)qCloseEq()">
  <div class="math-card" style="width:min(700px,94vw)">
    <div style="display:flex;justify-content:space-between;align-items:center"><b>معادله ریاضی (Equation)</b><button onclick="qCloseEq()" style="background:#ef4444;color:white;border:0;border-radius:8px;padding:5px 12px">×</button></div>
    <p style="font-size:12px;color:#64748b">یک قالب را انتخاب کنید — کادرهای خط‌چین بعد از درج، در خود سوال قابل تایپ هستند (روی آن کلیک کنید و بنویسید). این کادرها فقط راهنمای ویرایش‌اند و در چاپ دیده نمی‌شوند.</p>
    <div class="qeq-grid" id="qEqGrid"></div>
    <div style="margin-top:10px">
      <label style="font-size:12px;font-weight:700">یا عبارت دلخواه (چپ به راست):</label>
      <div style="display:flex;gap:8px;margin-top:5px">
        <input id="qEqCustom" style="flex:1;direction:ltr;text-align:left;border:1px solid #cbd5e1;border-radius:8px;padding:8px" placeholder="مثلاً: (−۳) × (+۴) =">
        <button class="btn-save" onclick="qInsertEqCustom()" style="border:0;border-radius:8px;padding:8px 14px">درج</button>
      </div>
    </div>
  </div>
</div>
<div id="mathModal" class="math-modal"><div class="math-card"><div style="display:flex;justify-content:space-between;align-items:center"><b>ویرایشگر سریع ریاضی / LaTeX ساده</b><button onclick="closeMathModal()" style="background:#ef4444;color:white;border:0;border-radius:8px;padding:5px 12px">×</button></div><p style="font-size:12px;color:#64748b">فرمول را با دکمه‌ها بسازید یا LaTeX ساده بنویسید؛ پیش‌نمایش به یونیکد/فرمت چاپی تبدیل می‌شود.</p><textarea id="mathInput" style="width:100%;min-height:70px;direction:ltr;text-align:left;border:1px solid #cbd5e1;border-radius:8px;padding:8px" oninput="updateMathPreview()" placeholder="مثلاً: \frac{2}{3}+\sqrt{x^2}+\alpha"></textarea><div class="math-grid" id="mathButtons"></div><div class="math-preview" id="mathPreview"></div><div style="display:flex;gap:8px;justify-content:flex-end;margin-top:10px"><button onclick="insertMathToQuestion()" class="btn-save">درج در نوشته</button><button onclick="closeMathModal()">انصراف</button></div></div></div>
...
let previewOnly=true, mobileEditorZoom=null;
let qItems=[], bankItems=[], drawingsData={}, sourceCrops={}, sourceOrder=[], pageStyle={}, printNote='', selectedImageId=null, selectedImageNode=null, imageTouchTools=null, tool='off', drawing=false, last=null, booting=true, printSavePromptPending=false, canvasTouched={};
...
/* v4.117.0: جمع بارم سوالات برگه — ارقام فارسی و ممیز / پشتیبانی می‌شود */
function totalExamScore(){
  let sum=0;
  qItems.forEach(it=>{
    if(it.type!=='q')return;
    const t=String(it.score??'').trim().replace(/[۰-۹]/g,d=>'۰۱۲۳۴۵۶۷۸۹'.indexOf(d)).replace('/','.').replace('٫','.');
    const n=parseFloat(t);
    if(!isNaN(n))sum+=n;
  });
  return Math.round(sum*100)/100;
}
function formatScoreFa(n){
  let t=String(n);
  if(t.includes('.'))t=t.replace('.','/');
  return t.replace(/[0-9]/g,d=>'۰۱۲۳۴۵۶۷۸۹'[+d]);
}
let lastAutoScore=null;
function syncHeaderScore(){
  const f=headerFields.find(x=>x.key==='score');
  if(!f)return;
  const total=totalExamScore();
  const auto=total>0?formatScoreFa(total):'';
  /* بازنویسی فقط اگر فیلد خالی است یا مقدار فعلی همان جمع خودکار قبلی است (دست‌نخورده) */
  const cur=String(f.value||'').trim();
  if(cur===''||cur===lastAutoScore){
    if(f.value!==auto){f.value=auto; renderHeaders(); renderHeaderEditor();}
  }
  lastAutoScore=auto;
}
function headerHtml(st,pageNo){if(pageNo%2===0)return '';
  /* v4.118.0: کادر فیلد با برچسب کوچک و مقدار وسط‌چین */
  let fields=headerFields.filter(f=>f.on).map(f=>{const v=(f.value!==undefined&&f.value!=='')?f.value:st[f.source]; return `<div class="box"><span class="box-l">${esc(f.label)}</span><span class="box-v">${esc(v!==undefined&&v!==null?v:'')}</span></div>`;}).join('');
...
let sourceFitSigs={};
function applySourceCropFits(allowReflow=true){let changed=false; document.querySelectorAll('.source-file').forEach(sf=>{const img=sf.querySelector('img'); if(!img||!img.complete||!img.naturalWidth)return; /* getBoundingClientRect is already transformed on phones; offsetWidth is the unscaled A4 layout width. */ const w=img.offsetWidth||sf.clientWidth||Math.round(img.getBoundingClientRect().width); if(!w)return; const fullH=w*img.naturalHeight/img.naturalWidth; const t=+sf.dataset.cropt||0,b=+sf.dataset.cropb||0; const visH=Math.max(0,fullH*(100-t-b)/100); img.style.marginTop=(-fullH*t/100)+'px'; sf.style.height=visH+'px'; const key=(sf.closest('.page')?.dataset.studentId||'')+'_'+(sf.dataset.sourcePage||''); const sig=Math.round(visH)+'_'+t+'_'+b; if(sourceFitSigs[key]!==sig){sourceFitSigs[key]=sig; changed=true;}}); if(changed&&allowReflow) scheduleQuestionReflow(); return changed;}
function sourceCropData(page){const r=sourceCrops[page]||{}; return {t:+r.t||0,r:+r.r||0,b:+r.b||0,l:+r.l||0,w:r.w===undefined?100:+r.w||100,x:+r.x||0,y:+r.y||0,contrast:r.contrast===undefined?100:+r.contrast||100,brightness:r.brightness===undefined?100:+r.brightness||100,bg:+r.bg||0};}
...
function questionRow(it){return `<tr class="question-row" draggable="true" data-qid="${it.id}"><td class="q-no">${it.no}</td><td class="q-content" style="height:${it.height}mm;font-family:${it.font}!important;font-size:${it.fontSize}px">${it.html}<div class="q-actions"><button onclick="editQuestion('${it.id}')">ویرایش</button><button class="q-move" onclick="moveQuestion('${it.id}',-1)" title="یک سوال بالاتر">↑ بالا</button><button class="q-move" onclick="moveQuestion('${it.id}',1)" title="یک سوال پایین‌تر">↓ پایین</button><button onclick="deleteQuestion('${it.id}')">حذف</button></div><div class="q-h-grip" title="کشیدن برای تنظیم ارتفاع سوال" onpointerdown="qHeightGripDown(event,'${it.id}')"></div></td><td class="q-score">${it.score}</td></tr>`;}
/* ============ v4.115.0: تنظیم ارتفاع سوال با کشیدن حاشیه پایین ============ */
let qHeightDrag=null;
function qHeightGripDown(e,qid){
  /* v4.116.0: رفع باگ — ردیف سوال draggable است و درگ HTML5 مرورگر رویدادهای pointer را
     لغو می‌کرد (pointercancel) → پیش‌نمایش قفل می‌شد. راه‌حل: pointer capture روی خود
     دستگیره + خاموش‌کردن موقت draggable ردیف در طول کشیدن. */
  e.preventDefault(); e.stopPropagation();
  const it=qItems.find(x=>x.id===qid); if(!it)return;
  const grip=e.currentTarget||e.target;
  const cell=grip.closest('.q-content');
  const row=grip.closest('.question-row');
  if(row)row.setAttribute('draggable','false');
  try{grip.setPointerCapture(e.pointerId);}catch(_){ }
  qHeightDrag={qid,sy:e.clientY,h0:parseFloat(it.height)||18,cell,grip,row,pid:e.pointerId};
  document.body.classList.add('qh-resizing');
  grip.onpointermove=ev=>{
    if(!qHeightDrag)return;
    const dyPx=ev.clientY-qHeightDrag.sy;
    const dyMm=dyPx*25.4/96;               /* px صفحه → mm برگه (رندر ۱:۱) */
    let h=Math.round((qHeightDrag.h0+dyMm)*2)/2;   /* گام نیم میلی‌متر */
    h=Math.max(8,Math.min(230,h));
    qHeightDrag.h=h;
    /* پیش‌نمایش زنده: ارتفاع td و min-height (td کوچک‌تر از محتوا نمی‌شود — min-height پایه را هم می‌بریم) */
    document.querySelectorAll(`[data-qid="${qHeightDrag.qid}"] .q-content`).forEach(c=>{c.style.height=h+'mm'; c.style.minHeight=h+'mm';});
    if(qHeightDrag.grip)qHeightDrag.grip.dataset.hint=h.toLocaleString('fa-IR')+' mm';
  };
  grip.onpointerup=grip.onpointercancel=ev=>{
    if(!qHeightDrag)return;
    const {qid,h,row,grip:g,pid}=qHeightDrag;
    try{g.releasePointerCapture(pid);}catch(_){ }
    g.onpointermove=g.onpointerup=g.onpointercancel=null;
    if(row)row.setAttribute('draggable','true');
    document.body.classList.remove('qh-resizing');
    qHeightDrag=null;
    if(h===undefined)return;
    const it2=qItems.find(x=>x.id===qid); if(!it2)return;
    it2.height=h;
    renderQuestions();                      /* سرریز صفحه دوباره محاسبه شود */
    showToast('ارتفاع سوال: '+h.toLocaleString('fa-IR')+' میلی‌متر');
  };
}
...
function renderQuestions(){normalizeAllQuestionImages();removeImgHandles();collectDrawings();syncHeaderScore();document.getElementById('pagesRoot').innerHTML=''; /* v4.102.0: سوالات فرم فقط بعد از صفحات فایل منبع قرار می‌گیرند */ const srcN=(examData.sourcePages||[]).length?sourcePageCount():0; let page=srcN>0?srcN+1:1; ensurePages(Math.max(1,srcN)); applySourceCropFits(); qItems.forEach(it=>{ if(it.type==='page'){page++; ensurePages(page); return;} ensurePages(page); appendItemToPage(it,page); if(isPageOverflow(page) && questionCountOnPage(page)>1){removeItemFromPage(it.id,page); page++; ensurePages(page); appendItemToPage(it,page);} }); toggleEmptyTables(); enableDrag(); resizeCanvases(); restoreDrawings(); autosaveLocal(); editorViewportScale(); }
...
function toggleEditor(){
  const p=document.getElementById('questionEditorPanel'), bank=document.getElementById('bankPanel');
  const opening=p.classList.contains('collapsed');
  if(opening&&bank)bank.classList.add('collapsed');
...
function closeQuestionModal(){document.getElementById('questionModal').style.display='none'; document.body.classList.remove('modal-open'); document.getElementById('editingQid').value=''; deselectSelectedImage();}
...
document.addEventListener('keydown',e=>{
  if(e.key==='Escape'){
    const qs=document.getElementById('qSymModal'); if(qs&&qs.style.display==='flex'){qCloseSym();return;}
    const qe=document.getElementById('qEqModal'); if(qe&&qe.style.display==='flex'){qCloseEq();return;}
...
function qRestoreSel(){const ed=document.getElementById('qText'); ed.focus(); if(qSavedRange&&ed.contains(qSavedRange.commonAncestorContainer)){const sel=getSelection(); sel.removeAllRanges(); sel.addRange(qSavedRange);}}
function qInsertHTML(html){qRestoreSel(); document.execCommand('insertHTML',false,html); updateQuestionPreview();}
...
function qDirection(dir){const ed=document.getElementById('qText'); qRestoreSel(); const sel=getSelection();
  if(sel.rangeCount&&!sel.isCollapsed&&ed.contains(sel.anchorNode)&&ed.contains(sel.focusNode)){
    ...
    span.setAttribute('dir',dir); span.style.direction=dir; span.style.unicodeBidi='isolate';
    const frag=range.extractContents(); span.appendChild(frag); range.insertNode(span);
    span.querySelectorAll('span[dir]').forEach(x=>{x.removeAttribute('dir'); if(x.style){x.style.unicodeBidi='';x.style.direction='';} if(x.attributes.length===0&&!x.getAttribute('style'))x.replaceWith(...x.childNodes);});
    const nr=document.createRange(); nr.selectNodeContents(span); sel.removeAllRanges(); sel.addRange(nr); qSavedRange=nr.cloneRange();
  }else{
    const existing=ed.children.length===1&&ed.firstElementChild.classList.contains('q-direction-block')?ed.firstElementChild:null;
    const block=existing||document.createElement('div');
    if(!existing){block.className='q-direction-block';block.innerHTML=ed.innerHTML;ed.innerHTML='';ed.appendChild(block);}
    block.setAttribute('dir',dir); block.style.direction=dir; block.style.unicodeBidi='isolate'; block.style.textAlign=dir==='ltr'?'left':'right';
    ed.setAttribute('dir',dir); ed.style.direction=dir; ed.style.textAlign=dir==='ltr'?'left':'right';
  }
  updateQuestionPreview();}
function qInsertFrac(){const num=prompt('صورت کسر:','۱'); if(num===null)return; const den=prompt('مخرج کسر:','۲'); if(den===null)return;
...
const qSymTabs={
 ...
 'عمومی':['«','»','،','؛','؟','…','—','–','٪','※','†','‡','•','◦','✓','✗','☑','☐','★','☆','♦','♠','♥','♣']};
let qSymCat='ریاضی';
function qOpenSym(){qRestoreSel(); const tabs=document.getElementById('qSymTabs'); tabs.innerHTML='';
  Object.keys(qSymTabs).forEach(cat=>{const b=document.createElement('button'); b.type='button'; b.className='qsym-tab'+(cat===qSymCat?' active':''); b.textContent=cat;
    b.onclick=()=>{qSymCat=cat;qRenderSym();document.querySelectorAll('.qsym-tab').forEach(x=>x.classList.toggle('active',x.textContent===cat));}; tabs.appendChild(b);});
  qRenderSym(); document.getElementById('qSymModal').style.display='flex';}
function qRenderSym(){const grid=document.getElementById('qSymGrid'); grid.innerHTML='';
  (qSymTabs[qSymCat]||[]).forEach(sy=>{const b=document.createElement('button'); b.type='button'; b.className='qsym-btn'; b.textContent=sy;
    b.onclick=()=>{qInsertHTML(sy.replace(/&/g,'&amp;').replace(/</g,'&lt;'));}; grid.appendChild(b);});}
function qCloseSym(){document.getElementById('qSymModal').style.display='none';}
/* --- معادله (مثل Word) --- */
const Q_EQ_BOX='<span class="eq-slot">&#8203;</span>';
const Q_FR='<span class="pfrac" style="display:inline-block;vertical-align:middle;text-align:center;margin:0 2px"><span style="display:block;padding:0 4px;border-bottom:1.2px solid currentColor;line-height:1.15">'+Q_EQ_BOX+'</span><span style="display:block;padding:0 4px;line-height:1.15">'+Q_EQ_BOX+'</span></span>';
const qEqTemplates=[
 ['کسر ساده', Q_FR],
 ['جمع دو کسر', Q_FR+' + '+Q_FR+' = '+Q_EQ_BOX],
 ['توان', Q_EQ_BOX+'<sup style="font-size:.72em">'+Q_EQ_BOX+'</sup>'],
 ['اندیس', Q_EQ_BOX+'<sub style="font-size:.72em">'+Q_EQ_BOX+'</sub>'],
 ['رادیکال', '√<span style="border-top:1.2px solid currentColor;padding:0 3px">'+Q_EQ_BOX+'</span>'],
 ['رادیکال با فرجه', '<sup style="font-size:.7em">'+Q_EQ_BOX+'</sup>√<span style="border-top:1.2px solid currentColor;padding:0 3px">'+Q_EQ_BOX+'</span>'],
 ['قدر مطلق', '|'+Q_EQ_BOX+'|'],
 ['پرانتز عملیات', '('+Q_EQ_BOX+') × ('+Q_EQ_BOX+') ='],
 ['جمع علامت‌دار', '(−'+Q_EQ_BOX+') + (+'+Q_EQ_BOX+') ='],
 ['معادله خطی', Q_EQ_BOX+' x + '+Q_EQ_BOX+' = '+Q_EQ_BOX],
 ['نسبت و تناسب', Q_FR+' = '+Q_FR],
 ['مجموعه', '{ '+Q_EQ_BOX+' , '+Q_EQ_BOX+' , '+Q_EQ_BOX+' }'],
 ['زاویه', '∠'+Q_EQ_BOX+' = '+Q_EQ_BOX+'°'],
 ['درصد', Q_EQ_BOX+' ٪ × '+Q_EQ_BOX+' = '],
];
function qOpenEq(){qRestoreSel(); const grid=document.getElementById('qEqGrid');
  if(!grid.dataset.built){qEqTemplates.forEach(([name,html])=>{const card=document.createElement('button'); card.type='button'; card.className='qeq-card';
    card.innerHTML='<span dir="ltr" style="unicode-bidi:isolate;pointer-events:none">'+html+'</span><small>'+name+'</small>';
    card.onclick=()=>{qInsertHTML('<span dir="ltr" style="unicode-bidi:isolate">'+html+'</span>&nbsp;'); qCloseEq();}; grid.appendChild(card);});
    grid.dataset.built='1';}
  document.getElementById('qEqModal').style.display='flex';}
function qInsertEqCustom(){const v=document.getElementById('qEqCustom').value.trim(); if(!v)return;
  qInsertHTML('<span dir="ltr" style="unicode-bidi:isolate">'+v.replace(/&/g,'&amp;').replace(/</g,'&lt;')+'</span>&nbsp;');
  document.getElementById('qEqCustom').value=''; qCloseEq();}
function qCloseEq(){document.getElementById('qEqModal').style.display='none';}
/* کادر eq-slot فقط راهنمای ویرایش — هنگام ذخیره سوال حذف می‌شود */
function qStripEqSlots(html){const tmp=document.createElement('div'); tmp.innerHTML=html;
  tmp.querySelectorAll('.eq-slot').forEach(x=>{x.classList.remove('eq-slot'); if(!x.getAttribute('class'))x.removeAttribute('class');
    const txt=x.textContent.replace(/\u200b/g,'').trim();
    if(txt==='')x.remove(); else if(x.attributes.length===0)x.replaceWith(...x.childNodes);});
...
function nextQuestionImageId(){return 'img_'+Date.now()+'_'+Math.random().toString(16).slice(2);}
function normalizeQuestionHtml(html,seen){
  const box=document.createElement('div'); box.innerHTML=String(html||'');
  box.querySelectorAll('.img-resize-box').forEach(x=>x.remove());
  const used=seen||new Set();
  box.querySelectorAll('img').forEach(img=>{
    let id=img.dataset.imgid||'';
    if(!id||used.has(id)){id=nextQuestionImageId(); while(used.has(id))id=nextQuestionImageId();}
    used.add(id); img.dataset.imgid=id; img.setAttribute('draggable','false'); img.classList.remove('selected-img');
    img.setAttribute('onclick',`selectQuestionImage('${id}',this)`);
  });
...
function decorateQuestionImages(root){
  ...
  images.forEach(img=>{
    if(!img.dataset.imgid)img.dataset.imgid=nextQuestionImageId();
    img.setAttribute('draggable','false');
    img.setAttribute('onclick',`selectQuestionImage('${img.dataset.imgid}',this)`);
    img.onclick=e=>{e.preventDefault();e.stopPropagation();selectQuestionImage(img.dataset.imgid,img);};
  });
}
function questionImageNodes(id){return [...document.querySelectorAll('img[data-imgid]')].filter(x=>String(x.dataset.imgid)===String(id));}
function currentSelectedImage(){
  const nodes=questionImageNodes(selectedImageId);
...
function ensureImageTouchTools(){
  ...
  box.innerHTML='<span id="mobileImageToolsLabel" class="mobile-image-tools-label">تصویر انتخاب نشده</span>'+
    '<button type="button" onclick="moveSelectedImage(-1,0)" aria-label="حرکت به چپ">←</button>'+
    '<button type="button" onclick="moveSelectedImage(1,0)" aria-label="حرکت به راست">→</button>'+
    '<button type="button" onclick="moveSelectedImage(0,-1)" aria-label="حرکت به بالا">↑</button>'+
    '<button type="button" onclick="moveSelectedImage(0,1)" aria-label="حرکت به پایین">↓</button>'+
    '<button type="button" onclick="resizeSelectedImageBy(-12)" aria-label="کوچک کردن تصویر">− اندازه</button>'+
    '<button type="button" onclick="resizeSelectedImageBy(12)" aria-label="بزرگ کردن تصویر">+ اندازه</button>'+
    '<button type="button" class="mobile-image-tools-edit" onclick="openImageEditor()">ویرایش تصویر</button>'+
    '<button type="button" class="mobile-image-tools-close" onclick="deselectSelectedImage()" aria-label="بستن ابزار تصویر">×</button>';
  document.body.appendChild(box); imageTouchTools=box; return box;
}
...
function deselectSelectedImage(){removeImgHandles();document.querySelectorAll('.selected-img').forEach(i=>i.classList.remove('selected-img'));selectedImageId=null;selectedImageNode=null;hideImageTouchTools();}
function commitImageChange(img){
  const ed=document.getElementById('qText');
  if(ed&&img&&ed.contains(img)){updateModelFromDomImage();updateQuestionPreview();}
  else syncRenderedImagesToModel();
}
function insertQuestionImage(input){[...(input.files||[])].forEach(file=>{compressImageFile(file).then(src=>{const id=nextQuestionImageId(); const ed=document.getElementById('qText'); ed.focus(); qRestoreSel(); document.execCommand('insertHTML',false,`<img src="${src}" data-imgid="${id}" draggable="false" style="width:220px;height:auto;position:absolute;left:0px;top:0px" onclick="selectQuestionImage('${id}',this)">`); decorateQuestionImages(ed); const inserted=questionImageNodes(id).find(x=>ed.contains(x)); if(inserted)selectQuestionImage(id,inserted); updateQuestionPreview();}).catch(msg=>alert(msg));}); input.value='';}
function selectQuestionImage(id,img){
  if(!id&&img)id=img.dataset.imgid;
  if(!id)return;
...
function resizeSelectedImage(w){
  ...
  const img=currentSelectedImage(); const current=img?(img.offsetWidth||parseFloat(img.style.width)||220):220;
  const width=Math.max(30,Math.min(760,Math.round(+w||current)));
  questionImageNodes(selectedImageId).forEach(i=>{i.style.width=width+'px';i.style.height='auto';});
  if(img)positionImgHandles(img); commitImageChange(img);
}
function resizeSelectedImageBy(delta){const img=currentSelectedImage();if(!img)return;resizeSelectedImage((img.offsetWidth||parseFloat(img.style.width)||220)+delta);}
function moveSelectedImage(dx,dy){
  const img=currentSelectedImage();if(!img)return; const scale=imageLayoutScale(img);
  const left=(parseFloat(img.style.left)||0)+dx/Math.max(.1,scale), top=(parseFloat(img.style.top)||0)+dy/Math.max(.1,scale);
  questionImageNodes(selectedImageId).forEach(i=>{i.style.left=Math.round(left)+'px';i.style.top=Math.round(top)+'px';});
  positionImgHandles(img); commitImageChange(img);
}
/* ============ v4.161.0: دستگیره‌های کشیدنی تغییر اندازه تصویر روی خود برگه ============ */
let imgHandleBox=null, imgResizeState=null, imgDragState=null;
function removeImgHandles(){if(imgHandleBox){imgHandleBox.remove(); imgHandleBox=null;} imgResizeState=null; hideImageTouchTools();}
function attachImgHandles(img){
  ...
  (mobile?['nw','ne','se','sw']:['nw','n','ne','e','se','s','sw','w']).forEach(dir=>{
    ...
    h.addEventListener('pointerdown',e=>{
      e.preventDefault(); e.stopPropagation();
      const r=img.getBoundingClientRect(), scale=imageLayoutScale(img);
      imgResizeState={img,dir,sx:e.clientX,sy:e.clientY,w:Math.max(1,img.offsetWidth||r.width/scale),h:Math.max(1,img.offsetHeight||r.height/scale),
                      l:parseFloat(img.style.left||0),t:parseFloat(img.style.top||0),ratio:(img.offsetWidth||r.width/scale)/Math.max(1,img.offsetHeight||r.height/scale),scale,pointerId:e.pointerId};
      try{h.setPointerCapture(e.pointerId);}catch(_){ }
      document.body.classList.add('img-resizing');
    });
    box.appendChild(h);
  });
  host.appendChild(box); imgHandleBox=box; positionImgHandles(img);
}
...
document.addEventListener('pointermove',e=>{
  ...
  let w=st.w, h=st.h, l=st.l, t=st.t, dir=st.dir;
  if(dir==='e'){w=st.w+dx;}
  else if(dir==='w'){w=st.w-dx; l=st.l+dx;}
  else if(dir==='s'){h=st.h+dy; w=h*st.ratio;}
  else if(dir==='n'){h=st.h-dy; w=h*st.ratio; t=st.t+dy;}
  else{const sx=(dir.includes('w')?-1:1), sy=(dir.includes('n')?-1:1), dw=dx*sx, dh=dy*sy; w=st.w+(Math.abs(dw)>=Math.abs(dh*st.ratio)?dw:dh*st.ratio); h=w/st.ratio; if(dir.includes('w'))l=st.l+(st.w-w); if(dir.includes('n'))t=st.t+(st.h-h);}
  w=Math.max(30,Math.min(760,w));
  questionImageNodes(st.img.dataset.imgid).forEach(i=>{i.style.width=Math.round(w)+'px';i.style.height='auto';i.style.left=Math.round(l)+'px';i.style.top=Math.round(t)+'px';});
  positionImgHandles(st.img);
});
function finishImageResize(){if(!imgResizeState)return; document.body.classList.remove('img-resizing'); const img=imgResizeState.img; imgResizeState=null; if(img&&img.isConnected){positionImgHandles(img);commitImageChange(img);}}
document.addEventListener('pointerup',finishImageResize);document.addEventListener('pointercancel',finishImageResize);
/* کلیک خارج از تصویر: لغو انتخاب و حذف دستگیره‌ها */
document.addEventListener('pointerdown',e=>{if(e.target.closest&&(e.target.closest('.img-handle')||e.target.closest('img[data-imgid]')||e.target.closest('.mobile-image-tools')||e.target.closest('.rich-tools')))return;if(imgHandleBox)deselectSelectedImage();});
function updateModelFromDomImage(){const edit=document.getElementById('editingQid').value; const ed=document.getElementById('qText'); if(edit&&ed){const it=qItems.find(x=>x.id===edit); if(it)it.html=normalizeQuestionHtml(ed.innerHTML);}}
...
function saveQuestion(){const edit=document.getElementById('editingQid').value; decorateQuestionImages(document.getElementById('qText')); const no=document.getElementById('qNumber').value||'1', score=document.getElementById('qScore').value||'', html=qStripEqSlots(normalizeQuestionHtml(document.getElementById('qText').innerHTML.trim())), height=document.getElementById('qHeight').value||18, font=document.getElementById('qFont').value, fontSize=document.getElementById('qFontSize').value||11, qtype=document.getElementById('qType').value; if(!html){alert('متن سوال را وارد کنید.');return;} if(edit){const it=qItems.find(x=>x.id===edit); if(!it){document.getElementById('editingQid').value=''; qItems.push({type:'q',id:'q_'+Date.now(),qtype,no,score,html,height,font,fontSize});} else Object.assign(it,{no,score,html,height,font,fontSize,qtype});} else qItems.push({type:'q',id:'q_'+Date.now(),qtype,no,score,html,height,font,fontSize}); renderQuestions(); clearQuestionForm(); autosaveLocal(); closeQuestionModal(); showToast(edit?'سوال ویرایش و در برگه به‌روزرسانی شد':'سوال در آزمون درج شد');}
function editQuestion(id){const it=qItems.find(x=>x.id===id); if(!it)return; deselectSelectedImage(); document.getElementById('editingQid').value=id; document.getElementById('qNumber').value=it.no; document.getElementById('qScore').value=it.score; document.getElementById('qText').innerHTML=normalizeQuestionHtml(it.html); decorateQuestionImages(document.getElementById('qText')); document.getElementById('qHeight').value=it.height; document.getElementById('qFont').value=it.font; document.getElementById('qFontSize').value=it.fontSize; document.getElementById('qModalTitle').textContent='ویرایش سوال'; document.getElementById('qInsertBtn').textContent='ذخیره تغییرات'; applyFontToEditor(); openQuestionModal(id);}
...
/* Touch-safe ordering: the saved qItems order remains the single source of truth
   for desktop drag, mobile buttons, print and the question bank copy. */
function moveQuestion(id,delta){
  const positions=qItems.map((x,i)=>x.type==='q'?i:-1).filter(i=>i>=0);
  const at=positions.indexOf(qItems.findIndex(x=>x.id===id)); if(at<0)return;
  const to=at+delta; if(to<0||to>=positions.length)return;
  const a=positions[at], b=positions[to], tmp=qItems[a]; qItems[a]=qItems[b]; qItems[b]=tmp;
  renderQuestions(); autosaveLocal(); showToast(delta<0?'سوال یک ردیف بالاتر رفت':'سوال یک ردیف پایین‌تر رفت');
}
...
function enableImageDrag(){
  const imgs=[...document.querySelectorAll('.q-content img,#qText img')]; imgs.forEach(img=>{decorateQuestionImages(img);img.onpointerdown=e=>{if(imgResizeState||e.button&&e.button!==0)return;selectQuestionImage(img.dataset.imgid,img);imgDragState={img,sx:e.clientX,sy:e.clientY,l:parseFloat(img.style.left||0),t:parseFloat(img.style.top||0),scale:imageLayoutScale(img),pointerId:e.pointerId};try{img.setPointerCapture(e.pointerId);}catch(_){ }e.preventDefault();};});
}
...
function setMobilePageZoom(delta){
  const root=document.getElementById('pagesRoot'); if(!root)return;
  const current=parseFloat(getComputedStyle(root).getPropertyValue('--editorScale'))||.44;
  mobileEditorZoom=Math.max(.38,Math.min(1.15,current+delta)); editorViewportScale();
}
function fitMobilePage(){mobileEditorZoom=null; editorViewportScale();}
function installMobileEditorChrome(){
  if(document.getElementById('mobileEditorDock'))return;
  ...
  dock.innerHTML='<button type="button" data-mobile-control="bank" onclick="toggleBank()" aria-expanded="false">بانک<br>سوال</button>'+
    ...
    '<output id="mobilePageZoom" aria-live="polite" onclick="fitMobilePage()" title="اندازه مناسب صفحه">—</output>'+
...
function beginMobileRangePreview(e){const t=e.target;if(!window.matchMedia||!window.matchMedia('(max-width:860px)').matches||!t?.matches?.('input[type="range"]'))return;if(!t.closest('.editor-panel,.image-controls'))return;mobileRangePointer=e.pointerId;mobileRangeTarget=t;document.body.classList.add('mobile-range-preview');t.setPointerCapture?.(e.pointerId);}
function endMobileRangePreview(e){if(mobileRangePointer===null||e.pointerId===mobileRangePointer){const t=mobileRangeTarget;mobileRangePointer=null;mobileRangeTarget=null;document.body.classList.remove('mobile-range-preview');if(t?.closest('.source-adjustments')){const p=+(document.getElementById('cropPage')?.value||1);sourceCropApply(p,true);}}}
document.addEventListener('pointerdown',beginMobileRangePreview,true);document.addEventListener('pointerup',endMobileRangePreview,true);document.addEventListener('pointercancel',endMobileRangePreview,true);
...
function renderBankPage(){
  ...
  items.forEach(b=>{
    ...
    if(b.academic_year)chips.push(`<span class="bi-chip">${esc(b.academic_year)}</span>`);
...
function measureBankQuestionHeightMM(html,fontFamily,fontSizePx){
  ...
  probe.style.cssText='position:absolute;visibility:hidden;left:-9999px;top:0;width:172mm;padding:4px;box-sizing:border-box;font-family:'+fontFamily+';font-size:'+fontSizePx+'px;line-height:1.7';
  probe.innerHTML=html;
  document.body.appendChild(probe);
  /* تصاویر position:absolute در جریان نیستند — پایین‌ترین لبه آن‌ها را جدا حساب کن */
  let extra=0;
  probe.querySelectorAll('img').forEach(im=>{
    const top=parseFloat(im.style.top||0), w=parseFloat(im.style.width||220);
    const nw=im.naturalWidth||w, nh=im.naturalHeight||(w*0.7);
    const h=w*(nh/Math.max(1,nw));
    extra=Math.max(extra,top+h);
  });
  const px=Math.max(probe.scrollHeight,extra);
  probe.remove();
  const mm=Math.ceil(px*25.4/96)+3;           /* +۳mm حاشیه اطمینان */
  return Math.max(12,Math.min(220,mm));
}
function insertBankQuestion(id){const b=(bankItems||[]).find(x=>+x.id===+id); if(!b)return; /* v4.102.0: شماره ردیف بعدی به‌صورت خودکار */ let mx=0; qItems.forEach(it=>{if(it.type!=='q')return; const v=parseInt(String(it.no??'').replace(/[۰-۹]/g,d=>String('۰۱۲۳۴۵۶۷۸۹'.indexOf(d))),10); if(!isNaN(v)&&v>mx)mx=v;});
  /* v4.114.0: فرمت پیش‌فرض درج از بانک — فونت تیتر، اندازه ۱۴، ارتفاع به‌اندازه کل محتوا */
  ...
  qItems.push({type:'q',id:'q_'+Date.now(),bank_id:b.id,qtype:b.question_type||'text',no:mx+1,score:b.score||'',html:normalizeQuestionHtml(b.question_html),height,font,fontSize}); renderQuestions(); showToast('سوال از بانک درج شد');}
...
function waitForPrintReady(cb,maxMs){
  // v4.77.0: never fire appPrint() on a half-rendered DOM (the old fixed
...
function startQualityPrintChecked(){
  ...
  buildPrintAssets(q,(d,t)=>pqProgress('سبک‌سازی تصاویر... ('+d+' از '+t+')',t?Math.round(d/t*40):40),assetMap=>{
    ...
    prepareAllStudentsAsync(assetMap,(d,t)=>pqProgress('ساخت صفحات چاپ... ('+Math.min(d*perStudent,totalPages)+' از '+totalPages+' صفحه)',40+Math.round(d/Math.max(1,t)*45)),()=>{
      ...
      waitForPrintReady(()=>{pqProgress('ارسال به چاپگر...',100); restore(); closePrintQualityModal(); checkClassGroupScope().then(ok=>{if(ok)appPrint();});}, Math.min(120000, 15000+totalPages*150));
    });
  });
}
...
let imageEditOriginal=null, imageEditTarget=null, imageEditLoadSeq=0;
function imageEditorControl(id){return document.getElementById(id);}
function imageEditorDefaults(){return {cropL:0,cropR:0,cropT:0,cropB:0,outW:420,bright:0,contrast:0,removeBg:false,bgThreshold:238};}
...
function insertEditedImageToQuestion(){
  ...
  const id=nextQuestionImageId(),ed=document.getElementById('qText');ed.focus();qRestoreSel();document.execCommand('insertHTML',false,`<img src="${src}" data-imgid="${id}" draggable="false" style="width:${Math.min(420,c.width)}px;height:auto;position:absolute;left:0px;top:0px" onclick="selectQuestionImage('${id}',this)">`);decorateQuestionImages(ed);const inserted=questionImageNodes(id).find(x=>ed.contains(x));if(inserted){if(!imageEditorSettingsAreDefault(settings))inserted.dataset.imageSource=imageEditOriginal.src;inserted.dataset.imageSettings=JSON.stringify(settings);selectQuestionImage(id,inserted);}updateQuestionPreview();closeImageEditor();
}
...
window.addEventListener('resize',resizeCanvases);window.addEventListener('resize',editorViewportScale);window.visualViewport?.addEventListener('resize',editorViewportScale);installMobileEditorChrome();renderHeaderEditor();initCropPages();rerenderPages();applyQuestionTemplate();loadDesignAndBank();setTimeout(()=>{resizeCanvases();editorViewportScale();},300);
</script></body></html><?php exit; }
