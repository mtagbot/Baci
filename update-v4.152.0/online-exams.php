...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if (isset($_POST['delete_exam'])) {
        ...
        /* v4.131.0 — حذف آزمون دیگر رکورد یتیم باقی نمی‌گذارد.
           پیش از این فقط online_exams و online_questions پاک می‌شد و
           attemptها، پاسخ‌ها، لاگ‌های نظارت، سشن‌های زنده، اسنپ‌شات‌های
           وب‌کم و فایل‌های آپلودیِ دانش‌آموزان تا ابد در دیتابیس و روی
           دیسک می‌ماندند.
           برای هر attempt از همان online_exam_delete_attempt_full()
           استفاده می‌شود که v4.125.0 نوشت — هم رکوردها و هم فایل‌ها را
           برمی‌دارد و مسیرها را در برابر traversal می‌سنجد. */
        $delAttempts = 0; $delFiles = 0;
        try {
            $attempts = DB::fetchAll("SELECT id FROM online_exam_attempts WHERE exam_id=?", [$eid]);
            foreach ($attempts as $a) {
                $r = online_exam_delete_attempt_full((int)$a['id']);
                if (!empty($r['ok'])) { $delAttempts++; $delFiles += (int)($r['files'] ?? 0); }
            }
        } catch (Exception $e) { error_log('delete_exam attempts: '.$e->getMessage()); }

        try {
            /* جدول‌هایی که به exam_id وصل‌اند (نه به attempt) */
            foreach (['online_exam_proctoring_logs','online_exam_live_sessions',
                      'online_exam_webcam_snapshots','online_exam_voice_notes'] as $t) {
                try { DB::execute("DELETE FROM `$t` WHERE exam_id=?", [$eid]); } catch (Exception $e) {}
            }
            DB::execute("DELETE FROM online_questions WHERE exam_id=?", [$eid]);
            DB::execute("DELETE FROM online_exams WHERE id=?", [$eid]);
        } catch (Exception $e) { error_log($e->getMessage()); }

        log_activity($_SESSION['admin_id'] ?? null, 'حذف آزمون آنلاین',
            "آزمون #$eid حذف شد (شامل $delAttempts شرکت‌کننده و $delFiles فایل).");
        set_flash_message('success', $delAttempts
            ? "آزمون حذف شد (به‌همراه $delAttempts شرکت‌کننده و $delFiles فایل)."
            : 'آزمون حذف شد');
            ...
            <div class="flex justify-between items-start">
                <div class="flex-1">
                    <div class="flex items-center gap-2 flex-wrap">
                        ...
                        <?php if($isLive): ?><span class="badge bg-red-500 text-white animate-pulse"><svg data-ui-icon="network" class="school-icon" viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M2 8c6-6 14-6 20 0M5 12c4-4 10-4 14 0M8 16c2-2 6-2 8 0M12 20h.1"/></svg> زنده: <?php echo tr_num($liveCount['c'],'fa'); ?> نفر در حال آزمون</span><?php endif; ?>
                ...
                <div class="flex flex-col gap-1 ml-4">
                    <a href="online-exam-form.php?id=<?php echo $ex['id']; ?>" class="btn btn-secondary text-xs">ویرایش</a>
                    ...
                    <form method="POST" class="inline" onsubmit="return confirm('تغییر وضعیت؟')">
                        ...
                        <button name="toggle_status" class="btn btn-warning text-xs w-full"><?php echo $ex['status']=='published'?'انتقال به پیش‌نویس':'انتشار'; ?></button>
                    </form>
                    <form method="POST" onsubmit="return confirm('حذف آزمون؟ این عمل قابل بازگشت نیست')">
                        <input type="hidden" name="csrf_token" value="<?php echo csrf_token(); ?>">
                        <input type="hidden" name="exam_id" value="<?php echo $ex['id']; ?>">
                        <button name="delete_exam" class="btn btn-danger text-xs w-full">حذف</button>
                    </form>
                </div>
            </div>
        </div>
