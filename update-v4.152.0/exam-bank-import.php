...
/* v4.111.0: ستون پایه تحصیلی در بانک سوالات (idempotent) */
try { DB::execute("ALTER TABLE exam_question_bank ADD COLUMN grade_level varchar(50) DEFAULT NULL"); } catch (Exception $e) {}

/* v4.111.0: شکست خاموش ممنوع — POST بزرگ‌تر از post_max_size باعث $_POST خالی می‌شود */
if ($_SERVER['REQUEST_METHOD'] === 'POST' && empty($_POST) && (int)($_SERVER['CONTENT_LENGTH'] ?? 0) > 0) {
    $importErr = 'حجم درخواست از سقف سرور (post_max_size = ' . ini_get('post_max_size') . ') بیشتر بود و هیچ داده‌ای ذخیره نشد. این نسخه حجم ارسال را به‌شدت کاهش داده — دوباره تلاش کنید.';
}
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST) && !verify_csrf($_POST['csrf_token'] ?? '')) {
    $importErr = 'نشست شما منقضی شده است — صفحه را رفرش کنید و دوباره تلاش کنید. (هیچ سوالی ذخیره نشد)';
}
if ($_SERVER['REQUEST_METHOD'] === 'POST' && verify_csrf($_POST['csrf_token'] ?? '')) {
    ...
    if ($act === 'preview') {
        ...
        if ($tok === '' || !is_file($pf)) { $importErr = 'نشست ایمپورت منقضی شده — دوباره فایل را انتخاب کنید.'; }
            ...
            /* v4.111.0: سوالات ویرایش‌شده — HTML فقط برای موارد تغییرکرده می‌آید؛
               بقیه از نسخه pending سرور (بر اساس idx) خوانده می‌شوند */
            ...
            if ($editedRaw !== '') {
                ...
                if (is_array($edited) && count($edited)) {
                    ...
                    foreach ($edited as $eq) {
                        if (!empty($eq['deleted'])) continue;
                        $idx = isset($eq['idx']) ? (int)$eq['idx'] : -1;
                        $h = trim((string)($eq['question_html'] ?? ''));
                        if ($h === '' && $idx >= 0 && isset($parsed['questions'][$idx])) {
                            $h = (string)$parsed['questions'][$idx]['question_html'];
                        }
                        if ($h === '') continue;
                        $cleanEd[] = [
            ...
            $gradeLevel = trim($_POST['grade_level'] ?? '') ?: trim((string)($parsed['grade_level'] ?? ''));
            if ($year === '' || $month === '') $importErr = 'سال تحصیلی و ماه آزمون را انتخاب کنید.';
                ...
                foreach ($parsed['questions'] as $q) {
                    ...
                    $meta = ['no' => $q['no'], 'height' => $q['height'], 'import_batch' => $batch,
                             'source_pdf' => $parsed['source_pdf'], 'grade_level' => $gradeLevel,
                             'image_files' => $imgFiles];
                    DB::execute('INSERT INTO exam_question_bank (source_exam_id, academic_year, exam_month, subject_name, teacher_id, designer_name, question_type, question_html, score, grade_level, meta_json, created_at_jalali) VALUES (NULL,?,?,?,?,?,?,?,?,?,?,?)',
                        [$year, $month, $subject, $teacherId, $designer, $q['question_type'], $html, $q['score'], $gradeLevel, json_encode($meta, JSON_UNESCAPED_UNICODE), jalali_now()]);
                    $importedCount++;
                }
                @unlink($pf);
                set_flash_message('success', 'ایمپورت انجام شد: ' . tr_num((string)$importedCount, 'fa') . ' سوال درس «' . $subject . '» پایه «' . $gradeLevel . '» به بانک سوالات اضافه شد (' . $year . ' / ' . $month . '). برای مشاهده: همین صفحه بانک سوالات، ستون «بانک سوالات طراحی‌شده».');
...
<div class="space-y-4">
 ...
 <div class="page-hero flex justify-between items-center flex-wrap gap-3">
    <div><h2 class="text-2xl font-bold"><svg data-ui-icon="download" class="school-icon" viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M3 16v5h18v-5M12 3v13m-5-5 5 5 5-5"/></svg> ایمپورت سوالات آزمون به بانک</h2><p class="text-sm text-muted">بارگذاری فایل ایمپورت تحلیل‌شده از PDF نمونه سوالات و ثبت سوالات (متن، گزینه‌ها و تصاویر) در بانک اطلاعاتی</p></div>
 ...
 <div class="card">
   ...
   <form method="POST" class="space-y-3">
     ...
     <div class="grid grid-cols-4 gap-3">
       ...
       <div><label class="text-xs font-bold">پایه تحصیلی</label>
         <select name="grade_level" class="form-select" required>
           <?php $pdGrade = trim((string)($previewData['grade_level'] ?? '')); ?>
           <?php foreach (['هفتم','هشتم','نهم'] as $gl): ?>
           <option value="<?php echo $gl; ?>" <?php echo $gl===$pdGrade?'selected':''; ?>><?php echo $gl; ?></option>
           <?php endforeach; ?>
         </select></div>
   ...
   <div class="imp-grid" id="impGrid">
     ...
     <div class="imp-q" data-idx="<?php echo $iqIdx; ?>">
       ...
       <div class="imp-tools">
         ...
         <span class="imp-sep"></span>
         <label class="imp-tb" title="بارگذاری تصویر در سوال" style="cursor:pointer"><svg data-ui-icon="image" class="school-icon" viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><rect x="2" y="3" width="20" height="18" rx="2"/><circle cx="8" cy="8" r="2"/><path d="m3 18 5-5 4 4 5-7 5 8"/></svg> تصویر<input type="file" accept="image/*" style="display:none" onchange="impUploadImage(this)"></label>
         <button type="button" class="imp-tb" onclick="impOpenCropper(this)"><svg data-ui-icon="crop" class="school-icon" viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M6 2v16h16M2 6h16v16M3 21 21 3"/></svg> برش تصویر</button>
...
function impCollectEdits(form){
  ...
  document.querySelectorAll('.imp-q').forEach((q,i)=>{
    ...
    /* v4.111.0: کاهش شدید حجم POST — HTML (شامل تصاویر base64) فقط وقتی واقعاً
       تغییر کرده ارسال می‌شود؛ سوالات دست‌نخورده از نسخه سرور (pending) خوانده می‌شوند */
    const init=impInitial[i]||{};
    const htmlChanged = tmp.innerHTML !== (init.html||'');
    const item={
      idx:i,
    ...
    if(htmlChanged) item.question_html=tmp.innerHTML;
    out.push(item);
  });
