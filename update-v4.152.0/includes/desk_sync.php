...
class DeskSync {
    ...
    private static function applyChange($c) {
        ...
        $row  = $c['row'];
        if(in_array($tbl,['class_exam_groups','class_exam_group_members'],true)){require_once __DIR__.'/class_exam_groups.php';if(ceg_sync_conflict($tbl,$row))throw new RuntimeException('تعارض گروه آزمون سایت و دسکتاپ؛ گروه‌های هم‌زمان خودکار ادغام یا حذف نمی‌شوند.');}
     ...
     */
    public static function remoteExamPdfPages($examId, $relPath, $declaredPages = 0, $selectedPages = []) {
        ...
        try {
            ...
            $res = self::call('exam_pages', ['exam_id' => $examId, 'declared_pages' => (int)$declaredPages, 'path' => $rel,
                'selected_pages' => implode(',', array_map('intval', (array)$selectedPages))]); /* v4.101.0 */
            $pages = [];
    ...
    private static function fileSyncTargets() {
        ...
        try {
            foreach (DB::fetchAll("SELECT id, question_file FROM exam_schedules WHERE question_file IS NOT NULL AND question_file != ''") as $r) {
                ...
                if ($rel !== '') {
                    /* v4.94.0: بعد از تبدیل، PDF منبع از هاست حذف می‌شود؛ اگر تصاویر
                       صفحات موجود است، خود PDF دیگر همگام/آپلود نمی‌شود تا به هاست برنگردد. */
                    $isPdf = strtolower(pathinfo($rel, PATHINFO_EXTENSION)) === 'pdf';
                    $hasPages = $isPdf && (glob(dirname(__DIR__) . '/uploads/exams/pdf-pages/exam_' . (int)$r['id'] . '/page_*.jpg') ?: []);
    ...
    public static function run($force = false) {
        ...
        try {
            require_once __DIR__.'/class_exam_groups.php';
