...
?>
<div class="space-y-6"><?php /* v4.61.0: teacher hero + app-style grid on every teacher page */ require_once __DIR__ . '/includes/teacher_nav.php'; render_teacher_nav(); ?><div class="page-hero flex justify-between items-center"><div><h2 class="text-2xl font-bold"><svg data-ui-icon="exam" class="school-icon" viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><rect x="5" y="4" width="14" height="18" rx="2"/><path d="M9 2h6v4H9Zm0 8h6m-6 4h3m-3 4h6"/></svg> امتحانات</h2><p class="text-sm text-muted">مدیریت برنامه امتحانی، صندلی، کلاس امتحان و چاپ سربرگ سوالات</p></div><div class="flex gap-2"><a class="btn <?php echo $tab==='schedule'?'btn-primary':'btn-outline'; ?>" href="exams.php?tab=schedule&year=<?php echo urlencode($year); ?>">آزمون‌های ماهانه مدرسه</a><a class="btn <?php echo $tab==='class'?'btn-primary':'btn-outline'; ?>" href="exams.php?tab=class&amp;year=<?php echo urlencode($year); ?>">آزمون‌های کلاسی دبیران</a><a class="btn <?php echo $tab==='assign'?'btn-primary':'btn-outline'; ?>" href="exams.php?tab=assign&year=<?php echo urlencode($year); ?>">صندلی و کلاس</a><a class="btn <?php echo $tab==='print'?'btn-primary':'btn-outline'; ?>" href="exams.php?tab=print&year=<?php echo urlencode($year); ?>">چاپ</a><?php if(!(function_exists('current_teacher_is_executive') && current_teacher_is_executive() && !is_admin_logged_in())): ?><a class="btn btn-warning" href="exam-question-bank.php">بانک سوالات</a><?php endif; ?></div></div>
<?php if($tab==='schedule'): ?>
