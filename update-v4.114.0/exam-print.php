...
function renderBank(items){bankItems=items;const box=document.getElementById('bankList'); box.innerHTML='';
  const cnt=document.getElementById('bankCount'); if(cnt)cnt.textContent=(items.length).toLocaleString('fa-IR');
  items.forEach(b=>{const gr=b.grade_level?` <span style="background:#e0e9ff;color:#1d4ed8;border-radius:5px;padding:0 6px;font-size:10px">${esc(b.grade_level)}</span>`:'';
    /* v4.114.0: طراح و بارم کنار جزئیات */
    const designer=b.designer_name?` | طراح: ${esc(b.designer_name)}`:'';
    const score=(b.score!==null&&b.score!==''&&b.score!==undefined)?` | بارم: ${esc(String(b.score))}`:'';
    box.insertAdjacentHTML('beforeend',`<div class="bank-item"><b>${esc(b.subject_name)}</b>${gr} <small>${esc(b.academic_year||'')} ${esc(b.exam_month||'')}${designer}${score}</small><div class="bank-q">${b.question_html}</div><div style="display:flex;gap:4px;margin-top:5px"><button onclick="insertBankQuestion(${b.id})">درج در برگه</button>${canEditBank?`<button onclick="deleteBankQuestion(${b.id})">حذف از بانک</button>`:''}</div></div>`);});}
