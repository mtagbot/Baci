...
if (isset($_GET['print'])) {
    $D = [
        ...
        'nid'    => ($_GET['nid'] ?? '1') === '1',
        'year'   => ($_GET['year'] ?? '1') === '1',
        'theme'  => in_array(($_GET['theme'] ?? 'classic'), ['classic', 'ribbon', 'minimal'], true) ? $_GET['theme'] : 'classic',
    ];
...
:root{--cp:<?php echo clean($themeColor); ?>;--ca:<?php echo clean($accentColor); ?>}
...
(function(){
  function draw(cv){
    ...
    qr.make();
    var count = qr.getModuleCount(), quiet = 2, scale = 12;
  ...
  function drawAll(){ document.querySelectorAll('canvas.card-qr[data-qr]').forEach(draw); }
        ...
        <div class="grid grid-cols-4 gap-4">
            <div>
                ...
                <select id="cTheme" class="form-select" data-no-search="1" onchange="applyCardDesign()">
                    <option value="classic">کلاسیک — طاق و شمسه</option>
                    <option value="ribbon">نواری — سربرگ رنگی</option>
                    <option value="minimal">مینیمال — خطوط ساده</option>
                </select>
            </div>
            <div>
...
function applyCardDesign(){
  ...
  pv.querySelectorAll('.card-id').forEach(function(c){
    c.classList.remove('th-classic','th-ribbon','th-minimal');
...
(function(){
  function draw(cv){
    try {
      ...
      qr.addData(cv.getAttribute('data-qr'));
      qr.make();
      var count = qr.getModuleCount(), quiet = 2, scale = 10;
      var size = (count + quiet*2) * scale;
      cv.width = size; cv.height = size;
      var ctx = cv.getContext('2d');
      ctx.fillStyle = '#fff'; ctx.fillRect(0,0,size,size);
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    applyCardDesign();
    document.querySelectorAll('canvas.card-qr[data-qr]').forEach(draw);
  });
})();
</script>
