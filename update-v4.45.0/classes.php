...
<div class="space-y-6">
        ...
        <div class="card col-span-1 shadow-lg">
            ...
            <form method="POST">
                ...
                <div class="mb-4">
                    ...
                    <select name="name_select" id="subjPickSel" class="form-select font-bold" onchange="toggleManualSubj()">
                        <optgroup label="دروس رسمی متوسطه اول (هفتم تا نهم)">
                            <?php foreach ($standardSubjects as $sn): ?>
                                <option value="<?php echo clean($sn); ?>"><?php echo clean($sn); ?></option>
                            <?php endforeach; ?>
                        </optgroup>
                        <?php $extraSubjects = array_values(array_diff($subjectPickerOptions, $standardSubjects)); if ($extraSubjects): ?>
                        <optgroup label="دروس موجود در سیستم">
                            <?php foreach ($extraSubjects as $sn): ?>
                                <option value="<?php echo clean($sn); ?>"><?php echo clean($sn); ?></option>
                            <?php endforeach; ?>
                        </optgroup>
                        <?php endif; ?>
                        <option value="__manual__">سایر... (تایپ دستی نام درس)</option>
                    </select>
                    <input type="text" name="name" id="subjManualInp" class="form-input font-bold mt-2" placeholder="نام درس را تایپ کنید" style="display:none">
                </div>
                <div class="mb-4">
            ...
            <script>
            function toggleManualSubj(){
                var s=document.getElementById('subjPickSel'), i=document.getElementById('subjManualInp');
                if(s&&i){ var manual = s.value==='__manual__'; i.style.display = manual?'block':'none'; i.required = manual; if(!manual) i.value=''; }
            }
            document.addEventListener('DOMContentLoaded', toggleManualSubj);
            </script>
        </div>
    ...
    <div class="card shadow-lg space-y-4">
        ...
        <form method="POST" class="grid grid-cols-6 gap-2 items-end bg-emerald-50 dark:bg-slate-800 p-3 rounded border" style="border-color:#6ee7b7">
            ...
            <!-- v4.44.0: تک زنگ — درس دوم در همان زنگ، هفته در میان -->
            <div style="grid-column: 1 / -1" class="pt-1">
                <label class="flex items-center gap-2 text-xs font-bold cursor-pointer">
                    <input type="checkbox" name="is_half_period" id="halfPeriodChk" value="1" onchange="toggleHalfPeriod()">
                    تک زنگ (دو درس در یک زنگ — هفته در میان تدریس می‌شوند)
                </label>
            </div>
            <div id="halfPeriodRow" style="display:none;grid-column: 1 / -1" class="grid grid-cols-6 gap-2 items-end p-2 rounded border" >
                <div style="grid-column: span 2" class="text-xs text-muted self-center">درس دوم همین زنگ (هفتهٔ بعد):</div>
                <div>
                    <label class="block text-xs font-semibold mb-1">درس دوم *</label>
                    <select name="subject_name2" id="subj2Sel" class="form-select text-xs font-bold">
                        <option value="">---</option>
                        <?php foreach ($subjectNameOptions as $sn): ?><option value="<?php echo clean($sn); ?>"><?php echo clean($sn); ?></option><?php endforeach; ?>
                    </select>
                </div>
                <div>
                    <label class="block text-xs font-semibold mb-1">دبیر درس دوم</label>
                    <select name="teacher_id2" class="form-select text-xs font-bold">
                        <option value="0">تعیین نشده</option>
                        <?php foreach ($teachersList as $tl): ?><option value="<?php echo $tl['id']; ?>"><?php echo clean($tl['full_name']); ?></option><?php endforeach; ?>
                    </select>
                </div>
                <div style="grid-column: span 2" class="text-[11px] text-muted self-center">مثال: «فرهنگ و هنر / ریاضی» — یک هفته درس اول، هفته بعد درس دوم.</div>
            </div>
        </form>
        <script>
        function toggleHalfPeriod(){
            var c=document.getElementById('halfPeriodChk'), r=document.getElementById('halfPeriodRow'), s=document.getElementById('subj2Sel');
            if(c&&r){ r.style.display = c.checked ? 'grid' : 'none'; if(s){ s.required = !!c.checked; if(!c.checked) s.value=''; } }
        }
        </script>
            ...
            <table>
                ...
                <tbody>
                    <?php foreach ($schedList as $sc): $rid = (int)$sc['id'];
                        $slotKey = $sc['class_name'].'|'.$sc['day_of_week'].'|'.$sc['period_num'];
                        $isShared = ($slotCounts[$slotKey] ?? 1) > 1;
                        $mates = [];
                        if ($isShared) { foreach (($slotMates[$slotKey] ?? []) as $mateSubj) { if ($mateSubj !== $sc['subject_name']) $mates[] = $mateSubj; } }
                    ?>
                    <!-- display row -->
                    <tr id="schedView<?php echo $rid; ?>">
                        ...
                        <td>
                            <span class="badge badge-info"><?php echo clean($sc['period_num']); ?></span>
                            <?php if ($isShared): ?><span class="badge badge-warning text-[10px]" title="دو درس در این زنگ — هفته در میان">تک زنگ</span><?php endif; ?>
                        </td>
                        <td class="font-bold">
                            <?php echo clean($sc['subject_name']); ?>
                            <?php if ($isShared && $mates): ?><span class="text-[10px] text-muted d-block" style="display:block">هفته در میان با: <?php echo clean(implode('، ', $mates)); ?></span><?php endif; ?>
                        </td>
