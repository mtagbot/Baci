...
if ($text === '/start') {
    ...
    if ($staffOnStart) {
        ...
        $tFamS = function_exists('teacher_respectful_name') ? teacher_respectful_name($staffOnStart) : $staffOnStart['full_name'];
        $roleNames = [];
...
if ($step === 'admin_username') {
    /* v4.152.0: نام کاربری پیش از پرسیدن رمز بررسی می‌شود تا پیام «رمز نادرست»
       برای یک نام کاربری ناموجود هم گمراه‌کننده نباشد. */
    $adminRow = bot_login_admin_by_username($text);
    if (!$adminRow) {
        DB::execute("DELETE FROM `$stateTable` WHERE `$chatCol`=?", [$chatId]);
        bot_webhook_safe_send($platform, $chatId, '❌ مدیر فعالی با این نام کاربری پیدا نشد. برای شروع دوباره /start را بزنید.', bot_main_keyboard($platform));
        http_response_code(200); exit;
    }
    bot_login_ask_admin_password($platform, $chatId, $adminRow, trim(tr_num($text, 'en')));
    http_response_code(200); exit;
}
if ($step === 'admin_password') {
    $username = (string)($state['temp_nid'] ?? '');
    $admin = bot_login_admin_by_username($username);
    if ($admin && verify_user_password($text, $admin['password'], ['table'=>'admins','id'=>$admin['id']])) {
        /* v4.152.0: این گفتگو به حساب مدیریت وصل می‌شود؛ نشست‌های قبلی همین
           گفتگو (مدیر یا دبیر) غیرفعال می‌شوند تا یک حساب یک ردیف فعال داشته
           باشد و پیام آزمایشی «تگ آزمایشی» فقط یک‌بار به این گفتگو برسد. */
        DB::execute("UPDATE bot_admin_sessions SET is_active=0 WHERE platform=? AND chat_id=?", [$platform, $chatId]);
        DB::execute("INSERT INTO bot_admin_sessions (platform, chat_id, admin_id, role_type, is_active, created_at_jalali) VALUES (?, ?, ?, 'admin', 1, ?)", [$platform, $chatId, $admin['id'], jalali_now()]);
        DB::execute("DELETE FROM `$stateTable` WHERE `$chatCol`=?", [$chatId]);
        $url = bot_create_login_url('admin', $admin['id']);
        $aName = trim((string)($admin['name'] ?? ''));
        $msg = '✅ ورود مدیر تأیید شد' . ($aName !== '' ? ': ' . $aName : '') . "\n\n"
             . '🔗 حساب مدیریت شما روی ربات «' . bot_login_platform_fa($platform) . '» ثبت شد؛ از این پس پیام‌های مدیریتی — از جمله پیام آزمایشی «تگ آزمایشی» — به همین گفتگو فرستاده می‌شود.' . "\n\n"
             . "برای دسترسی کامل به تمام امکانات پنل مدیریت از لینک امن زیر استفاده کنید (اعتبار ۱۵ دقیقه):\n{$url}\n\n"
             . 'میان‌برها: دانش‌آموزان، دبیران، کلاس‌ها، برنامه هفتگی، اعلان‌ها، ربات‌ها و تنظیمات همگی از همین پنل در دسترس‌اند.';
        bot_webhook_safe_send($platform, $chatId, $msg, bot_main_keyboard($platform));
    } else {
        /* سه تلاش؛ بعد از آن گفتگو بسته می‌شود تا کسی رمز مدیر را حدس نزند. */
        $tries = (int)($state['temp_payload'] ?? 0) + 1;
        if ($tries >= 3) {
            DB::execute("DELETE FROM `$stateTable` WHERE `$chatCol`=?", [$chatId]);
            bot_webhook_safe_send($platform, $chatId, '⛔️ سه بار رمز نادرست وارد شد؛ برای امنیت این مرحله بسته شد. برای شروع دوباره /start را بزنید.', bot_main_keyboard($platform));
        } else {
            DB::execute("UPDATE `$stateTable` SET temp_payload=? WHERE `$chatCol`=?", [(string)$tries, $chatId]);
            bot_webhook_safe_send($platform, $chatId, '❌ نام کاربری یا رمز مدیر نادرست است. دوباره رمز را ارسال کنید (تلاش ' . tr_num($tries, 'fa') . ' از ' . tr_num(3, 'fa') . '):');
        }
    }
    http_response_code(200); exit;
}
if ($step === 'teacher_username') {
    /* v4.152.0: مدیر می‌تواند به‌جای کد ملی دبیر، نام کاربری مدیریت را بفرستد.
       ورودی ده‌رقمی همچنان مسیر دبیر/دانش‌آموز است. */
    $rawTeacherId = trim(tr_num((string)$text, 'en'));
    if (!preg_match('/^\d{10}$/', $rawTeacherId) && bot_login_manager_entry($platform, $chatId, $text)) {
        http_response_code(200); exit;
    }
...
if ($step === 'teacher_password') {
    ...
    if ($teacher && verify_user_password($text, $teacher['password'], ['table'=>'teachers','id'=>$teacher['id']])) {
        ...
        $tFam = function_exists('teacher_respectful_name') ? teacher_respectful_name($teacher) : $teacher['full_name'];
...
if ($step === 'awaiting_nid') {
    /* v4.152.0 — ورود مدیر از همین مرحله: مدیر در پاسخ به «کد ملی ۱۰ رقمی»
       نام کاربری خودش را می‌فرستد. پیش از این، نام کاربری با پیام خطای
       «کد ملی باید دقیقاً ۱۰ رقم عددی باشد» رد می‌شد و حساب مدیریت هرگز به
       ربات اضافه نمی‌شد. */
    $rawIdentity = trim(tr_num((string)$text, 'en'));
    if (!preg_match('/^\d{10}$/', $rawIdentity) && bot_login_manager_entry($platform, $chatId, $text)) {
        http_response_code(200);
        exit;
    }
    ...
    if (!preg_match('/^\d{10}$/', $nid)) {
        ...
        bot_webhook_safe_send($platform, $chatId, $invalidNidMsg);
        http_response_code(200);
        exit;
    }
    ...
    if ($staffMatch) {
        ...
        $tFamD = function_exists('teacher_respectful_name') ? teacher_respectful_name($staffMatch) : $staffMatch['full_name'];
    ...
    if (!$student) {
        ...
        if (bot_login_manager_entry($platform, $chatId, $text)) { http_response_code(200); exit; }
...
if ($step === 'staff_personnel_code') {
    ...
    $okStaff = false;
    if ($teacher) {
        /* v4.131.0: کد پرسنلی اینجا عمداً «عامل اتصال حساب» است، نه رمز —
           پس می‌ماند. آنچه حذف شد مقایسه با ستون خام password است. */
        $okStaff = hash_equals((string)$teacher['personnel_code'], $code)
            || verify_user_password($code, (string)$teacher['password'], ['table'=>'teachers','id'=>$teacher['id']]);
    }
    if ($okStaff) {
        DB::execute("UPDATE bot_admin_sessions SET is_active=0 WHERE platform=? AND chat_id=? AND role_type='teacher'", [$platform, $chatId]);
        DB::execute("INSERT INTO bot_admin_sessions (platform, chat_id, teacher_id, role_type, created_at_jalali) VALUES (?, ?, ?, 'teacher', ?)", [$platform, $chatId, $teacher['id'], jalali_now()]);
        ...
        $tFamL = function_exists('teacher_respectful_name') ? teacher_respectful_name($teacher) : $teacher['full_name'];
    ...
    } else if (bot_login_manager_entry($platform, $chatId, $text)) {
        /* v4.152.0: نام کاربری مدیریت در این مرحله هم پذیرفته می‌شود. */
    } else {
...
if ($step === 'awaiting_serial') {
    ...
    /* v4.152.0: اگر مدیر نام کاربری خودش را اینجا بفرستد، ورود مدیر شروع
       می‌شود؛ فقط بعد از رد شدن سریال دانش‌آموز. */
    if (bot_login_manager_entry($platform, $chatId, $text)) { http_response_code(200); exit; }
