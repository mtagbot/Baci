...
function exam_pdf_pages_to_images($pdfRelPath, $examId, $declaredPages = 0) {
    ...
    $out = array_slice($out, 0, $declaredPages);
    /* v4.94.0: تبدیل کامل شد → PDF منبع دیگر لازم نیست؛ برای آزادسازی فضای هاست حذف می‌شود */
    if (count($out) >= $declaredPages && $declaredPages >= 1) @unlink($full);
    return $out;
}
...
<div class="editor-panel" id="questionEditorPanel">
  ...
  <label>شماره ردیف سوال</label><input id="qNumber" type="number" min="1" value="1"><label>بارم / نمره</label><input id="qScore" type="text" value="1"><label>ارتفاع همین سوال</label><input id="qHeight" type="range" min="10" max="90" value="18" oninput="previewQuestionHeight(this.value)"><label>اندازه فونت سوال</label><input id="qFontSize" type="range" min="8" max="24" value="11" oninput="applyFontToEditor()"><label>فونت متن سوال</label><select id="qFont" onchange="applyFontToEditor()"><option value="Vazirmatn">وزیرمتن (Vazirmatn)</option><option value="Yekan">یکان (Yekan)</option><option value="Sahel">ساحل (Sahel)</option><option value="BTitr">تیتر (B-Titr)</option><option value="Tahoma">Tahoma</option><option value="serif">Serif</option><option value="sans-serif">Sans</option></select><label>افزودن فونت دلخواه</label><input type="file" accept=".ttf,.otf,.woff,.woff2" onchange="loadCustomFont(this)">
...
<div class="bank-panel collapsed" id="bankPanel">
  <b>بانک اطلاعاتی سوالات</b> <small id="bankSubjectBadge" style="display:none;background:#dbeafe;color:#1e40af;border-radius:6px;padding:2px 7px;font-weight:700"></small>
...
function pageStyleAttr(){/* v4.91.0: اندازه عکس/مهر ثابت است — مقادیر ذخیره‌شده قدیمی نادیده گرفته می‌شود */ return Object.entries(pageStyle).filter(([k])=>k!=='--photo'&&k!=='--stamp').map(([k,v])=>`${k}:${v}`).join(';');}
...
function showBankSubject(subj){const el=document.getElementById('bankSubjectBadge'); if(!el)return; if(subj){el.textContent='فقط درس: '+subj; el.style.display='inline-block';} else el.style.display='none';}
...
let designBankItems=[];
function renderDesignBank(items){designBankItems=items||[];const box=document.getElementById('designBankList'); if(!box)return; box.innerHTML=''; if(!designBankItems.length){box.innerHTML='<small>آزمون ذخیره‌شده‌ای با فایل منبع یافت نشد.</small>'; return;} designBankItems.forEach(x=>{const thumbs=bankPageViews(x).slice(0,6).map(p=>`<span style="display:inline-block;background:#fff;overflow:hidden;flex:none"><img src="${esc(p.src)}" loading="lazy" style="${p.style};height:86px;width:auto"></span>`).join(''); box.insertAdjacentHTML('beforeend',`<div class="bank-exam"><b>${esc(x.subject||'')}</b> <small>${esc(x.grade||'')} ${esc(x.class||'')}</small><br><small>طراح: ${esc(x.designer||'—')} | ماه: ${esc(x.month||'—')} | سال: ${esc(x.year||'—')}</small><div class="bank-exam-thumbs" onclick="openExamZoom(${x.exam_id})" title="بزرگ‌نمایی صفحات">${thumbs}</div><div style="display:flex;gap:4px"><button onclick="importSavedExam(${x.exam_id})">درج این آزمون در ویرایشگر</button>${canEditBank?`<button onclick="deleteSavedExam(${x.exam_id})" style="background:#fee2e2">حذف از بانک</button>`:''}</div></div>`);});}
function openExamZoom(id){const x=designBankItems.find(e=>+e.exam_id===+id); if(!x)return; document.getElementById('examZoomPages').innerHTML=`<div style="margin-bottom:8px"><b>${esc(x.subject||'')}</b> — طراح: ${esc(x.designer||'—')} | ماه: ${esc(x.month||'—')} | سال: ${esc(x.year||'—')}</div>`+bankPageViews(x).map(p=>`<div style="background:#fff;border:1px solid #e2e8f0;border-radius:6px;margin-bottom:8px;overflow:hidden;text-align:center"><img src="${esc(p.src)}" style="${p.style};border:0;margin:0"></div>`).join(''); document.getElementById('examZoomModal').style.display='flex';}
function closeExamZoom(){document.getElementById('examZoomModal').style.display='none';}
function importSavedExam(id){if(!confirm('آزمون انتخابی از بانک در ویرایشگر فعلی درج شود؟ طراحی فعلی جایگزین می‌شود.'))return; const fd=new FormData(); fd.append('action','import_design'); fd.append('exam_id',examId); fd.append('dt',designToken); fd.append('source_exam_id',id); fetch('exam-design-api.php',{method:'POST',body:fd}).then(r=>r.json()).then(j=>{if(!j.ok){alert(j.error||'خطا در درج آزمون'); return;} examData.sourcePages=j.sourcePages||[]; const d=j.design||{}; qItems=d.questions||[]; if(d.headerFields) headerFields=fixHeaderFieldOrder(d.headerFields); drawingsData=d.drawings||{}; sourceCrops=d.sourceCrops||{}; sourceOrder=d.sourceOrder||[]; pageStyle=fixLoadedHeaderH(d.style||{}); printNote=d.printNote||''; const pn=document.getElementById('printNote'); if(pn)pn.value=printNote; renderHeaderEditor(); rerenderPages(); autosaveLocal(); alert(j.message||'آزمون درج شد.');});}
function deleteSavedExam(id){if(!confirm('این آزمون ذخیره‌شده به‌طور کامل از بانک حذف شود؟'))return; const fd=new FormData(); fd.append('action','delete_design'); fd.append('exam_id',examId); fd.append('dt',designToken); fd.append('source_exam_id',id); fetch('exam-design-api.php',{method:'POST',body:fd}).then(r=>r.json()).then(j=>{if(j.ok)loadBank(); else alert(j.error||'خطا');});}
