...
if (!function_exists('att_process_scan')) {
    ...
    function att_process_scan($payload) {
        ...
        if ($existing) {
            ...
            if ($existing['status'] === 'absent' && $existing['source'] === 'auto') {
                ...
                if (get_setting('att_notify_late', '1') === '1') {
                    ...
                    att_defer(function () use ($sid, $today, $minutes, $exId) {
                        $n = notify_student_attendance_bots($sid, 'late', $today, $minutes, 'ورود با تأخیر پس از ثبت غیبت خودکار', $exId);
        ...
        if (get_setting('att_notify_late', '1') === '1') {
            att_defer(function () use ($sid, $today, $minutes, $rid) {
