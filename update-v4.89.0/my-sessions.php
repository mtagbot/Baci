    ...
    function ms_verify_account_password($uType, $uId, $pass) {
        ...
        try {
            if ($uType === 'admin') {
                ...
                return $row && (password_verify($pass, $row['password']) || hash_equals((string)$row['password'], $pass) || md5($pass) === $row['password']);
            }
            if ($uType === 'teacher') {
                $row = DB::fetch("SELECT password, personnel_code FROM teachers WHERE id=?", [$uId]);
                return $row && (password_verify($pass, (string)$row['password']) || hash_equals((string)$row['password'], $pass) || hash_equals((string)$row['personnel_code'], $pass));
            }
            if ($uType === 'student') {
                $row = DB::fetch("SELECT password, serial_number, national_id FROM students WHERE id=?", [$uId]);
                return $row && (password_verify($pass, (string)$row['password']) || hash_equals((string)$row['password'], $pass) || hash_equals((string)$row['serial_number'], $pass) || hash_equals((string)$row['national_id'], $pass));
            }
        } catch (Exception $e) {}
        return false;
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['revoke_session'])) {
    if (verify_csrf($_POST['csrf_token'] ?? '')) {
        if (!ms_verify_account_password($uType, $uId, $_POST['account_password'] ?? '')) {
            set_flash_message('error', 'رمز حساب کاربری نادرست است. بدون تایید رمز، بستن نشست انجام نمی‌شود.');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['revoke_all_others'])) {
    if (verify_csrf($_POST['csrf_token'] ?? '')) {
        if (!ms_verify_account_password($uType, $uId, $_POST['account_password'] ?? '')) {
            set_flash_message('error', 'رمز حساب کاربری نادرست است. بدون تایید رمز، بستن نشست‌ها انجام نمی‌شود.');
            redirect('my-sessions.php');
        }
...
<div class="w-full space-y-4">
    <div class="card shadow-lg">
        ...
        <p class="text-[11px] text-muted mt-3">توضیح: «آخرین فعالیت» با دقت حدود یک دقیقه به‌روزرسانی می‌شود. نشست‌های بی‌تحرک بیش از ۶۰ روز خودکار حذف می‌شوند. بستن هر نشست نیازمند تایید رمز حساب کاربری است.</p>
    </div>
</div>
