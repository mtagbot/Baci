    ...
    function bot_exam_source_page_files($exam) {
        ...
        if (class_exists('Imagick')) {
            ...
            for ($i = 0; $i < $declared; $i++) {
                try {
                    $pg = new Imagick(); $pg->setResolution(180, 180); $pg->readImage($full . '[' . $i . ']');
...
if (!function_exists('bot_exam_design_pdf_path')) {
    ...
    function bot_exam_design_pdf_path($examId) {
        ...
        $S = 6.0;                                     // render scale: px per mm  (~152 dpi)
        $PX = 25.4 / 96.0 * $S;                       // css-px → render-px
        $W = (int)round(210 * $S); $H = (int)round(297 * $S);
        $headerH = bot_exam_css_mm($style['--headerH'] ?? '', 34);
        $photoMm = bot_exam_css_mm($style['--photo'] ?? '', 18);
        $stampMm = bot_exam_css_mm($style['--stamp'] ?? '', 18);
        ...
        // layout height of the source block (clip/translate do NOT affect layout — like the browser)
        ...
        foreach ($questions as $q) {
            ...
            $rows[] = ['page' => $pageNo, 'y' => $y, 'h' => $rowH, 'lines' => $lines, 'pt' => $pt,
                       'lineH' => $lineH, 'no' => $qNo, 'score' => trim((string)($q['score'] ?? '')), 'imgs' => $imgs];
            $tblYs[$pageNo] = $y + $rowH;
            $rowCount[$pageNo] = ($rowCount[$pageNo] ?? 0) + 1;
        }
        $totalDesignPages = max($srcCount, $pageNo, 1);
        ...
        for ($p = 1; $p <= $totalDesignPages; $p++) {
            ...
            ob_start(); imagejpeg($page, null, 87); $bodyJpegs[$p] = ob_get_clean();
            imagedestroy($page);
        }
        ...
        $fixedVals = [
            ...
            'teacher' => 'آقای ' . (string)($exam['t_name'] ?? ''),
        ...
        foreach ($students as $si => $st) {
            ...
            // photo box (right, RTL first grid column)
            $px1 = $W - $mm4 - $photoW; $py1 = (int)(($hH - $photoW) / 2);
            imagerectangle($strip, $px1, $py1, $px1 + $photoW, $py1 + $photoW, $ink);
            $photoPath = '';
            ...
            if ($photoPath !== '') {
                ...
                if ($ph) {   // object-fit: cover
                    $pw0 = imagesx($ph); $ph0 = imagesy($ph);
                    $sc = max($photoW / max(1, $pw0), $photoW / max(1, $ph0));
                    $cw = (int)round($photoW / $sc); $ch = (int)round($photoW / $sc);
                    $ox = (int)(($pw0 - $cw) / 2); $oy = (int)(($ph0 - $ch) / 2);
                    imagecopyresampled($strip, $ph, $px1 + 1, $py1 + 1, $ox, $oy, $photoW - 1, $photoW - 1, $cw, $ch);
                    imagedestroy($ph);
                }
            } else {
                gd_text($strip, 'عکس', $px1 + (int)($photoW / 2), $py1 + (int)($photoW / 2) + 6, 9 * $PX * 0.75, $box, false, 'center');
            }
            // stamp box (left)
            $sx1 = $mm4; $sy1 = (int)(($hH - $stampW) / 2);
            ...
            $boxH = (int)round((10 * 1.45) * $PX + 2 * $S);
            $gap = (int)round(1.2 * $S);
            $titlePt = 11 * $PX * 0.75; $subPt = 10 * $PX * 0.75; $fieldPt = 10 * $PX * 0.75;
            ...
            gd_text($strip, $exTitle, $cx1 + (int)($cw / 2), $yCur, $titlePt, $ink, true, 'center');
            ...
            ob_start(); imagejpeg($strip, null, 90); $headerJpegs[$si] = ob_get_clean();
            imagedestroy($strip);
        }
...
if (!function_exists('bot_send_exam_design_pdf')) {
    function bot_send_exam_design_pdf($platform, $chatId, $examId) {
        ...
        return bot_send_document($platform, $chatId, $info['path'], $caption, null, 'application/pdf', 'Azmoon-' . (int)$examId . '.pdf');
    }
}
...
if (!function_exists('bot_role_finish_objection')) {
    ...
    function bot_role_finish_objection($platform, $chatId, $msgId, $newScore, $staff) {
        ...
        $tFam = function_exists('teacher_respectful_name') ? teacher_respectful_name($staff['full_name']) : $staff['full_name'];
