...
let designBankItems=[];
function renderDesignBank(items){designBankItems=items||[];const box=document.getElementById('designBankList'); if(!box)return; box.innerHTML=''; if(!designBankItems.length){box.innerHTML='<small>آزمون ذخیره‌شده‌ای با فایل منبع یافت نشد.</small>'; return;} designBankItems.forEach(x=>{const thumbs=(x.pages||[]).slice(0,6).map(pg=>`<img src="${esc(pg)}" loading="lazy">`).join(''); box.insertAdjacentHTML('beforeend',`<div class="bank-exam"><b>${esc(x.subject||'')}</b> <small>${esc(x.grade||'')} ${esc(x.class||'')}</small><br><small>طراح: ${esc(x.designer||'—')} | ماه: ${esc(x.month||'—')} | سال: ${esc(x.year||'—')}</small><div class="bank-exam-thumbs" onclick="openExamZoom(${x.exam_id})" title="بزرگ‌نمایی صفحات">${thumbs}</div><div style="display:flex;gap:4px"><button onclick="importSavedExam(${x.exam_id})">درج این آزمون در ویرایشگر</button>${canEditBank?`<button onclick="deleteSavedExam(${x.exam_id})" style="background:#fee2e2">حذف از بانک</button>`:''}</div></div>`);});}
function openExamZoom(id){const x=designBankItems.find(e=>+e.exam_id===+id); if(!x)return; document.getElementById('examZoomPages').innerHTML=`<div style="margin-bottom:8px"><b>${esc(x.subject||'')}</b> — طراح: ${esc(x.designer||'—')} | ماه: ${esc(x.month||'—')} | سال: ${esc(x.year||'—')}</div>`+(x.pages||[]).map(pg=>`<img src="${esc(pg)}">`).join(''); document.getElementById('examZoomModal').style.display='flex';}
