...
async function checkLocationRobust(){
    /* v4.124.0: فرایند کاملاً خودکار و بی‌صدا — دانش‌آموز فقط «لطفا صبر کنید» می‌بیند.
       آبشار داخلی (نامرئی): دقیق (GPS ماهواره‌ای) → متعادل (شبکه+GPS) → کم‌دقت (شبکه) → IP.
       در هر صورت دانش‌آموز وارد آزمون می‌شود؛ هیچ انتخاب/دکمه‌ای به او داده نمی‌شود. */
    let statusEl = document.querySelector('#check_location .status');
    let detailEl = document.getElementById('locationDetail');
    statusEl.textContent='لطفا صبر کنید…';
    if(detailEl) detailEl.textContent='';

    const finishOk = (mode) => {
        permissionState.location = true;
        statusEl.textContent = '✅ تایید شد';
        if(detailEl) detailEl.textContent = '';
        document.getElementById('check_location').classList.add('bg-green-50');
        document.getElementById('check_location').classList.remove('bg-red-50','bg-amber-50');
        checkAllPermissionsDone();
    };

    /* بدون geolocation یا http: مستقیم IP — بی‌صدا */
    const geoUsable = navigator.geolocation && (location.protocol==='https:' || location.hostname==='localhost' || location.hostname==='127.0.0.1');
    ...
    if(geoUsable){
        /* آبشار سه‌مرحله‌ای: دقیق → متعادل → کم‌دقت (هر مرحله چند استراتژی) */
        const cascade = [
            /* دقیق: GPS ماهواره‌ای */
            [{enableHighAccuracy:true,  timeout:20000, maximumAge:0},
             {enableHighAccuracy:true,  timeout:25000, maximumAge:10000}],
            /* متعادل: شبکه سپس GPS سبک */
            [{enableHighAccuracy:false, timeout:12000, maximumAge:30000},
             {enableHighAccuracy:true,  timeout:15000, maximumAge:30000}],
            /* کم‌دقت: فقط شبکه/وای‌فای */
            [{enableHighAccuracy:false, timeout:15000, maximumAge:60000},
             {enableHighAccuracy:false, timeout:20000, maximumAge:0}],
        ];
        for(const stage of cascade){
            for(const opts of stage){
                try {
                    const pos = await tryGetPos(opts);
                    window._lastPos = pos;
                    logProctor('location_acquired', 'acc='+Math.round(pos.coords.accuracy||0)+'m highAcc='+(opts.enableHighAccuracy?1:0));
                    finishOk(opts.enableHighAccuracy?'high':'low');
                    return;
                } catch(err){ /* بی‌صدا به مرحله بعد */ }
            }
        }
    }

    /* همه مراحل GPS شکست خورد (یا http بود): موقعیت تقریبی IP — همچنان بی‌صدا و بدون توقف */
    try {
    ...
    } catch(e){ window._lastPos = null; }
    logProctor('location_ip_fallback', 'GPS unavailable — IP-approx or unknown');
    finishOk('ip');
}
...
function startTimer(){
    ...
    function updateTimerDisplay(){
        ...
        if(remainingSeconds<=0){
            ...
            /* v4.123.0: بدون alert مسدودکننده — بنر + ارسال خودکار */
            td.textContent='⏱ 00:00:00';
            let bn=document.createElement('div');
            bn.style.cssText='position:fixed;top:12px;left:50%;transform:translateX(-50%);z-index:1300;background:linear-gradient(135deg,#dc2626,#ef4444);color:#fff;padding:12px 24px;border-radius:999px;font-weight:800;font-size:14px;box-shadow:0 8px 28px rgba(220,38,38,.45)';
            bn.textContent='⏰ زمان آزمون تمام شد — پاسخ‌های ذخیره‌شده شما در حال ارسال است…';
...
function saveAnswer(qid){
    ...
    setAutosaveState('saving'); fetch('online-exam-api.php',{method:'POST',body:fd}).then(()=>updateProgress());
    updateProgress(); setAutosaveState('saved');
}
...
function setAutosaveState(st){
    ...
    if(st==='saving'){dot.classList.add('saving'); if(txt)txt.textContent='در حال ذخیره…';}
    else if(st==='saved'){dot.classList.add('saved'); if(txt)txt.textContent='ذخیره شد ✓'; setTimeout(()=>{if(txt)txt.textContent='ذخیره خودکار فعال'; dot.classList.remove('saved');},2200);}
}
